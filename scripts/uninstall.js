...
// The extension class admits `*` because the drift gate compares against the
// NORMALISED name: architecture-drift extracts `rework-*.fired-*-*` from the
// hook source, where each `${VAR}` collapses to `*`. Without it, a state file
...
export const CLAUDEMD_STATE_FILE_RE =
  /^(?:(?:ext-read|failopen|mem-coverage|vocab-scan|rework|xrepo)-[A-Za-z0-9_*-]*(?:\.[A-Za-z0-9*-]+)?|session-start(?:-[A-Za-z0-9_*-]+)?\.ref|sandbox-pending(?:-[A-Za-z0-9_*-]+)?\.list|session-summary(?:-[A-Za-z0-9_*-]+)?\.lastrun|tmp-baseline(?:-[A-Za-z0-9_*-]+)?\.txt|last-session-summary\.json(?:\.last-shown)?|upstream-check\.lastrun|bootstrap-failed\.json(?:\.last-shown)?|hook-root\.json(?:\.[0-9]+)?|user-content-backup\.json|statusline-prev\.json|mem-audit\.lastrun|tmp-sweep\.(?:stamp|last\.json|lock)|l2-task-counter|ship-baseline-recent|installed\.json|install\.lock)$/;
