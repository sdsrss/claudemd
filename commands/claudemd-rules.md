---
name: claudemd-rules
description: Audit the HARD-rules manifest. Cross-references spec/hard-rules.json with rule-hits.jsonl bySection data over the last N days (default 90). Surfaces §13.1 quarterly demote candidates (hook-enforced rules with 0 hits in window) and stale-review entries.
---

Default window is 90 days (matches §13.1 quarterly demote cadence). If the user passes a number (e.g. `/claudemd-rules 30`), set `CLAUDEMD_RULES_DAYS=$ARGS` before invocation.

Run: `CLAUDEMD_RULES_DAYS=${ARGS:-90} node ${CLAUDE_PLUGIN_ROOT}/scripts/hard-rules-audit.js`
...
| `staleReviews` | rules whose `last_demote_review` is null or older than window — operator's quarterly task list |
