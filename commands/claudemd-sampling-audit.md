...
- **G0 behaviour** (`behaviorMetrics`): rework ≥8, test-edit disposition, Skill rate. `reworkSessions` / `editedSessions` count Edit/Write only — the pre-registered number. The `…AnyChannel` pair also counts files a Bash command modified (`toolUseResult.bashEditDiff`, CC ≥2.1.278), with `editsViaTool` / `editsViaBash` beside it; `bashEditCapableSessions` says how many sessions ran on a CLI that records them. `skillInvocations` is the pre-registered count of every Skill call; `skillInvocationErrors` is the subset whose result came back as an error (`Unknown skill: …`), so succeeded = the difference. Subagent transcripts (`<session>/subagents/*.jsonl`, CC ≥2.1.278) are counted in `subagentBehaviorMetrics` with `subagentTranscripts`, not in the main block, and the rule detectors do not read them.
