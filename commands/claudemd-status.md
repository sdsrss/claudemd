...
- When `--verbose` is passed: the JSON includes a `verbose` block with `verbose.killSwitches.perHook` (env var name + event + effective vs persisted state for every shipped hook), `verbose.killSwitches.subFeature` (toggles that silence one feature of a hook without disabling the hook), and `verbose.escapeTokens` (every per-invocation bypass token with its target hook + spec section — read the count off the list rather than quoting one here). Use this when the user asks "which env var disables hook X" or "what's the bypass token for §Y" without having them grep README.
