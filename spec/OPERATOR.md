...
- **Carried to next §13.2 batch review**: 6 §4 Routing primaries `off` in `skillOverrides` with 0 measured invocations — re-enable-vs-rewrite. **Re-queued 2026-09-01**: this item was in the 2026-07-25 carry list and then appeared in no batch-review record — neither adjudicated nor re-carried, i.e. it fell off the governance loop silently while the line above it stayed listed as carried after being decided. Both halves of that failure are what this pair of entries now makes visible: a settled item must say so, and an unsettled one must survive the review that skipped it.
