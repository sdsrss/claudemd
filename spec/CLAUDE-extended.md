# AI-CODING-SPEC v6.16.0 — Extended
...
**v6.16.0 (minor, 2026-07-11)** — §11-EXT ship-runbook consolidation (SHOULD):

- `[add]` **§11-EXT MEMORY.md tag syntax → Ship-runbook consolidation**: ship-trigger tags concentrate on exactly ONE memory file per project (the ship runbook holding the full release flow); ship-adjacent lessons are `[[linked]]` from it, not ship-tagged. §11 read-the-file at ship then costs one predictable Read. Grounded in memory-read-check telemetry 2026-05-20 → 2026-07-10 (~20 deny events: modal match_count=1, but recurring generic-tag FP fan-out — bypass reasons dominated by "residual keyword tag hits"). SHOULD-level — `hard-rules.json` rules unchanged.

### Why minor (and why now)

Rule added (backward-compatible SHOULD) → minor per §13 META. Ships one day after v6.15.0: `OPERATOR.md §13.1` ≥20-task minor-spacing overridden by explicit operator request (2026-07-11 session); the spacing rule guards against rules-without-invocation-data — this rule is derived from 7 weeks of rule-hits telemetry.

**Older entries** (v6.15.1 §0.1 operator-threshold relocation, v6.15.0 §2.1 Model tiering + Candidate-1 net-delete, v6.14.2 trigger-list `e.g.` markers + context7 conditionalized, v6.14.1 §2.1 skill-MUST-invoke clarification, v6.14.0 §10 template relax + vocab trim, v6.13.2 terminology + §13 enforcement partition, v6.13.0 Three-tier default, v6.12.0 §13.3 + body-structure scope, v6.11.x compression series + earlier): see `~/.claude/CLAUDE-changelog.md`.

**Sizing** (v6.16.0, 2026-07-11, single post-edit `wc -c` per `feedback_spec_sizing_recursive_rewrite.md` option 1): core 24739 → 24739 bytes (Δ 0, header digits only); extended 46273 → 46440 bytes (Δ +167, §11-EXT bullet + entry swap); OPERATOR.md 7546 bytes (unchanged). Size budget: core 24739/25000 (**261 bytes headroom, 98.96%**); extended 46440/50000 (**3560 bytes headroom, 92.88%**). Drift envelope: ±20B accepted for this Sizing line's own corrective rewrite. Runtime L0/L1/L2 ≈ 6.1k tokens (core only).

**Operator carry-forward**: core untouched at 261B headroom (98.96%) — any next core addition MUST pair a net-delete that fully covers it. Remaining candidate pool (`tasks/core-net-delete-candidates-v6.14.md`): C2 (−250~300, audit gate unverified) + C4 (−230~280, medium risk); C1 consumed v6.15.0, C3 consumed v6.15.1. Net-zero / net-delete remains the permanent core posture (impact-audit #4 demote rejected as category error — do NOT re-attempt; see `project_impact_audit_followups_v0233.md`). Candidate compaction: §10-V extended block (~700B) once `reference_banned_vocab_examples.md` is confirmed canonical via /claudemd-rules hit data over ≥30d. Measurement track: first real sampling baseline landed 2026-07-10 (`tasks/sampling-audit-2026-07-10.md`); next per `docs/spec-optimization-plan-2026-07-10.md` A2–A5.
