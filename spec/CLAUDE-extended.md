# AI-CODING-SPEC v6.15.0 — Extended
...
**v6.15.0 (minor, 2026-07-10)** — §2.1 Model tiering (spawned-agent model selection) + Candidate-1 net-delete:

- `[add]` **§2.1 Model tiering** (core, after Tool escalation): spawned agents (Agent tool `model` param / Workflow `opts.model`+`opts.effort`) default to inheriting the session model — omit `model` when unsure. Whitelist downgrade: sonnet for mechanical fan-out (search / fetch / extract / classify / enumerate) + lint-or-test-gated bulk edits; opus for test-gated plan-step code. NEVER downgrade decision-shaped stages: orchestrate / synthesize / verify / judge / root-cause debug / L3 / §5-hard / §8 content. Invariants: verifier tier ≥ generator; anomalous downgraded output → one re-run at inherited tier; evidence bar is tier-independent (Iron Law #2). SHOULD-level routing guidance — NOT HARD; `spec/hard-rules.json` untouched, §13.2 budget unaffected. Design rationale: `tasks/specs/model-tiering.md` — tier by error shape, not task difficulty (recall-shaped / mechanically-verifiable errors → downgradable; decision-shaped errors propagate silently → never).
- `[net-delete]` **§7 metric-coupled row examples** (core): 6 project-specific examples removed per `tasks/core-net-delete-candidates-v6.14.md` Candidate 1 (−169B measured vs −280~340 estimated). Category names ("bench / oracle / compile-time budget") + the "Metric-coupling typical triggers" sentence remain the identification anchors.

### Why minor (not patch)

New rule added (backward-compatible guidance) per §13 META. Deliberately NOT HARD: proactive quality/cost guidance, not scar-driven — fails §13.2 promotion gates (0 repro incidents) by design, so it ships as SHOULD prose only.

**Older entries** (v6.14.2 trigger-list `e.g.` markers + context7 conditionalized, v6.14.1 §2.1 skill-MUST-invoke clarification, v6.14.0 §10 template relax + vocab trim, v6.13.2 terminology + §13 enforcement partition, v6.13.0 Three-tier default, v6.12.0 §13.3 + body-structure scope, v6.11.x compression series + earlier): see `~/.claude/CLAUDE-changelog.md`.

**Sizing** (v6.15.0, 2026-07-10, single post-edit `wc -c` per `feedback_spec_sizing_recursive_rewrite.md` option 1): core 24596 → 24978 bytes (Δ +382, Model tiering +552 − Candidate-1 examples −169 − 1B rounding); extended 46148 → 46653 bytes (Δ +505, v6.15.0 entry); OPERATOR.md 7018 bytes (unchanged). Size budget: core 24978/25000 (**22 bytes headroom, 99.91%**); extended 46653/50000 (**3347 bytes headroom, 93.31%**). Drift envelope: ±20B accepted for this Sizing line's own corrective rewrite. Runtime L0/L1/L2 ≈ 6.2k tokens (core only).

**Operator carry-forward**: v6.15.0 spent +382B of core headroom on Model tiering net of Candidate-1 (22B left, 99.91%). Core is now effectively AT the cap: any next core addition MUST pair a net-delete that fully covers it (Candidate 2 in `tasks/core-net-delete-candidates-v6.14.md` remains available, audit gate unverified; Candidate-1 actuals came in at half the estimate — re-measure before relying on remaining estimates). Net-zero / net-delete remains the permanent core posture (impact-audit #4 demote rejected as category error — do NOT re-attempt; see `project_impact_audit_followups_v0233.md`). Candidate compaction: §10-V extended block (~700B) once `reference_banned_vocab_examples.md` is confirmed canonical via /claudemd-rules hit data over ≥30d. Measurement track: first real sampling baseline landed 2026-07-10 (`tasks/sampling-audit-2026-07-10.md`); next per `docs/spec-optimization-plan-2026-07-10.md` A2–A5.
