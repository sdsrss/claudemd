...
**Sizing** (v6.29.0, 2026-09-07, single post-edit `wc -c`; ±20B self-rewrite envelope): core 24642 → 24906 bytes (Δ +264: four wording repairs, the subagent clauses in §0 and §5, and the L1-bugfix report threshold — the last a net-delete); extended 44482 → 45199 bytes (Δ +717: the subagent rule's detail, the L2 four-section correction, the plan-drift bullet losing its third signal token, the §13 partition counts, and a Recent-changes entry that had been two minors stale); OPERATOR.md 15996 → 16017 bytes (Δ +21: the demote-loop counts). Size budget: core 24906/25000 (**94 bytes headroom**); extended 45199/50000 (**4801 bytes headroom**). Drift envelope: ±20B for this line's own rewrite. Core is at 99.6% — per §0.1 the next version that adds to core must remove more than it adds, and at this headroom that is no longer deferrable.
