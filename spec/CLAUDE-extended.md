...
**v6.10.0 (minor, 2026-04-23)** — data-driven net contraction. Grounding: external audit vs 6-week history of `projects--mem` / `projects--code-graph-mcp` / `projects--claudemd` flagged core at 95% of §13.1 ceiling (24.9k/25k), evidence rule scattered across §0/§7/§10/§EXT §7-EXT/B.2, and dual routing tables (§2.2 + §EXT §4 FLOW) with tie-breaker. §13.2-compatible: 0 new HARD; rule consolidation + rationale externalization only — budget unchanged, 20-task counter reset.

- **§2.1 ROUTE unified** (core) — prior §2.1 skill soft-triggers + §2.2 ROUTE (L0–L2 subset) + §2.3 TOOLS (orchestration) merged into one §2.1 ROUTE table + escalation-principles block + soft-trigger clause. Dual-routing tie-breaker dropped. §EXT §4 FLOW still authoritative on L3/ship (unchanged). Runtime saving: ~1.4k chars.
- **§5 AUTH compaction** (core) — 14-row hard/soft column table → hard-default enum + soft list + none-case. 12 operations listed verbatim, no AUTH-level semantic change. ~−400 chars.
- **§7 evidence examples pruned** (core) — Iron Law #2 Good-example set 3 → 2 (L1 + L2), with 中文 user explainer consolidated to one trailing line. Bugfix anchor rule unchanged. ~−250 chars.
- **§8 sub-rule bodies tightened** (core) — 8.V1–V4 each compressed to 1–2 lines; historical incident detail (v0.8.3 leak count etc.) removed from core (already in CLAUDE-changelog v6.7.1 / v6.7.4 entries). ~−500 chars.
- **§10 honesty rules DRY** (core) — Specificity clause tightened, full banned-vocab list reference kept at §EXT §10-V. Scope qualifier shortened. ~−200 chars.
- **§11 session-exit HARD preserved** (core) — rule binding unchanged; v0.11.4 anecdote pruned from core (preserved in CLAUDE-changelog v6.7.4). ~−150 chars.
- **§0 / §EXT TOC / misc cross-ref sweep** — Fast-Path L0-only block tightened; depth-triggers explainer shortened; §EXT TOC updated; obsolete `§EXT §8-EXT` pointer dropped (section never existed — sub-rule detail is in changelog). ~−200 chars.
- **CLAUDE-changelog.md backfill** — v6.9.0 entry archived from extended §Recent changes to changelog. Chain was v6.9.3 → v6.9.2 → [gap] → v6.8.1; now v6.9.3 → v6.9.2 → v6.9.0 → v6.8.1.

HARD tally unchanged: §0.1 Core growth discipline / §0 Hard-AUTH override / Iron Law #2 / §7 Ship-baseline / §7 User-global-state audit / §8 Verify-before-claim (V1–V4) / §10 Four-section order / §10 Honesty rules / §10 Specificity / §11 MEMORY.md read-the-file / §11 Session-exit mid-SPINE. Zero added; zero modified in semantics; §13.2 budget cost = 0; counter reset.

**Sizing** (v6.10.0, 2026-04-23): core 19553 chars ≈ 4.9k tokens; extended 42601 chars ≈ 10.6k tokens; runtime when both loaded 62154 chars ≈ 15.5k tokens. On-disk incl. CLAUDE-changelog.md (23645 chars, loaded only on explicit Read): 85799 chars. Core Δ vs v6.9.3: 23823 → 19553 (−4270 chars, −17.9%). Extended Δ vs v6.9.3: 42427 → 42601 (+174 chars, +0.4% — net flat: v6.9.0 entry archived to changelog offset by v6.10.0 entry added here). Runtime L0/L1/L2 ≈ 4.9k tokens (core only; vs v6.9.3 ≈ 6.0k — net −1.1k tokens on every turn). L3/Override/ship ≈ 15.5k tokens (vs v6.9.3 ≈ 16.6k — net −1.1k tokens per L3 turn). Size budget (§13.1): core 19553/25000 (5447 chars headroom, up from 1177 in v6.9.3, i.e. headroom 4.6× expanded); extended 42601/50000 (7399 chars headroom). v6.10.0 is the second net contraction since v6.7.5 (after v6.9.0), explicitly §13.2-rewarded via 20-task counter reset.
