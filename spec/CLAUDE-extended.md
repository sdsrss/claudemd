# AI-CODING-SPEC v6.10.0 — Extended
...
**v6.11.0 (minor, 2026-04-24)** — data-driven ROI optimization across §1 / §2 / §5 / §5.1 / §7 / §9 / §10 / §11. Grounding: retrospective analysis of 5-day session history across `projects--mem` (v2.47.0 → v2.50.0) and `projects--code-graph-mcp` (v0.11.4 → v0.16.2) surfaced 4 silent-bug classes that Iron Law #2 + Ship-baseline + V1–V4 did not catch, plus two ergonomic frictions under `AUTONOMY_LEVEL=aggressive`. §13.2-compatible: 0 new HARD; 2 new SHOULD (§7 Metric-coupling, §9 Parallel-path completeness) + 1 classification-list expansion (§2 LLM-visible metadata → L3) + 3 clarifications of existing HARD (§0 Hard-AUTH via §5 obvious-follow-on, §1 Recommend-first single-option, §10 banned-vocab list excerpt) + 3 non-HARD rule demotions (§11 Re-Read / Correction / Context pressure → §11-EXT). Budget cost = 0; 20-task counter not reset (no HARD delta).

- **§9 Parallel-path completeness** (core, SHOULD L2+) — new principle. Change touching a node with multiple parallel implementation paths (fallback / feature flag / match default arm / SQL `ORDER BY` + `LIMIT` / early-return / FTS-vs-LIKE fallback / multi-language dispatch) MUST enumerate and verify each path. Grounding: 4 repros in 5 days — `ast_search` `ORDER BY f.path LIMIT` silent truncation (code-graph v0.14.x), TSX `config.name` default-arm silent-kill (code-graph v0.15.0→v0.15.1), `dead-code --json` early-return missed empty-envelope write (code-graph v0.16.x), CJK LIKE-fallback filter gap (mem v2.49.0→v2.49.1). Logged as HARD candidate in `tasks/rule-candidates-2026-04.md` with repro-count 4; promotion blocked by §13.2 20-task counter until repro pattern persists under the SHOULD rule's presence.
- **§2 L3 trigger — LLM-visible metadata** (core, classification-list) — added: MCP tool descriptions / MCP `instructions` field / adoption-memory files / shipped prompt templates / plugin skill descriptions → L3 regardless of LOC. Grounding: `feedback_routing_bench.md` — these fields steer Claude Code routing at runtime; `feedback_mcp_instructions_budget.md` — ~2KB harness cutoff makes single-byte edits load-bearing. Prior language (`released-artifact user-visible default behavior change`) was underpowered because LLM-visible fields don't look user-visible.
- **§7 Metric-coupling check** (core, SHOULD L2+) — new sub-rule. Changes to code coupled to a metric / bench / oracle / compile-time budget must cite before-and-after numbers. Triggers listed. Grounding: `feedback_routing_bench.md` ($0.10 P@1 bench before/after tool-description edits), `feedback_semantic_compression_estimator.md` (estimator field-alignment against `MAX_SEARCH_CODE_LEN`), `feedback_mcp_instructions_budget.md` (`const _: () = assert!(len <= 1500)` compile-time guard).
- **§5 Obvious-follow-on not exempt** (core, clarifies §0 Hard-AUTH override — no new HARD) — mid-bundle e2e discovery of an adjacent bug triggers individual re-AUTH even when "obvious". Exception: authorized fix blocked without it → proceed but surface as mid-bundle extension in REPORT. Grounding: `feedback_mid_bundle_scope_auth.md` (mem CJK bundle 2026-04-23 silent LIKE-fallback patch).
- **§1 Recommend-first single-option** (core, clarifies existing principle) — clear-scope single-obvious-option execute directly without "shall I proceed" preamble. Grounding: `feedback_autonomous_fixes.md` (code-graph solo-dev + bypassPermissions profile wants fix-test-iterate without approval).
- **§5.1 aggressive skip-list** (core) — explicit list of ceremony items `aggressive` users skip: skill soft-trigger announcement optional; §1 Recommend-first single-option preamble-free; clear-scope bugfix goes fix → test → iterate. Never-downgrade set unchanged (§8 / Iron Law #2 / §5 Hard-AUTH still bind).
- **§10 Banned-vocab quick-list** (core) — the 10 most-frequent EN + 中文 offenders inlined in core. Grounding: §EXT §10-V reference is lost after compaction; core pointer alone doesn't prevent reuse. Full EN+中文 enumeration still lives at §EXT §10-V.
- **§11 Re-Read / Correction / Context pressure** (core → §11-EXT demotion) — three maintenance heuristics collapsed to one pointer line in core; full text moved to §11-EXT. Grounding: 0 grounded-incident hits in 30-day session history — SHOULD-level guardrails, not rules that need to bind every turn. Core byte-cost saved to fund new §7 / §9 rules.
...
**Sizing** (v6.11.0, 2026-04-24): core 20299 → 23212 chars (+2913, +14.3%); extended 42604 → 45428 chars (+2824, +6.6%). Core under §13.1 25k-char ceiling (1788 chars headroom, 92.8% utilized — tight, next minor MUST net-delete). Extended under 50k (4572 chars headroom, 90.9% utilized). Runtime L0/L1/L2 ≈ 5.8k tokens (core only; +~0.7k vs v6.10.2 4.9k + v6.10.1/v6.10.2 adds ~0.2k). L3/Override/ship ≈ 17.2k tokens (+~0.7k vs v6.10.2). §13.2 budget cost = 0; 20-task counter preserved from v6.10.2.
