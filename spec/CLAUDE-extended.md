# AI-CODING-SPEC v6.17.0 — Extended
...
**v6.17.0 (minor, 2026-07-11)** — four-method spec-audit letter-fix batch (verified red-team findings + C5/C6 net-delete; audit dossier: `tasks/spec-audit-2026-07-11.md`):

- `[change]` **§3 stricter-reading scoped** (audit #3): applies to safety/AUTH-relevant ambiguity; explicit whitelists/skip-lists stay effective. Root cause of the S1 probe fork (whitelisted log-string upgraded L0→L1 via unscoped stricter-reading) and of the whole "letter-stricter voids relaxations" leak class — the only deviation direction the 12-probe suite found.
- `[change]` **§2.2 targeted-Read exception** (audit #6): targeted Read of a core-referenced §EXT section OK at any level — un-strands L1/L2-governing relaxations that live only in extended (§5.1-EXT effects table, §7-EXT additive exception).
- `[add]` **8.V1 + test-runner pass-fail count** (audit #1): anti-hallucination list now covers Iron Law #2's most typical confabulation shape; verification channel widened to Read/Grep/tool output.
- `[add]` **§2 self-reference resolution** (audit #5): LLM-visible-metadata L3 clause points spec self-edits at §EXT §13 META, resolving the §2↔§13 letter conflict.
- `[change]` **§7 residue-check example §8-conformant** (audit #2): `find <explicit-path> -maxdepth 2 -newer <baseline>` / `du -sh <explicit-path>` — removes the literal collision between two HARDs on `~/.claude/`.
- `[delete]` **Fast-Path "pre-classified follow-up"** (audit #10): self-referential unbounded L0 authorization removed from the whitelist.
- `[delete]` **§1.5 LOC exclusion clause** (audit #8): `git diff --stat` cannot compute "excl. blank/comment-only"; raw counts are the operating definition.
- `[move]` **C5 — §0.1 tier definitions → `OPERATOR.md §13.1`**: core keeps Tier-2 default landing + hard cap + net-delete + pointer.
- `[delete]` **C6 — §9 Parallel-first compressed** (−116B measured): harness parallelizes independent tool calls natively; the one-line rule stays.
- Deferred: audit #4 (§0↔§12 ship parenthetical) — status quo accepted: ship always loads extended (§2.2 hardening) and §12 manual-ship-atomicity + per-project runbook memory already cover the historical over-re-ASK incident. Audit #7 (§1.5 Module fallback) — external-adoption-facing, deferred under internal freeze.

### Why minor (and why now)

Two rules relaxed (§3 scope, §2.2 exception) → minor per §13 META; the §13.2 HARD budget is untouched (no HARD added; letter fixes are patch-exempt). Ships same-day as v6.16.0 under the same operator-requested audit session; every change traces to a verified red-team finding or a probe-measured fork, which is the invocation-data bar the §13.1 minor-spacing rule exists to protect.
...
**Sizing** (v6.17.0, 2026-07-11, single post-edit `wc -c` per `feedback_spec_sizing_recursive_rewrite.md` option 1): core 24739 → 24648 bytes (Δ −91: paired adds ≈ +264, deletes #8+#10 −54, C5 ≈ −185 derived, C6 −116 measured); extended 46440 → 48488 bytes (Δ +2048, audit-batch entry swap); OPERATOR.md 8314 bytes (Δ +768: R2 Tier-2 index budget + C5 tier definitions). Size budget: core 24648/25000 (**352 bytes headroom, 98.59%**); extended 48488/50000 (**1512 bytes headroom, 96.98%**). Drift envelope: ±20B accepted for this Sizing line's own corrective rewrite. Runtime L0/L1/L2 ≈ 6.0k tokens (core only).

**Operator carry-forward**: batch is net-delete (core −91B) — the "next addition pairs a covering net-delete" posture held. Candidate pool (`tasks/core-net-delete-candidates-v6.14.md`): **C2 CLOSED** by audit data (ship-baseline fired on 15 distinct days / 254 events vs 20 releases in 30d — ad-hoc pushes DO rely on the core §7 CI row; do not move); C5 + C6 consumed this version; **C4 is the only remaining candidate** (−230~280, medium risk) — next core addition needs C4 or fresh staging. Net-zero / net-delete remains the permanent posture (impact-audit #4 demote rejected as category error — do NOT re-attempt; see `project_impact_audit_followups_v0233.md`). Candidate compaction: §10-V extended block (~700B) — `reference_banned_vocab_examples.md` shipped 48d ago, gate now evaluable via /claudemd-rules hit data. Measurement track: sampling baseline 2026-07-10 (`tasks/sampling-audit-2026-07-10.md`); A4 hand-labeling decision point 2026-08-09 (`tasks/spec-audit-2026-07-11.md` calendar).
