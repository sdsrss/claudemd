...
**Sizing** (v6.29.0, 2026-09-07, single post-edit `wc -c`; ±20B self-rewrite envelope): core 24642 → 24666 bytes (Δ +24: four wording repairs — the L1/L2 file boundary, `internal` on the log-string whitelist, `Not done` added to the L1 short-report condition, `tc` spelled out); extended 44482 → 45584 bytes (Δ +1102: the subagent-has-nobody-to-ASK rule, the L2 four-section correction, the plan-drift bullet losing its third signal token, the §13 partition counts and this entry); OPERATOR.md 15996 → 16017 bytes (Δ +21: the demote-loop counts). Size budget: core 24666/25000 (**334 bytes headroom**); extended 45584/50000 (**4416 bytes headroom**). Drift envelope: ±20B for this line's own rewrite. Core is at 98.7% — per §0.1 the next version that adds to core must remove more than it adds.
