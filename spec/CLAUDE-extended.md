# AI-CODING-SPEC v6.25.4 — Extended
...
**v6.25.4 (patch, 2026-09-01)** — one operator-facing adjudication written down. No agent-facing rule text changed: core and extended are byte-identical apart from their title lines and, here, this entry and the Sizing line.

- OPERATOR §13.2's carry-forward list settles the §4-Routing-vs-`skillOverrides` item that v6.25.3 re-queued. **§4 and §12 are unchanged, deliberately**: all eight disabled primaries already own §12 Fallback rows, §12's Detection paragraph names `skillOverrides`-off as the case it handles, and `spec/` ships to users who *do* have those skills — rewriting §4 to match one machine's local disables would degrade the routing table for everyone else. The measured count is 8, not the 6 the carry note recorded. What was missing was never spec text but a detector: the 2026-07-24 discovery was an accident during an unrelated `/doctor` run, and `doctor`'s new `routing:skills-enabled` check now reads the installed extended spec against `skillOverrides` on every run. No rule added, relaxed or removed; enforcement partition unchanged (6/16/2/1).

**Sizing** (v6.25.4, 2026-09-01, single post-edit `wc -c` per `feedback_spec_sizing_recursive_rewrite.md` option 1): core 23445 → 23445 bytes (Δ +0: title line only); extended 44405 → 43819 bytes (Δ −586: this entry is shorter than the v6.25.3 one it replaces); OPERATOR.md 14262 → 15514 bytes (Δ +1252: the §4-routing-vs-skillOverrides adjudication replacing the carried item — operator-facing, never Agent-loaded, no runtime cost). Size budget: core 23445/25000 (**1555 bytes headroom**); extended 43819/50000 (**6181 bytes headroom**). Drift envelope: ±20B for this line's own rewrite. Runtime L0/L1/L2 ≈ 5.5k tokens (core only).
