...
- Measured 2026-09-06 during the v0.77.0 ship. The turn that carried it ran **3h02m and 126 tool calls without yielding**, because §11 Mid-SPINE turn-yield permitted no stop before VALIDATE. Two review subagents finished inside that turn and **zero** of their completion notifications entered context; the queue flushed **230 ms** after the turn closed. Sixteen `sleep` calls totalling 6870 s — 64% of the window — returned nothing, and six messages to already-idle teammates produced six further empty completions. §12's step-7 pre-tag review is HARD and sits inside §12's atomic ship window, so the two rules deadlocked by construction: the file side-channel that actually shipped v0.77.0 was invented at runtime, not specified.
- **§11 Mid-SPINE turn-yield** takes a fourth yield trigger — awaiting a spawned subagent. Yielding IS the delivery mechanism and completion re-invokes the agent with no user input, so the trigger costs no wall-clock and no user attention. Sleep-polling and re-messaging an idle teammate are named banned in the same breath, because both were tried and neither delivered.
...
**Sizing** (v6.26.0, 2026-09-06, single post-edit `wc -c` per `feedback_spec_sizing_recursive_rewrite.md` option 1): core 23445 → 23755 bytes (Δ +310: §11's fourth yield trigger, the only core edit); extended 43819 → 45567 bytes (Δ +1748: the §11-O delivery bullet, §12's second atomicity exception, and this version's Recent-changes entry replacing a shorter one); OPERATOR.md 15514 → 15514 bytes (Δ +0: untouched this release). Size budget: core 23755/25000 (**1245 bytes headroom**); extended 45567/50000 (**4433 bytes headroom**). Drift envelope: ±20B for this line's own rewrite. Runtime L0/L1/L2 ≈ 5.5k tokens (core only).
