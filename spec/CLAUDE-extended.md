# AI-CODING-SPEC v6.15.1 — Extended
...
**v6.15.1 (patch, 2026-07-10)** — §0.1 operator-threshold relocation (Candidate-3 move, no rule change):

- `[move]` **§0.1 tier promotion/demotion thresholds** (core → `OPERATOR.md §13.1`): the "≥3 sessions in 30d" / "≥5 sessions + elaboration wasn't consulted" promotion gates and the `/claudemd-rules` demotion recommendation moved out of Agent-loaded core per `tasks/core-net-delete-candidates-v6.14.md` Candidate 3 — their enforcement is `external` (operator-executed via audit data), so core carried thresholds the Agent cannot act on. Core keeps the Agent-consumed facts: Tier-2 default landing zone, tier structure, hard cap + net-delete clause, Sizing-line tracking. Same relocation family as the v6.13.0 §13.1 extraction. −239B measured (vs −350~400 estimated — third consecutive candidate landing under estimate; re-measure before relying on C2/C4 estimates).

### Why patch (not minor)

No rule added / removed / relaxed — the §0.1 HARD rule (cap + net-delete + Tier-2 default) is unchanged; only operator-facing elaboration relocated to its §13.1 home. Trigger: 2026-07-10 user-requested core-attention review; this ship executes its C3 recommendation as a user-authorized standalone compression (paired-with-addition default overridden by explicit instruction).

**Older entries** (v6.15.0 §2.1 Model tiering + Candidate-1 net-delete, v6.14.2 trigger-list `e.g.` markers + context7 conditionalized, v6.14.1 §2.1 skill-MUST-invoke clarification, v6.14.0 §10 template relax + vocab trim, v6.13.2 terminology + §13 enforcement partition, v6.13.0 Three-tier default, v6.12.0 §13.3 + body-structure scope, v6.11.x compression series + earlier): see `~/.claude/CLAUDE-changelog.md`.

**Sizing** (v6.15.1, 2026-07-10, single post-edit `wc -c` per `feedback_spec_sizing_recursive_rewrite.md` option 1): core 24978 → 24739 bytes (Δ −239, Candidate-3 move); extended 46653 → 46273 bytes (Δ −380, v6.15.1 entry replaces the longer v6.15.0 entry); OPERATOR.md 7018 → 7546 bytes (Δ +528, threshold bullet — human-only, unbudgeted). Size budget: core 24739/25000 (**261 bytes headroom, 98.96%**); extended 46273/50000 (**3727 bytes headroom, 92.55%**). Drift envelope: ±20B accepted for this Sizing line's own corrective rewrite. Runtime L0/L1/L2 ≈ 6.1k tokens (core only).

**Operator carry-forward**: v6.15.1 bought back 239B of core headroom (261B, 98.96%) — still effectively AT the cap: any next core addition MUST pair a net-delete that fully covers it. Remaining candidate pool (`tasks/core-net-delete-candidates-v6.14.md`): C2 (−250~300, audit gate unverified) + C4 (−230~280, medium risk); C1 consumed v6.15.0, C3 consumed v6.15.1. Net-zero / net-delete remains the permanent core posture (impact-audit #4 demote rejected as category error — do NOT re-attempt; see `project_impact_audit_followups_v0233.md`). Candidate compaction: §10-V extended block (~700B) once `reference_banned_vocab_examples.md` is confirmed canonical via /claudemd-rules hit data over ≥30d. Measurement track: first real sampling baseline landed 2026-07-10 (`tasks/sampling-audit-2026-07-10.md`); next per `docs/spec-optimization-plan-2026-07-10.md` A2–A5.
