...
- **Mid-SPINE turn-yield** (HARD, all levels): once a turn has executed ≥1 tool call inside an active SPINE cycle, continue planned steps through VALIDATE. `<system-reminder>` blocks (hook output, mid-turn recall, PostToolUse flushes) are NOT turn boundaries. **Yield only on**: `[AUTH REQUIRED]`, direction actually ambiguous, context pressure (§11 Context pressure → `tasks/<slug>-paused.md`), or **awaiting a spawned subagent** — its report enters context ONLY at turn end, so yield naming what is awaited; completion re-invokes you, no user input needed. Sleep-polling and re-messaging an idle teammate are banned there: the first cannot deliver, the second only re-invokes it into an empty completion. "Natural-feeling" stop points and single-Edit completion are not yields. Silent mid-cycle yield followed by next-turn "done" claim = Iron Law #2 violation. **Tell**: `继续 / next / 怎么停了 / why did you stop` after a turn that neither asked nor closed (§10 format) = confirmed prior yield; if the prior turn asked, `继续` answers it — NOT a tell.
