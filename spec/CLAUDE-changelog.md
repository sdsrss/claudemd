...
The 2026-09-06 spec audit (`docs/audit/20260906-231957.md`, local) compared core line by line with the current Claude Code system prompt. Turn continuation, subagent delivery at turn end, memory-file hygiene and honest reporting are now stated natively by the harness; the spec carried each of them a second time, in two cases in a form that contradicts the harness's own style rules. Net-delete release: core 23944 → 23405 bytes, extended 45899 → 44364 bytes. No new HARD rule; enforcement partition unchanged (6/16/2/1); every `hard-rules.json` `section_anchor` resolves unchanged.

- `[remove]` **§11 Mid-SPINE turn-yield** keeps its four triggers, the anti-silence clause and the Tell, and drops the 2026-09-06 measurement plus the two-substitutes rationale — both live in the v6.26.0 entry below and in the harness's own description of background subagents. 1261 → 921 bytes.
- `[remove]` **§EXT §11-O** and **§EXT §12 second exception** state the delivery fact in one sentence each; the `TaskOutput` deprecation note goes with it. §12's exception now covers any subagent the ship owes, not only the pre-tag reviewer (audit SPEC-M8).
...
- `[add]` **§2.1** names the harness `Agent` tool (fork / general-purpose) as the parallel primitive; `sp:dispatching-parallel-agents` is the optional wrapper.
