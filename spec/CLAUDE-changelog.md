...
- **Look it up, don't guess** (partial): §8.V1 bound citations and §2.1 sent no-code Q&A to docs-lookup; implementation-time facts had no rule. §2.1 Non-skill defaults: an unfamiliar or possibly stale fact → context7 / official docs / web search, cite the source, never guess. It replaces §2.1's Q&A docs-lookup clause; the "direct answer" half stays in §EXT §4. A first draft put it in §1 Principles, whose header reads "reference when ambiguous"; §2.1's defaults carry no such gate. The trigger is still the agent's own judgment of "unfamiliar", which no placement changes. Consequence: as a §2.1 default it yields to an explicit per-task user instruction (§3 User relaxation); whether a `feedback_*` / `user_*` memory outranks it is not settled by §3 Persisted memory, which names only §2.1's soft-trigger defaults. A preference-shaped default, so yielding to the user is intended.
...
**§1** now defines the language as the one the human types in, fixed for the session, and lists the messages that never switch it. It binds every message the user reads, wait/yield notes and relayed subagent findings named. `docs/` prose moves from English ("shipped reference+contract docs") to the user's language for new docs. An existing doc keeps its language, so an English doc gains English paragraphs and nothing is translated wholesale; identifiers and table keys a test parses stay English. README and subagent prompts are named in the English list.

Not changed: thinking language stays unspecified. In the Opus 5.5 / Fable turns where thinking is stored, English thinking was followed by English text 27 of 33 times inside human-中文 turns, 中文 thinking 4 of 52 — an association whose direction is not established, and the reason not to require English thinking here.
...
- `[fix]` **§2 LEVEL's file boundary lives in one place** (audit SPEC-M1). L1 said `files ≤2` while §1.5 counts a source file and its co-located test as ONE, so "≤2" meant up to four physical files to a reader who applied the pairing rule and two to a reader who did not; and L2's `multi-file` overlapped a two-file L1 outright. A first draft qualified L1 and left L2 bare, which the pre-tag review showed only relocated the overlap — the pairing clause lives inside the **Local-Δ** definition, not ambient. L1's own file count is gone (`Local-Δ only` already bounds it) and the qualifier sits on L2, where a plain reading produced the collision.
...
- `[fix]` **§10's L1 AND L1-bugfix short reports require `Not done` empty too** (audit SPEC-M4). The condition read `Failed+Uncertain empty → Done: <what>.`, so a task with an empty Failed and Uncertain but a real `Not done` was told to report one line — dropping the section §10's own four-section order exists to protect. The audit cited `core:212-214`; a first pass repaired 212 and left 213, the L1-bugfix rule, carrying the identical hole three lines down in the same list — and an L1 bugfix is the most common L1 shape. Both lines now name `Not done`, and both are pinned.
...
- `[add]` **A subagent has nobody to ASK** (audit SPEC-M14). §0's ambiguity ASK and §5's `[AUTH REQUIRED]` both block on a user; a spawned agent has none, which this harness states in its own system text and the spec never did. A subagent takes §0's option (b), states the reading in its report, and stops at a §5 hard-AUTH boundary with `[PARTIAL: <op> needs AUTH]` rather than self-authorizing or waiting. **In core §0 and §5, with the detail in §EXT §11-O** — the first draft put the whole rule in extended only, and the pre-tag review convicted it with this release's own other half: extended is not loaded at L0–L2, and a subagent reading core alone would emit `[AUTH REQUIRED]` and block on a confirmation that cannot arrive. Not tagged HARD, but it is not obligation-free either, which an earlier draft of this entry claimed: it flips §0's default to (b) and substitutes §5's signal, for subagents only.
...
- `[fix]` **§5 Hard** is tagged `(HARD, self-enforced)` and enters `hard-rules.json` as `§5-hard-auth`. Eleven hard-AUTH categories had no manifest row, so §13.1's demote review and `/claudemd-rules` could not see the layer at all, while `sampling-audit.js` had been emitting a `§5-hard-auth` detector against it since v0.28.0. Verified 2026-09-07: a literal `rm -rf src/` returns exit 0 from `pre-bash-safety-check.sh`, and the pre-tag review widened that — five of the eleven categories (delete file/dir, prod deps, `.env` write, migration/DB schema, CI/deploy config) pass with no hook arm at all (audit SPEC-H4). Partition 6/16/2/1 → 6/17/2/1; 25 → 26 rules.

- `[add]` **§8 Escape tokens** declares the bypasses AUTH artifacts — the `[allow-…]` / `[skip-…]` literals a deny advertises, **and every `DISABLE_*` kill switch and hook feature flag**, which is the wider route and the one that records nothing. The hooks advertised the exit to whoever read the deny, which under `bypassPermissions` is the Agent (audit SPEC-H3). Deny text in `pre-bash-safety-check.sh`, `banned-vocab-check.sh` (×2), `memory-read-check.sh` and `ship-baseline-check.sh` (×2) now says the same. The clause names the tokens by *shape*, not by literal: spelling them out puts the vocab-hook's own bypass literal into a file the Agent re-reads every turn, and that hook treats its literal anywhere in a command as authorization to skip — so documenting the tokens would have written a self-exempting commit message by default.

- `[add]` **§3 User relaxation** settles what §5.1, §8.V3 and §13 each handled piecemeal: the Order line ranks *conflicts*, not permissions, so spec **defaults** — §1 language contract, §2.1 routing, ceremony in §5.1's skip-list sense — yield to an explicit user instruction, while **HARD rules and §5 AUTH gates do not**; those move only through their own named channels (§5.1 `AUTONOMY_LEVEL`, `SAFE_DELETE_PATHS:`, §8.V3's on-real-repo exception), and §8 never. Clauses in neither set: recommend and proceed, reading stated in one line — a draft of this repair routed them to a blocking ASK instead, which would have contradicted §1's single-obvious-option rule, §0 Fast-Path and §5.1's `aggressive` skip-list all at once. Round 14's complaint was a *default* losing to a user who had asked for English (audit SPEC-H1), and the clause is scoped to that. A broader first draft — relax anything outside the §5.1 Never-downgrade set — was withdrawn by the pre-tag review: it contradicted §EXT §13's `§8/HARD never yield` across a boundary L0–L2 can never see, since core §2.2 forbids loading extended there, and its floor did not contain itself, so one instruction could have removed the floor. A `spec-structure.test.js` join now fails if either half of that agreement is edited away.

**Budget**: core 23478 → 24642 bytes, 98.6% of the 25000 cap with 358 bytes left; `spec-coherence-audit` reports it as a LOW. v6.27.0's net-delete is what paid for these clauses and the room is now spent — per §0.1 the next version adding to core must remove more than it adds. Extended 44596 → 44482.
...
- `[remove]` **§11 Mid-SPINE turn-yield** keeps its four triggers, the anti-silence clause and the Tell, and drops the 2026-09-06 measurement — it lives in the v6.26.0 entry below — while the two-substitutes fact stays as a parenthesis, because the harness prompt states delivery-at-completion but not that sleeping or pinging an idle agent fails to deliver (pre-tag review M2). 1261 → 977 bytes.
- `[remove]` **§EXT §11-O** and **§EXT §12 second exception** state the delivery fact in one sentence each; the `TaskOutput` deprecation note goes with it. §12's exception now covers any subagent whose report the ship needs, not only the pre-tag reviewer (audit SPEC-M8; the pre-tag review's M3 closed the gap that "a subagent the ship owes" left between §11's trigger and §12's exception).
- `[move]` **§5 Obvious-follow-on** folds into §0 Hard-AUTH override, the rule it clarified from two sections away. **§0.1** keeps the byte cap and the net-delete obligation; tier landing moves to `OPERATOR.md §13.1`, together with §1.5's "citation without a §EXT pointer is a defect" maintainer note.
- `[move]` **§5.1** inlines the `aggressive` skip-list. L0–L2 never load extended, so a skip-list that lived only there bound nobody at the levels it was written for (audit SPEC-M7).
- `[fix]` **§11 Post-compaction** re-Reads the plan and, when the task had loaded it, extended — not core, which the harness injects every turn (audit SPEC-M5).
- `[fix]` **§7 `Done:` example** and the Appendix B.2 examples are sentence-form. The harness bans parentheticals and em-dashes in user-facing text; the canonical examples were teaching both.
- `[remove]` Harness duplicates: §11's "Always skip" list and §11-EXT-MEM's index-refresh line. §2.1's "Skill MUST wording" paragraph shrinks to one clause — superpowers 6.3.0 itself states that CLAUDE.md takes precedence over skills, and the C1 over-ceremony measure is 0/4 on this repo.
- `[remove]` Five references to memory files that do not ship with the spec (`reference_banned_vocab_examples`, `feedback_memory_layer_routing`, `feedback_spec_sizing_recursive_rewrite`, `feedback_macos_shell_portability`, `feedback_hook_platform_lib_source`) are dropped or re-pointed at the in-repo artifact (audit SPEC-M12). §1.5-EXT's Local-Δ note duplicated core §1.5 verbatim.
- `[relax]` **§EXT §10-R Lessons file** is SHOULD and loses its session-start read: `tasks/lessons.md` last gained an entry on 2026-05-09; MEMORY.md and the recall plugin are the session-start layer.
- `[add]` **§2.1** names the harness `Agent` tool (fork / general-purpose) as the parallel primitive; `sp:dispatching-parallel-agents` is the optional wrapper; §4 Routing and §11-O say the same (pre-tag review M1).

Deferred with the data they need (audit §7.2): the §0 default-ASK relaxation, full deletion of the §2.1 skill-wording clause, and a tokenizer-measured Sizing figure. The Sizing line no longer carries a token estimate.
...
- `[relax]` **§11 Mid-SPINE turn-yield** takes a fourth yield trigger — *awaiting a spawned subagent* — alongside `[AUTH REQUIRED]`, genuine ambiguity and context pressure. The yield must name what is awaited, so the anti-silence clause that motivates the rule is untouched, and completion re-invokes the agent without user input, so the trigger costs neither wall-clock nor user attention. An unresumed yield owes `tasks/<slug>-paused.md`, as a context-pressure yield does, so the new turn-end shape is not the one shape that leaves no artifact. The two substitutes are described rather than forbidden — §13.2's shortcut licenses fixing a rule, not adding bans to it — and the description is that sleeping postpones the turn end that IS the delivery, while a message to an idle teammate only re-invokes one that has nothing left to add — six such pings produced six of the eight `finished` events the user saw arrive at once, four of them carrying only `DONE`.
- `[relax]` **§EXT §12 Manual-ship atomicity** takes a second exception for the pre-tag review wait. Author ≠ reviewer is HARD; §12 itself has no numbered steps, but a project whose runbook orders that review between the push and the tag puts it inside the atomic window that forbade turn-ending, and there the two HARDs deadlock. Only one of them can hold.
...
- `[fix]` `hard-rules.json` `_doc` said the manifest drives a "quarterly demote (rules with 0 hits in 90d)". The window is **30d** (OPERATOR.md §13.1, and `DEFAULT_WINDOW_DAYS` in `scripts/hard-rules-audit.js`), the review cadence is every 20 L2+ tasks or 30 days (§13.2), and the two are different concerns the sentence had merged. Three further copies of "quarterly" had spread to that script's USAGE, `commands/claudemd-sparkline.md` and `rule-hits-parse.js`; `doctor.js` still described the demote signal as "0 hits in 90d". All corrected, and a new gate in `tests/scripts/spec-structure.test.js` fails the word in live spec/tooling text and pins the 30d window in both the manifest and the script.
- `[fix]` `hard-rules.json` §10-specificity `note` described the hook as enforcing on `git commit` only, with everything else self-enforced. Path 2 (`BANNED_VOCAB_PROSE_SCAN`, default ON since v0.21.0) also scans the prior assistant turn's prose on ship verbs and denies on it. The `note` is what §13 META tells the agent to calibrate expectation from, so under-reporting mechanical coverage there is the same defect class as the `§11-memory-read` over-report corrected in v6.24.
- `[fix]` Extended §12 Fallback table carried a `gs:/canary` row for a skill no §4 Routing row selects and no other passage mentions — a documented degradation path for a decision the agent never makes. Removed. The existing §4→§12 join only ran forward; the reverse join now exists, with a control proving it reddens on the removed row.
- `[fix]` Extended §4 Routing `low-freq utilities` listed `gs:/freeze, /guard, /retro` while the §12 division-of-labour table and the Fallback table both list four with `/careful`. Three tables, one set, one short.
- `[fix]` Extended §5.1-EXT `aggressive` row downgraded `Δ-contract public API` to soft for internal-only consumers, while the skip-list two paragraphs below states §5 Hard-AUTH still binds — dead text by §3 stricter-reading, in the one profile this repo actually runs. Removed; the `Published client` definition it referenced is re-pointed at the §5 Hard row it actually serves, where it binds at every autonomy level.
- `[fix]` Core §11 preamble read "SHOULD L0/L1; MUST L2+ where marked" while two bullets in the section are marked `(HARD, all levels)` — reconciled only by the reader noticing "where marked". The preamble now states the default and the override.
- `[fix]` OPERATOR.md §13.4 said `tasks/` files are referenced "across 7 different sections" and listed seven that omitted core §11, §6 and §7-EXT — core §11 being the one that binds without loading extended. Same paragraph claimed "None are auto-created by the plugin" three lines above a table row naming `scripts/sampling-audit.js` as the writer. Both corrected.
- `[fix]` OPERATOR.md carry-forward list still carried the banned-vocab deny-gate bypass item after it was adjudicated on 2026-08-24 (no demote), while the `skillOverrides` item from the same list appeared in no subsequent batch-review record — neither decided nor re-carried. One entry now records the decision, the other is explicitly re-queued.
- `[add]` OPERATOR.md §13.1 gains two written-down structural facts the audit kept re-deriving: **demote-loop coverage** (12 of 25 HARD rules have no hit channel at all — 11 `self` + 1 `external` — so `demoteCandidates: []` means "no instrumented rule went silent", never "every rule earns its place") and a **§8 Never-list inclusion criterion** (a bullet earns a manifest row when a hook arm can decide it from the command string alone; four bullets are self-enforced by construction, and the `~/.claude/` traversal bullet is decidable-but-unimplemented, i.e. a detector candidate rather than an exclusion).

Operator-facing text only for the last three; agent runtime loads none of OPERATOR.md.
...
- **`[fix]` §10 four-section order no longer claims "Stop hook enforces".** `transcript-structure-scan` self-describes as advisory-only and opt-in, exits at a default-OFF guard, and runs on a Stop event that cannot block; `hard-rules.json` classifies the rule `enforcement: "self"`. Three artifacts disagreed three ways, and §13 META instructs the agent to calibrate from the manifest — so the core parenthetical was actively misleading about whether anything would catch a violation.
- **`[fix]` §5.1 `aggressive` contradiction.** Core's skip-list ended "§8 SAFETY + Iron Law #2 + §5 Hard-AUTH still bind" without qualification, while §5.1-EXT's table downgraded `cross-module refactor (≥3 Modules)` and internal-consumer Δ-contract to soft. Both could not be followed. §3 stricter-reading resolves to hard, so the table row now states that cross-module stays HARD and only the internal-only Δ-contract case relaxes.
- **`[fix]` §3 Order ranks harness / MCP / skill instructions.** §2.1 declares MCP per-tool instructions authoritative and rules on skill "MUST invoke" wording, but §3 — the section that owns precedence — never enumerated that layer, so conflicts involving it had no resolution rule. A live sample this session: a session-level conditional instruction was misread as a prohibition with nothing to rank it against.
- **`[fix]` Evidence ladder + cold-start tagged L2+.** Both bind at L2+ but core filed them under "L3 evidence rules", and §2.2 forbids L2 loading extended — so an L2 agent had no trigger they existed. Same shape as the v6.22.0 Iron Law #1 fix, one rule further on.
- **`[fix]` §0.1 cap stated in bytes** (every measurement of it always was — the stated "chars" understated real headroom by ~3.4×) and its headroom pointer now names `CLAUDE-extended.md`, where the live Sizing line is; the changelog it pointed at stops at v6.11.2 (2026-04-29), so an agent consulting §0.1 at the moment §0.1 matters read 93.6% instead of the real figure.
- **`[add]` §2 names the spec artifact** and its L2 proposal trigger. §2.S carries an L2 obligation and had zero core references, i.e. no way to reach it via the targeted-Read path §2.2 sanctions.
- **`hard-rules.json`**: `§11-post-compaction` added (HARD in core §11, live telemetry, no manifest row); `§iron-law-2`, `§10-four-section-order`, `§10-honesty`, `§11-session-exit` now declare the `rule_hits_section` their advisory hooks already emit under — previously null, which made §13.1 demote review compute 0 hits for them by construction. Partition: 7 hook / 15 self / 1 both / 1 external over 24 rules.

Paired net-delete per §0.1: C7 (§5.1 skip-list body → §5.1-EXT), C8 (§2 depth-trigger mnemonic), C12 (§2 override-mode trigger detail) — core 24714 → 24574B.

---

## v6.22.0 — 2026-07-25

Minor: **level/precedence seam closure** (2026-07-25 four-dimension audit: content consistency was test-gated, level/precedence semantics gated by nothing — 2 HIGH + 2 MEDIUM all on that seam). (1) **Iron Law #1 surfaces in core at its true level**: the rule binds L2+ (extended heading + §12 repair table both apply it at L2) but core's only pointer filed it under "L3 evidence rules" — and §2.2 forbids L2 from loading extended, so an L2 agent had no trigger telling it the rule existed. Core §7 now carries the one-line L2+ form; the extended intro stops calling it L3-only. (2) **§11 turn-yield Tell recognizes a compliant L1 close**: the v6.21.1 precondition's parenthetical defined "closed" as four-section only, while §10 mandates single-line `Done:` for clean L1 — a correct L1 close followed by `继续` self-diagnosed as a yield violation. "Closed" now = any §10-format close (rule relaxed → minor bump). (3) **§3 ranks project CLAUDE.md explicitly** (with current-turn user): §EXT §13's drift check asserted "project CLAUDE.md wins per §3 TRUST order" while §3's Order line never enumerated it — two rules, no tie-break; the §13 line now states the delegation boundary (§5.1-class settings win, §8/HARD never yield). (4) **§2 L0 row drops `config`**: it contradicted the §0 Fast-Path whitelist (no config) and §5 hard-AUTH config classes; stricter reading wins, trivial config edits route L1. Paired net-delete per §0.1: C9 (§9↔§1 verbatim double-write, −138B; `hooks/session-summary.sh` comment retargeted) + C11 (footer version-history pointer dup, −54B) + §1 `reasoning` dropped from the English-only list (visible thinking follows user language; written artifacts already covered) + §7-EXT-TMP pointer dedup. §EXT §12 fallback table gains `gs:/document-release` and `/careful` rows (M1). Two new structural tests pin the class: Iron Law #1 level-tag join (core mention ↔ extended heading) and §3-rank citation coverage (every entity extended cites as "per §3"-ranked must appear in §3's Order line). Core Δ −1B (24715 → 24714).

## v6.21.2 — 2026-07-25

Patch: **§EXT §10-V compressed to OK-shapes + pointers** — the five `**Banned …**` enumeration lines leave extended; the mechanical gate (`hooks/banned-vocab.patterns`, untouched) and `reference_banned_vocab_examples.md` carry the full list; core §10 quick-check unchanged, core pointer retargeted. Grounds (2026-07-25 audit of 30d hook telemetry): 15/15 denied matches were core-quick-check members (robust ×8, comprehensive ×6, significantly ×1) — the extended long tail earned zero incremental catches while costing ~0.8KB of every L3/ship load; global sampling puts the external §10-V violation rate at 44/13190 turns (0.33%). Demoting the deny gate itself was evaluated and REJECTED: `hard-rules-audit.js` lists no §10-V demote candidacy (31 hits in window ≠ zero-hit criterion, per `feedback_demote_needs_data_not_intuition`); the 16/31 (51.6%) bypass-escape-hatch rate across 8 projects is flagged to the next §13.2 batch review — demote-by-bypass-rate has no codified rule, so the decision belongs there, not in a patch. Records: `tasks/banned-vocab-demote-evaluation-2026-07-25.md`. Join fixture `tests/fixtures/banned-vocab-canonical.json` updated in step: pattern-bearing rows flipped to in_spec=false with exempt_reason, ten spec-only/pattern-null rows retired (would be drift-6 vestigial); enforcement behavior byte-identical. Extended Δ ≈ −0.8KB; core Δ ≈ +30B (pointer).

## v6.21.1 — 2026-07-24

Patch: **§11 turn-yield tell gains its missing precondition** — the rule asserted something the data contradicts 37/37. Old text treated the next user message being `继续 / next / 怎么停了 / why did you stop` as, by itself, confirmation that the prior turn yielded mid-SPINE. A hand-labeling pass over every flagged instance in all 151 available transcripts (`tasks/sampling-detector-labeling-2026-07-24.md`) found **zero** true positives among 37: 28 of the prior turns ended with an explicit question (`要继续吗?` / `还是先停在这里?`), the rest had closed with a four-section report, were waiting on an operator decision, or had been cut off by a spend limit — and in two the agent had literally written "说一声或 `继续`", soliciting the exact word the spec reads as an indictment. The tell now requires that the prior turn **neither asked nor closed**. This is not cosmetic: a rule that converts every legitimate ASK-then-answer exchange into a self-diagnosed §11 violation pressures the agent away from stopping to ask, working against §5 hard-AUTH blocking, §0 initial-prompt ambiguity, and §1 Recommend-first — all three of which depend on the agent being willing to yield. Detector side (`scripts/sampling-audit.js`) mirrors the precondition; six sibling detectors were closed rather than repaired (see plugin CHANGELOG 0.54.0). Core Δ ≈ +182B (within headroom, unpaired per §0.1).

## v6.21.0 — 2026-07-24

Minor: **§EXT §12 fallback completeness** — routing-vs-availability drift closed for disabled skills. (1) The Fallback table gains a `gs:/qa, /qa-only` row (`gs:/browse` pass over the changed user-facing surface → report per §7 L2 evidence → repair via §12 Review-finding repair). It was the only §4 Routing primary with no fallback row, and the §4 `ship L2` row ("/qa: skip unless user-facing") routes a user-facing ship straight at it. (2) The Detection sentence's premise is widened: it assumed a missing skill produces a failing call, but a skill switched off via `skillOverrides` — or never installed — is simply absent from the session's skill list, so no call fails and auto-degrade never fires. The routed skill must now be checked against the listing before invoking; unlisted → take its fallback row and name the substitution in one prose line; no row → REPORT under Uncertain, no improvised substitute. Trigger: a 2026-07-24 `/doctor` run found 6 §4 Routing primaries (`gs:/office-hours` `/freeze` `/benchmark` `/qa` `/canary` `/design-consultation`) set `"off"` in `~/.claude/settings.json#skillOverrides`; 5 already had fallback rows, `qa` did not, and Detection covered none of the six because `off` is silent rather than failing. SHOULD-level text only — `hard-rules.json` bumps `spec_version` and nothing else; §13.2 ratchet untouched; §0.1 headroom sufficient, so the addition ships unpaired. Extended Δ: Sizing line.

## v6.20.1 — 2026-07-24

Patch: production-readiness-audit letter fixes — wording / cross-ref only, identical behavior. (1) §13 META enforcement-partition counts corrected: "22 HARD rules … 6 hook" → "23 … 7 hook" (stale since the §8-curl-sh manifest entry, 2026-07-13); new drift test `hard-rules-9` pins the §13 prose counts to the reduce-computed `hard-rules.json` partition so a count can no longer drift unguarded. (2) Duplicate §EXT heading ids made unique — `§7-EXT TMP_RETENTION` → `§7-EXT-TMP`, `§11-EXT Memory operations` → `§11-EXT-MEM`, `§11-EXT macOS` → `§11-EXT-MAC` (`§7-EXT VALIDATE` / `§11-EXT Session heuristics` stay canonical); `### MEMORY.md tag syntax` relocated from under §0.2-EXT (structural orphan) into §11-EXT-MEM; core pointers updated; `spec-coherence-audit` CHECK 1 now flags duplicate `##+ §id` headings. (3) v6.20.0 Recent-changes entry externalized to this file (extended keeps only the current entry) — extended regained ~2.2KB headroom. Core/extended Δ: both Sizing-line targets.

## v6.20.0 — 2026-07-15

Minor: **§2.1 Model tiering removed** (core `§2.1` paragraph + `§EXT §2.1-EXT MODEL TIERING`). The whole spawned-agent tier-selection rule is deleted — downgrade-eligible category enumeration (Sonnet/Opus), the NEVER-downgrade list (orchestrate / synthesize / verify / judge / root-cause / L3 / §5-hard / §8), the verifier ≥ generator invariant, and the anomalous-output re-run clause. Operator decision: model self-allocates spawned-agent tiers; quality-first with zero spec constraint. Rationale — the `Agent` tool already defaults to inheriting the parent (session) model when `model` is omitted, so quality-first is the harness default without any spec text, and the model chooses per-spawn tiers on its own judgment. SHOULD-level rule, `hard-rules.json` untouched (no hook change). The §2.1 ROUTE `sp:dispatching-parallel-agents` row also lost its "note downgrade" annotation. Standalone net-delete (frees budget, §13.2 counter reset). Evidence that prompted the review (and the accepted trade-off): 2026-07-15 ubuntu-sec was the first real sample of the rule firing — the orchestrator self-allocated verify/review subagents to sonnet incl. same-tier self-review, the exact pattern the guardrail targeted; operator chose zero constraint over re-adding the guardrail (tracked in `feedback_tiering_verify_downgrade_gap.md`, reopenable if it recurs). Core/extended Δ: both Sizing lines.
