...
| `PostToolUse` after `git merge` / `pull` / `fetch` / `push` or `gh pr merge` | `branch-prune` (v0.93.0+) | Deletes local branches whose remote branch was deleted (`[gone]`) and whose tip is on the default branch, plus `worktree-agent-*` branches on it; never the default branch or a worktree checkout, never (`worktree-agent-*` aside) a branch whose upstream still exists or one with no upstream, and nothing during a rebase/bisect. Names each with its full sha. Squash-merged gone branches are listed by `node scripts/housekeeping.js branches`, never deleted. Local git only. **On by default.** |
