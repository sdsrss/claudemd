...
> A **personal AI-coding discipline harness**: one developer's opinionated **AI-CODING-SPEC v6.34**, encoded as Claude Code shell hooks and shipped with the plugin. Built and dogfooded on my own repos — fork and adapt, don't adopt wholesale.
...
| Spec v6.34 | `~/.claude/CLAUDE.md` · `CLAUDE-extended.md` · `CLAUDE-changelog.md` · `OPERATOR.md` (backup-before-overwrite) |
...
| `/claudemd-clean-residue [--apply]` | Dry-run-by-default cleanup across four scopes: `$TMPDIR` (`claudemd-sync-*` / `claudemd-memtags-hay-*` sentinels, historical `claudemd-(mockgh\|work).*` sandboxes), `~/.claude/tmp` tool-exhaust past the §EXT §7-EXT retention window, the seven ephemeral classes in `~/.claude/.claudemd-state`, and headless-probe dirs (`-tmp-*`, `-var-tmp`) in `~/.claude/projects` past the same window, never one holding a memory file. Exits 3 if `--apply` leaves targets behind. |
...
├── spec/                     # shipped v6.34 CLAUDE*.md trio + OPERATOR.md + hard-rules.json manifest
