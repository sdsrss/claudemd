...
| Session stop whose last reply is English while the human writes 中文 | `reply-language-check` (v0.96.0+) | Stop, the one claudemd hook that returns `{"decision":"block"}`: the turn continues for one more message, asking for the same content in 中文 (no new work, no tools); the English text already shown stays. The human's language comes from the human's own messages only — task notifications, teammate messages, meta rows and tool results never count; a slash command counts by its arguments — and a request for English lets every reply through. Headless `claude -p` runs are skipped. Asks at most once per turn (`stop_hook_active`). Opt-in: `REPLY_LANGUAGE_CHECK=1`, or `=log` to record without asking; default OFF. |
