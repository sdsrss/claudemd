...
| `tests/hooks/preToolUse-fastpath-order.test.sh` | the hook is `PreToolUse:Bash` and calls `hook_is_readonly_bash` | Only `.tool_name` and `.tool_input.command` may be extracted ABOVE the fast-path exit. Telemetry fields (`session_id`, `tool_use_id`, `cwd`) go below it — a read-only command exits there and never uses them, so every jq spawn in front of the exit is work the fast-path cannot skip. `memory-read-check.sh` is the reference shape. |
