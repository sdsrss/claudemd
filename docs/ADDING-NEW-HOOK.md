...
| `tests/hooks/hook-common.test.sh` | the hook resolves a path inside the per-project `memory/` dir | "Was this memory file read this session?" is `hook_memfile_was_read`, not a private `grep`. The bare-substring form matches the hint's own banner (it embeds the absolute path) and any assistant prose quoting it, so a private copy answers "read" for a file nobody opened — twice fixed, once per hook, before the predicate got one home. |
| `tests/scripts/subject-set-drift.test.js` | always | No hand-written list of hook names anywhere: derive from `HOOK_REGISTRY`. If the new hook is left OUT of `scripts/lib/doctor-hook-tests.js`'s liveness table, it must be named in `LIVENESS_SKIPPED` with a reason. (Both tables moved there from `scripts/doctor.js` in v0.76.3; this line still said `doctor.js` until 2026-09-14.) |
| `tests/scripts/doctor-hook-selftests.test.js` | always | The behavioural half of the row above. It calls `runHookSelfTests` as a value and requires every row it pushes to be green, so a hook in the liveness table must exit 0 on a synthetic event of its registered type with no shell-crash signature on stderr, and must guard on the `DISABLE_*` name `HOOK_REGISTRY` gives it — a mismatch there means doctor cleared a variable the hook does not read, so it reported green on a hook it never actually ran. |
