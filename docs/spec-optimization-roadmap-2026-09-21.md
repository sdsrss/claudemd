...
**口径更正(2026-09-21,G0 落地时发现)**:上面这组数出自 `testedit.mjs` 的**宽口径** matcher——目录命中
`tests?/` / `spec/` / `__tests__/` 或文件名命中 `.test.` / `.spec.` / `_test.`,**不要求代码扩展名**。本节原先把它
写成"代码扩展名 AND 测试目录/命名",那是附 A 为 G0 预登记的**严口径**(去掉 `spec/`,要求代码扩展名),两者不是
同一个 matcher。G0 实现的是严口径。两者在同一时刻(2026-09-21 22:0x,语料已增长到本节测量之后)的差为:
宽口径 56/1677 = 3.3%,严口径 52/1652 = 3.1%——**结论方向不变**(弱化不是主要失效模式),故本节的分类结论保留,
只更正它所声称的匹配口径。原始的 1,618 分母无法重算:它是 2026-09-21 20:0x 那一刻的语料快照,语料此后持续增长。
...
1. **3.2(c) 的匹配口径写错**:正文声称"代码扩展名 AND 测试目录/命名",而 1,618 这组数实际来自宽口径
   matcher(含 `spec/`、不要求代码扩展名)。G0 按附 A 预登记的严口径实现;两口径同刻对比 3.3% vs 3.1%,
   结论不变。更正写在 3.2(c) 正文内。
