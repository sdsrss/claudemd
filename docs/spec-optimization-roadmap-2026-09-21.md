...
**一处 ±3 残差,原因在本文档自己**:3.3 的 `28,099` 与 `5,055` 相加是 33,154,而 3.2 的 tool_use 是 33,140——
...
所以这两个数本来就不来自同一瞬间。**三个率各自的主分母(126 / 1,618 / 33,140)全部落在 ±1 内**;
落在 ±1 外的只有 3.3 这一对分层 tool_use,且其不可同刻复算是原始测量的性质,不是仪器的。
