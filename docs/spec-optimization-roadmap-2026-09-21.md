...
> 相加比 3.2 的 tool_use 多 14。按 3.2 的同一时刻复算是 external 28,092 与 self 5,048,相加与 3.2 逐位相等;
> 占比两层仍各为 0.2%,第一行与第三行逐项复现。推导命令与完整交代(含一条已撤回的 −1)见附 A。
...
| 判据分母 | 命令 | 得数 | 本文档 | Δ |
|---|---|---|---|---|
| tool_use(3.2a/f) | `--until=1790021570` | 33140 | 33,140 | 0 |
...
| external tool_use(3.3) | 同上 | 28092 | 28,099 | −7 |
...
(此刻 external=28,103)。`strat.mjs` 按项目目录顺序逐个读,两层之间隔着几十秒,期间本机有活跃会话在写 transcript,
所以这两个数本来就不来自同一瞬间。

补做的是把这条从「不可复算」降成「已定位」:在 3.2 的同一时刻读两层,得 external 28,092 与 self 5,048,
**相加 33,140,与 3.2 的 tool_use 逐位相等(Δ=0)**。也就是说分层口径本身是可同刻复算的、且与总量自洽;
上表那两行 −7 的 Δ 是文档里那两个数各自晚一分多钟读出来的结果,不是两套口径的分歧。
3.3 表里其余各行在同一时刻逐项复现:第一行的 65/109 与 9/17 两项 Δ=0;第三行的两个弱化占比同样不变,
其分层计数是 external 48/1480、self 3/139(同一条命令的 `byClass.*.behaviorMetrics`,弱化+删除比测试文件 Edit)。
原表数字**保留不改**,时刻差记在这里和 3.3 的脚注里——这是测量记录,不是可以就地修正的结论。

**撤回一条本会话自己写过的结论(2026-09-22,预发评审 H1/H2)**:上表初稿把 tool_use 写成 `33139(Δ −1)`、
把分层写成 `28,091 / 5,048`,并在 CHANGELOG 与 `8e3e243` 的提交信息里说「差 1」「落在 ±1 内」。
那个 −1 不是语料的性质,是 `--until` 自己的缺陷:epoch 秒被当成 `.000` 的排他上界,于是**那一秒之内的行被整段丢掉**。
两条已发布的复现命令因此都短了一秒的量,而且两次都短在对作者不利的方向——真实结果比声称的更好,所以下游没人会发现。
修法在工具而不在文案:秒级(不带小数)的上界现在覆盖整秒,`--until=1790021570` 与
`--until=2026-09-21T20:12:50.999Z` 是同一个界;要取边界就把小数写出来(`.000`)。
修完后 3.2(c) 那条 `--until=1790021507` 也按字面复现 1618 与 804/763/37/14——此前它实际返回 1617 与 762。
`8e3e243` 的提交信息已推送、不可改,这里是它的更正记录。

同一轮评审还把 `--until` 的另外四处收紧了(下界随上界重锚、越界瞬时拒收、ISO 必带时区、有界运行丢弃无时戳行)。
这四处**不改变本表任何一个数**:四条 epoch 命令在修改前后逐项相同,所以它们是稳健性修复而不是口径变更。
本表的数是在全部五处修完之后重跑的。
