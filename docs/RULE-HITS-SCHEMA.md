...
| `fail-open` | any hook calling `hook_record_failopen` — `banned-vocab`, `memory-read-check`, `pre-bash-safety`, `ship-baseline`, `session-end-check`, `hook-common` | hook silently skipped enforcement due to a missing prerequisite. `extra.reason` ∈ {`jq-missing`, `bad-event`, `patterns-missing`, `prereq-missing`, `heredoc-awk-empty`}. Rate-limited to 1 row per (hook, reason) per 60s via `~/.claude/.claudemd-state/failopen-*.ts`. Section: `§hooks-fail-open`. Round-6. |
