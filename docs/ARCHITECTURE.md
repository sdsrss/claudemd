...
| PostToolUse | `rework-breaker.sh` | G2: per-session per-file Edit/Write/Bash-edit tally (Bash via `tool_response.bashEditDiff`); injects one line at each multiple of the pre-registered threshold 8. Opt-in `REWORK_BREAKER=1` (§13.3 default-OFF) | `§1-root-cause` |
