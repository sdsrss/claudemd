...
| Stop | `sandbox-disposal-check.sh` | Test/probe residue that appeared or changed since the session's previous Stop (mtime, not ownership), depth 1: `claudemd-*` in `/tmp`; `tmp.*` / `claudemd-*` in `~/.claude/tmp` and `/var/tmp`; `~/.claude/projects/` dirs whose encoded cwd is a temp dir and that hold nothing older than that Stop, minus the session's own transcript dir. macOS `/tmp` (a symlink) is not descended. Advisory; opt-in `SANDBOX_DISPOSAL_BLOCK=1` returns `{"decision":"block"}` once per turn (`stop_hook_active`) | `§8.V4` |
