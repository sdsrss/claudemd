...
| Stop | `reply-language-check.sh` | The last reply has no CJK character and at least 10 words once code, URLs, paths and tags are stripped (an `API Error:` line or a conventional-commit first line is not judged), and the human is 中文: the newest reply-language directive in the human's newest 20 messages, else — unless the newest is a short request for an English artifact — their majority language. Human messages exclude tool results, meta, compact-summary, sidechain rows, rows whose `origin.kind` is not `human`, and machine-prefixed rows (a slash command counts by its arguments; `queued_command` attachments count). Interactive sessions only (entrypoint not `sdk-*`). Returns `{"decision":"block"}` asking for a 中文 restatement, once per turn (`stop_hook_active`). Opt-in `REPLY_LANGUAGE_CHECK=1` or `=log` (§13.3 default-OFF) | `§1-language` |
