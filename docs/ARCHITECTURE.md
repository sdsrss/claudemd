...
| `tests/` | 77 node suites, 28 hook suites, 4 integration suites, shared libs under `tests/lib/` | `npm test` (= `bash tests/run-all.sh`); `npm run test:scripts` / `test:hooks` / `test:coverage` |
...
- `~/.claude/.claudemd-state/hook-root.json` — the directory Claude Code actually loaded the plugin from, measured from inside a running hook rather than inferred from the filesystem: `{"root","ts","version","sid"}`, written by `hook_record_plugin_root` (`hooks/lib/hook-common.sh`) on behalf of `session-start-check.sh` and `session-end-check.sh`, so at most twice per session. Read by `runningPluginRoot()` (`scripts/lib/paths.js`), which is what doctor's `hook-drift` rows compare against instead of `activePluginRoot()`: the cache resolution can only ever answer "what is REGISTERED", so for a plugin that runs from where it sits (a plugin directory passed on the Claude Code command line, a skills-directory plugin, a synced tree) it hands back an unrelated leftover and every comparison built on it runs against a stranger. **Persistent, not per-session**: it is rewritten every session, so it never ages out and is deliberately NOT in `clean-residue.js`'s `STATE_EPHEMERAL` allowlist — `state-dir-orphans` does not count it and `/claudemd-clean-residue` does not delete it, while an uninstall with `CLAUDEMD_PURGE=1` does. Written atomically: `printf` into `hook-root.json.<pid>`, then `mv` onto this name. That temp is not a resting state — it exists only between the two — so it gets no bullet of its own and is a `STATE_IGNORE` entry in the drift gate; `CLAUDEMD_STATE_FILE_RE` still matches it, so a process killed in the window leaves nothing an explicit purge cannot reach.
