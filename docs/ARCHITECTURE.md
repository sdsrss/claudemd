...
| Stop | `evidence-gate.sh` | G1b: a completion claim in the last assistant message with no non-error Bash result after the last code edit (Edit/Write, or a code file in a Bash result's `bashEditDiff`) whose command names a runner or whose output carries a runner verdict. Opt-in `EVIDENCE_GATE=1` (§13.3 default-OFF) | `§iron-law-2` |
...
| Stop | `sandbox-disposal-check.sh` | Test/probe residue that appeared or changed since the session's previous Stop (mtime, not ownership), depth 1: `claudemd-*` in `/tmp`; `tmp.*` / `claudemd-*` in `~/.claude/tmp` and `/var/tmp`; `~/.claude/projects/` dirs whose encoded cwd is a temp dir and whose every direct entry is newer than that Stop, minus the session's own transcript dir. macOS `/tmp` (a symlink) is not descended. Advisory; opt-in `SANDBOX_DISPOSAL_BLOCK=1` returns `{"decision":"block"}` once per turn (`stop_hook_active`) | `§8.V4` |
