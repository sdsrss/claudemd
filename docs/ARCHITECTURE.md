...
| `argv.js` | Strict `--key=value` argv parser plus the CLI-entry contract every script shares: which module is the program, and what an argv-shape error costs | `parseStrict` `parseStrictOrExit` `invokedAsMain` `printHelpAndExit` `parsePositiveInt` `validateAndExpandFlags` `ArgvError` |
...
3. On a FRESH install (no manifest at either location) it runs `scripts/install.js` inline under a 4 s timeout — Claude Code assembles `~/.claude/CLAUDE.md` into context at session start, so a detached run cannot land the spec in time for the session that bootstrapped it. On a version MISMATCH (upgrade) it calls `hook_spawn_install`, which runs the same script detached under a 10 s timeout; the old spec is already on disk, so lateness costs only staleness. `CLAUDEMD_FORCE_ASYNC_BOOTSTRAP=1` forces the detached path on both.
4. `install.js` backs up the existing spec, copies the shipped files with a SHA-256 post-check, evicts legacy hook entries from `settings.json` (writing it only when eviction removed something), writes the manifest — last, and atomically, so a run killed at either ceiling leaves no manifest and the next SessionStart retries the same path.
...
- **`install()` holds a cross-process lock; settings.json writes inside it are idempotent, last-writer-wins**: read-modify-write with atomic tmp+rename (no torn file possible). The settings mutations themselves (legacy-hook eviction, statusline adopt) are idempotent and re-applied next session, and since v0.71.4 `install.js` writes settings.json **only when eviction removed something**, which in steady state is never. What this bullet used to say — that the race is acceptable *because every change is idempotent* — was false for the one change that is not: `createBackup` **renames** the user's own `~/.claude/CLAUDE.md` away, so of two concurrent fresh installs the loser found nothing to move and returned an EMPTY newest `backup-<stamp>-1`, which is the directory `CLAUDEMD_SPEC_ACTION=restore` reads (1 of 30 trials; 4 of 40 concurrent upgrades also tripped the spec-copy SHA check). Since the Round-14 audit (SCR-H2) `install()` takes `~/.claude/.claudemd-state/install.lock` with `open(O_CREAT|O_EXCL)` — not `mkdir`, which this repo measured admitting two holders under uutils coreutils — and a second install returns `spec: "skipped-locked"` without touching anything. A lock is treated as abandoned — and taken over — once it is older than 10 minutes **or** the pid it records is no longer running, because this runs on every SessionStart and must not be brickable by one killed process. The owner check is what makes the common case fast: `platform_timeout 4` SIGTERMs the synchronous bootstrap, SIGTERM runs no `finally`, and by age alone the lock it leaves reads as seconds-young for the next ten minutes (round-17 FLW-H1). The takeover itself `rename`s the lock it judged aside and then checks that the file it moved is the one it judged — by BYTES first, then mtime, then inode and device, because inode identity is not identity: comparing `ino`/`dev` alone was 57/57 green on tmpfs and red on ext4, which hands the inode freed by the winner's `rmSync` straight back to the winner's own `open(wx)` — `unlink` + `open(wx)` let a late taker delete the winner's fresh claim and claim on top of it, 1 of 40 aged-lock trials (round-17 SCR-H1).
...
- **The spec trio lands all-or-nothing, and each file swaps atomically**: `spec-hash.js#copySpecFiles` is the single path for writing `spec/*.md` into `~/.claude` (install and update both use it). Each file is written to a sibling temp path, sha256-verified there, and `rename`d onto the real destination, so a session reading `~/.claude/CLAUDE.md` sees the old bytes or the new ones and never a prefix — a bare `copyFileSync` is `open(O_TRUNC)` + write, and the file it truncates is the one every session reads at startup (Round-14 SCR-M1). The temp file goes beside the *resolved* destination so a symlinked spec stays a symlink; when the destination is a live link that cannot be resolved (an unreadable dotfiles dir), the copy falls back to writing THROUGH the link and failing loudly rather than replacing it. When the caller took a backup — `createBackup` *renames* the originals away — any failure restores every file from it before rethrowing. Added v0.71.4 (R11-09): update.js previously ran a bare `copyFileSync` loop, so a mid-loop ENOSPC left one new file and three missing, the partial state `update.js:37-39` forbids because §EXT cross-references dangle.
...
- **One main guard, one exit-2 contract**: a CLI's entry block is `if (invokedAsMain(import.meta.url))` and its argv errors go through `parseStrictOrExit` — both in `scripts/lib/argv.js`. The hand-written comparison this replaced (`import.meta.url` against a `file://` string built from `process.argv[1]`) is false under a symlinked directory or a space in the path, so the CLI exited 0 having done nothing; `install.js` reported success and wrote no manifest (2026-09-05 audit P0-1). `lint-argv` now rejects that shape outright, and its `findMainBlockGuard` recognises all three spellings — a guard the finder cannot see makes the file "not a CLI" and skips every check pointed at it.
...
- **`${CLAUDE_PLUGIN_ROOT}` wins when it is set; `__dirname` is the fallback**: `paths.js#resolvePluginRoot` returns the env var whenever it is non-empty and only then derives from `__dirname` / `${BASH_SOURCE[0]}`. This bullet used to call the variable "a hint", which is the opposite of what the resolver does and matters because pointing it at a stale versioned cache dir is how the never-downgrade guard in `install.js` gets exercised (Round-14 audit SCR-L3). `doctor.js`'s hook-drift check deliberately does NOT consult it — its whole subject is the difference between *this* tree and the marketplace install Claude Code actually fires, so reading the env var there would compare a thing to itself.
...
sections on `SessionStart` with `source` `compact` or `resume`, and
`ledger-staleness.sh` says so at `Stop` when a session edited code and wrote
nothing here (G7's third arm, v0.91.0, opt-in `LEDGER_STALENESS=1`).
...
- `~/.claude/.claudemd-state/hook-root.json` — `{"root","ts","version","sid"}` naming the directory Claude Code actually loaded the plugin from, measured from inside a running hook rather than inferred from the filesystem. Written by `hook_record_plugin_root` (`hooks/lib/hook-common.sh`) for `session-start-check.sh` and `session-end-check.sh`, both non-blocking. Read by `runningPluginRoot()` (`scripts/lib/paths.js`), the local basis for both `hook-drift` rows: the root doctor's own tree is compared against, and the local side of the `hook-drift:upstream` comparison. `activePluginRoot()` resolves only an INSTALLED location — the registry entry, then the newest version in the cache, then the marketplace clone — so for a plugin that runs from where it sits it answers with an unrelated leftover. Rewritten whenever those hooks fire, so it never ages out — not an orphan at any age, and removed only by an uninstall with `CLAUDEMD_PURGE=1`.
...
- `~/.claude/logs/claudemd.jsonl` — rule-hits append log (size-capped rotation at 5 MB → `.1` and `.2`). Every JS reader **of hits** goes through `rule-hits-parse.js#readLogRows`, which reads all three generations via `logGenerations` (oldest → newest); before v0.71.4 it read only the primary, so the first rotation would have silently truncated every window to whatever had accumulated since it. Two consumers deliberately stay on the primary alone because they are measuring the live file rather than reading hits: `status.js`'s `logLines` and `doctor.js`'s size check. The rotation itself is serialised by a `claudemd.jsonl.rotating` lock directory (`mkdir`, atomic, no flock on a fail-open path): two hooks passing the size check together used to interleave their renames and lose both archives, which stopped being free the moment `logGenerations` made the archives analysis input (2026-09-05 audit P1-1). The loser skips rotation rather than waiting. The reap runs BEFORE the size check and dates the lock with `stat`, not `find -mmin`: nested inside the size check it could not run at all (a killed holder leaves the log already rotated, hence under the cap), and BSD `find` rounds an age up to the next full minute, so on macOS a two-second-old lock read as stale and the loser reaped a live one. Both were found by the post-ship review of v0.76.0 and fixed in v0.76.1.
...
| Stop | `evidence-gate.sh` | G1b: a completion claim in the last assistant message with no non-error Bash result after the last code edit (Edit/Write, or a code file in a Bash result's `bashEditDiff` — that command's own result counts only by its command, not its output) whose command names a runner or whose output carries a runner verdict. Opt-in `EVIDENCE_GATE=1` (§13.3 default-OFF) | `§iron-law-2` |
| Stop | `ledger-staleness.sh` | G7 iii: a recent `tasks/<slug>-ledger.md` under the event's cwd, a code-file edit in the transcript tail (Edit/Write, or a Bash result's `bashEditDiff`), and no edit of that ledger by either channel in the same span (the window is a staleness threshold, read on both sides). Opt-in `LEDGER_STALENESS=1` (§13.3 default-OFF) | `§11-ledger` |
| Stop | `reply-language-check.sh` | The last reply has no CJK character and at least 10 words once code, URLs, paths and tags are stripped (an `API Error:` line or a conventional-commit first line is not judged), and the human is 中文: the majority language of the human's newest 20 messages (requests about language are not parsed). Human messages exclude tool results, meta, compact-summary, sidechain rows, rows whose `origin.kind` is not `human`, and machine-prefixed rows (a slash command counts by its arguments; `queued_command` attachments count). Interactive sessions only (entrypoint not `sdk-*`). Returns `{"decision":"block"}` asking for a 中文 restatement, once per turn (`stop_hook_active`). Opt-in `REPLY_LANGUAGE_CHECK=1` or `=log` (§13.3 default-OFF) | `§1-language` |
| PostToolUse | `rework-breaker.sh` | G2: per-session per-file Edit/Write/Bash-edit tally (Bash via `tool_response.bashEditDiff`, bulk commands over 20 code files skipped); injects one line at each multiple of the pre-registered threshold 8. Opt-in `REWORK_BREAKER=1` (§13.3 default-OFF) | `§1-root-cause` |
