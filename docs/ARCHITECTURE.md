...
- `~/.claude/.claudemd-state/hook-root.json` — `{"root","ts","version","sid"}` naming the directory Claude Code actually loaded the plugin from, measured from inside a running hook rather than inferred from the filesystem. Written by `hook_record_plugin_root` (`hooks/lib/hook-common.sh`) for `session-start-check.sh` and `session-end-check.sh`, both non-blocking. Read by `runningPluginRoot()` (`scripts/lib/paths.js`), which is the basis doctor's `hook-drift` row compares against: `activePluginRoot()` resolves only cache paths, so for a plugin that runs from where it sits it answers with an unrelated leftover. Rewritten whenever those hooks fire, so it never ages out — not an orphan at any age, and removed only by an uninstall with `CLAUDEMD_PURGE=1`.
