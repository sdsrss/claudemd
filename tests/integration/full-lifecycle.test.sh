...
if [[ -f "$HOME/.claude/settings.json" ]]; then
  RESIDUE=$(jq '[.hooks // {} | to_entries[] | .value[] | .hooks[] | select(.command | test("/hooks/(banned-vocab-check|ship-baseline-check|memory-read-check|pre-bash-safety-check|residue-audit|sandbox-disposal-check|session-start-check|session-summary|version-sync)\\.sh"))] | length' "$HOME/.claude/settings.json" 2>/dev/null || echo 0)
...
[[ "$MCOUNT" == "9" ]] || { echo "FAIL: manifest entry count ($MCOUNT != 9)"; exit 1; }
