...
tx "$(human "$ZH")" "$(human 'please restate the plan for the release in plain words')"
expect_silent "L7 the latest human message is English: nothing to correct" "$(run "$(stop "$EN25")")"

tx "$(human "$ZH")" "$(said "$ZH")" "$(human "$NOTIF")"
OUT=$(run "$(stop "$EN25")")
...
tx "$(human '后面都用英文回复')" "$(said "$EN25")" "$(human "$ZH")"
expect_block "L10c a request older than the latest 中文 message no longer holds" "$(run "$(stop "$EN25")")"

tx "$(human "$ZH")" "$(meta 'Caveat: the messages below were generated by the user while running local commands.')"
expect_block "L11 an isMeta row is not the human" "$(run "$(stop "$EN25")")"
tx "$(human "$ZH")" "$(compact 'This session is a summary of a previous conversation that ran out of context and is long.')"
expect_block "L12 a compact summary is not the human" "$(run "$(stop "$EN25")")"
tx "$(human "$ZH")" "$(side 'Review the commit range and report findings with file and line numbers please.')"
expect_block "L13 a sidechain row is not the human" "$(run "$(stop "$EN25")")"
tx "$(human "$ZH")" "$(result 'Tests: 43/43 passed. All suites are green and nothing was written to stderr at all.')"
...
tx "$(human "$ZH")" "$(slash polish 'run the full polish pass over the scheduler scripts')"
expect_silent "L19c English arguments in the newest command are the human writing English" "$(run "$(stop "$EN25")")"
