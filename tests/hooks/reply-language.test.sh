...
# --- L10: what the human SAYS about language is not parsed (a documented limit) ---
...
# Complaints about English, and saying which FILES are English, stay 中文 sessions.
NEG_OK=0
...
assert_eq "L10f complaints and a file-language remark still get a 中文 restatement" 5 "$NEG_OK"
tx "$(human '帮我写个 commit message')"
expect_silent "L10g a commit message the human asked for is let through (by its shape)" \
...
QM='please switch to the other plan and report back when it is done'
tx "$(human "$ZH")" "$(queued "$QM" human)" "$(queued "$QM" human)"
