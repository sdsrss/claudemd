...
test('R10-13: the join is capable of failing (mutation control)', () => {
  ...
  const mutated =
    /^(?:(?:ext-read|failopen|mem-coverage|vocab-scan|rework)-[A-Za-z0-9_*-]*(?:\.[A-Za-z0-9*-]+)?|session-start(?:-[A-Za-z0-9_*-]+)?\.ref|session-summary(?:-[A-Za-z0-9_*-]+)?\.lastrun|tmp-baseline(?:-[A-Za-z0-9_*-]+)?\.txt|last-session-summary\.json(?:\.last-shown)?|upstream-check\.lastrun|bootstrap-failed\.json(?:\.last-shown)?|hook-root\.json(?:\.[0-9]+)?|user-content-backup\.json|mem-audit\.lastrun|tmp-sweep\.(?:stamp|last\.json)|l2-task-counter|ship-baseline-recent|installed\.json|install\.lock)$/;
  const stateOnly = [...statePathsInSource()]
    .filter(([, family]) => family === 'state')
    .map(([name]) => name)
