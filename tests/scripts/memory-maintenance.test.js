...
import {
  ...
  RECALL_MAX_AGE_DAYS,
  STALE_AGE_DAYS,
} from '../../scripts/lib/memory-maintenance.js';
...
async function makeMemLiteDb(dir, project, now) {
  ...
  const ins = db.prepare(
    'INSERT INTO observations (project, title, lesson_learned, cited_count, created_at_epoch, superseded_at, demoted_at) VALUES (?,?,?,?,?,?,?)'
  );
...
test('E2 (b)+(c): recall repatriation + stale-durable candidates, in-window mention keeps a file alive', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      r.recallRepatriation.map(c => c.file),
      ['recall_old_plugin_gap.md']
    );
    assert.ok(r.recallRepatriation[0].ageDays > RECALL_MAX_AGE_DAYS);
    assert.deepEqual(
      r.staleDurable.map(c => c.file),
      ['feedback_stale_never_hit.md'],
...
test('E2 (a): promote-to-durable from mem-lite — cite>=3, age>=30d, live, same project only', async t => {
  ...
  try {
    ...
    const r = await memoryMaintenance({
      cwd,
      memDir: path.join(tmp, 'no-such-memdir'),
      logPath: path.join(tmp, 'no.jsonl'),
      memLiteDbPath: dbPath,
      now,
    });
    assert.equal(r.promoteSkipped, null);
    assert.deepEqual(
      r.promoteToDurable.map(c => c.title),
      ['hot lesson'],
...
test('a log row with no parseable ts does not count as an in-window mention', async () => {
  ...
  try {
    ...
    assert.ok(
      r.staleDurable.some(x => (x.file || x) === 'project_stale_topic.md'),
