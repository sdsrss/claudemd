...
const PINS = [
  ...
  {
    what: 'core §10 — the L1-bugfix report condition, on the same threshold as the L1 row (v6.29.0 wording)',
    file: CORE,
    anchor: '- **L1-bugfix**: single-line `Done:`',
    why: 'A threshold of 2 on Failed/Uncertain here while the L1 row uses 1 lets a single blocked item ship as a one-line Done: — the worse half of SPEC-M4.',
    line: '- **L1-bugfix**: single-line `Done:` with bugfix anchor by default; four-section when Not done/Failed/Uncertain non-empty OR scope ≥2 files.',
  },
  {
    ...
    why: 'A subagent reading core alone would otherwise emit the signal and block on a confirmation that cannot arrive; `non-hard part` is what keeps a spawn prompt from reading as the authorization.',
    line: '`[AUTH REQUIRED op:<what> scope:<files> risk:<why>]` blocks until user confirms. **Soft AUTH**: proceed, surface diff/plan inline first. Per-task, per-scope. Files outside grant → re-AUTH. **Subagent**: nobody to confirm — do the non-hard part, report `[PARTIAL: <op> needs AUTH]`, never self-authorize (§EXT §11-O).',
  },
  {
