...
**Tests.** Each changed rule line is pinned whole in `tests/scripts/spec-structure.test.js`; the six pin names carry no version, because deriving one from the spec H1 (this release's first attempt) made it a tautology that ran ahead of every pin it did not introduce. The three new pins were run against a 12-case mutation battery in a sandbox clone — paraphrase, inversion and same-anchor decoy fail the gate for all three rules, and the three neighbour-revocation cases pass, which is the limit the file's own comment documents as open. Both reviewers reproduced that battery independently and got the same 9 RED / 3 GREEN. A corpus-wide decision diff drove all 663 rows of `tests/fixtures/bash-safety/corpus.tsv` through the pre- and post-change hooks: 663 identical decisions, 0 changed. Suite: node 1081 → 1088, `version-cascade-check` consistent across 3 spec sites and 6 semver sites, hook suites and `shellcheck --severity=warning` clean.
