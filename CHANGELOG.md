...
**Tests.** node 1091 → 1134 across 4 new suites (72 → 76 files), hook suites green, `pre-bash-safety` 763 → 772 rows. Nineteen whole-line golden pins now cover the changed spec rules and the four unpinned lines they depend on; each was verified to fail under a one-line mutation of its own subject. `version-cascade-check` ok across 3 spec sites and 6 semver sites; `spec-coherence-audit --strict` exit 0.
