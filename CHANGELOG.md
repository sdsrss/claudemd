...
**Tests.** node 1091 → 1131 across 4 new suites, hook suites green, `pre-bash-safety` 763 → 772 rows. The five changed spec rule lines are pinned whole in `tests/scripts/spec-structure.test.js` and each was verified to fail under a one-line mutation of its own subject. `version-cascade-check` ok across 3 spec sites and 6 semver sites; `spec-coherence-audit --strict` exit 0.
