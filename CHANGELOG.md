...
**For contributors: the golden pins cover their neighbours.** A pin asserts one line verbatim; a bullet placed beside a rule, a preamble above it, a `### Superseded` header over it, or a clause appended to the NEXT line could revoke that rule with every gate green — four shapes demonstrated at exit 0 in an earlier review, one of them re-demonstrated against 1088 green tests. Every section of both spec files is hashed now — each `## ` section including its subsections, plus the preamble above the first heading — and a test asserts that those spans partition the file, so "the whole file is watched" is a property the suite checks rather than a percentage in a comment. The ordered inventory of every heading is hashed too, which catches a `### Superseded` inserted at the END of a section, where the section's own bytes do not move, and a duplicate of a registered heading. This does not judge what the surrounding prose means; that approach two review rounds falsified. It makes a revocation impossible to land without the hash update in the same diff, wherever in the file it is written — the pre-tag review's sharpest case was a 14-byte-**shrinking** edit inside `### §5.1` that removed §5 Hard-AUTH from what binds under `aggressive` and passed every gate in the repository, including the byte-budget one; it now fails, naming `## §5 AUTH`. The cost, deliberately taken: any spec edit anywhere fails this gate until its section hash is re-blessed, which is the prompt to re-read the rules sharing that section.

**Tests.** node 1152 → 1173 across 76 → 77 suite files; `pre-bash-safety` corpus 683 → 735 data rows (1063 → 1170 lines, the rest comments) and that suite 783 → 835 passing; hook suites 1434/1434 in total; **and the verdict differential that matters: all 683 corpus rows that existed at v0.81.0 were driven through both hooks, 0 moved**; shellcheck 64 files clean at warning+; `tests/run-all.sh` OVERALL all suites passed; `npm run check` exit 0 read from the exit code, not from grepping its output; `spec-coherence-audit --strict` C=0 H=0 M=0 L=1 (the core-headroom LOW, unchanged); `version-cascade-check` ok across 3 spec sites and 6 semver sites.

**The pre-tag review changed what ships, again.** Two fresh subagents with empty context, told to falsify. The code half returned **2 Critical, 1 High, 2 Low**, and both Criticals were in code this release had just written — false negatives on payloads bash executes, each verified here as DENY on v0.81.0 and ALLOW on the reviewed commit before anything was repaired.

The first: the separator fold asked a paren **counter** whether it sat inside `$( )`, and bash parses that recursively. A `)` closing a `case` pattern, or one inside `${x//)/-}`, desynchronises the counter — and the single-token unquote turns `")"` into a bare `)` before the walker ever runs, so the desync needs no unbalanced paren at all. Everything after it was folded, erasing the `;` and the `|` that the curl-sh regex needs, and that arm keeps `|` on purpose, so it was the one blinded while `rm` and `npx` (which split on `(`, `)` and backtick) kept denying. A body carrying a command substitution is emitted verbatim now; only a body whose sole expansion is `$VAR` is folded, which is the false-deny this release exists to close. The counter no longer decides what is hidden — only where the string ends, and getting that wrong lands in the branch that exposes.

The second: an unmatched `(` pushed the depth counter past the real closing quote, so the body read as unterminated — and the raw copy kept for exactly that case had dropped the `(` of `$(`, because the walker appended the `$` and stepped over the paren. The command word reaching all three arms was `$rm` / `$curl` / `$npx`, which none of them match. One character of state repairs it; the branch's own comment had been auditing the separators, which were intact, rather than the token in front of them.

The High was older than this release and is fixed here after a scope check: `x=$(curl … | sh)` — the ordinary spelling of a command substitution — reached no gate, because the wrapper strip reads the first whitespace-delimited token, `x=$(curl` matches the assignment prefix, and the whole token goes with the fetch word inside it. The review's proposed repair is **not** the one taken: truncating that token at the opener would put `$(date)` at the head of `FOO=$(date) rm -rf $X` and hide the `rm` from its own arm. The right-hand side is checked as an additional view instead, which can only expose more text to a deny-on-match regex. A corpus row pins the shape the rejected repair would have broken.

**Two things this release does not fix, stated rather than buried.** A quoted body carrying a real command substitution is now exposed whole, so prose like `-m "use $(date) ; rm -rf $X later"` denies as it did in v0.81.0 — a false deny with an escape token, not a miss. And a `case` pattern's `)` is a command position no anchor carries, so `$(case $x in a) curl … | sh;; esac)` allows; it allowed in v0.81.0 too. `)` is ordinary in prose, so widening the anchor for it needs its own false-positive probe set rather than a ride along with two Criticals.
