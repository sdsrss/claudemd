...
- **A backtick body inside double quotes is no longer erased before the gates see it** — that is what makes the first item reachable, and it has a cost, stated because it will reach you before this sentence does: prose that shares a double-quoted body with a backtick span now denies where it passed. `git commit -m "docs: run `make setup`; then npx prettier"` is the shape. Note what your shell does with that line: it RUNS `make setup`. Single quotes are both the way past the gate and the way to stop bash executing the snippet you are quoting.
