...
**Tests.** node 1091 → 1143 across 4 new suites (72 → 76 files), `pre-bash-safety` 763 → 783 corpus rows, hook suites green. Nineteen whole-line golden pins now cover the changed spec rules and the four unpinned lines they depend on; each was verified to fail under a one-line mutation of its own subject. `version-cascade-check` ok across 3 spec sites and 6 semver sites; `spec-coherence-audit --strict` exit 0.
...
**Budget.** Closing the two Highs put core at 24899 of 25000 — **99.6%, 101 bytes left**. Per §0.1 the next version that adds to core must remove more than it adds; there is no longer room to defer that.
