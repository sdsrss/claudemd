...
**Rotation claims the generation instead of holding a mutex.** `rule_hits_append` rotated its log under a `mkdir` mutex chosen because mkdir is atomic on every filesystem this runs on. The 0.76.2 pre-tag review found that is not true of the development host: 200 trials of four processes racing one path produced 206 wins where 200 were expected, because `/usr/bin/mkdir` there is uutils coreutils 0.8.0, the Ubuntu 25.10 default. A rotator now renames the live log to a private `<log>.rotating.<pid>.<n>` and archives from there, so ownership comes from `rename(2)` unlinking the source. On the same host `/usr/bin/mv` is GNU coreutils 9.7, and 200 trials of four processes racing one rename gave exactly one winner every time — 0 with more than one, 0 with none. Measured on a harness where exactly one rotation is legitimate (the log starts just over a 1 MB cap, so the generation left behind is a few hundred bytes): four concurrent appends over 200 trials per arm, the mutex loses an archive 8 times out of 200 with a stale lock present and the claim 0. The 8 are not a double rotation — two processes get inside the mutex, and the second, still holding a size reading from before the first rotated, carries the fresh archive onto `.2`.

**Migration, for the rotation change.** A `~/.claude/logs/claudemd.jsonl.rotating` DIRECTORY is the v0.76.0–v0.76.2 mutex. Nothing creates one any more and the next append removes it, so an upgrade needs no action. In its place a killed rotation can leave a `claudemd.jsonl.rotating.<pid>.<n>` FILE holding a full generation of rows; that is data rather than a lock, so the reaper completes the interrupted rotation instead of deleting it, placing the generation chronologically and dropping it only when two newer archives already occupy both slots. `uninstall`'s purge covers both shapes.
...
**Two reviewers were given the range with empty context and told to falsify it, and they blocked the tag.** Both findings that did are in code this release added, and both silently destroyed a full generation of rule-hits rows — at the 5 MB default cap, roughly 190 days of it. The reaper aged a claim by mtime, which `rename(2)` PRESERVES, so an in-flight claim carried the age of its rows rather than of the claim: whenever the previous append was over a minute ago, a concurrent append reaped a live rotation and deleted what it held. Measured at 31/200 on 32-way concurrency — and 0/200 with a freshly written log, which is the only condition this release's own harness ever built, so it measured the case where the defect is unreachable. Stamping the file after the rename does not close it either (41/200): `mv` then `touch` is two steps and a reaper lands between them. The claim now carries its birth time in its NAME, written by the rename itself, and the same arm measures 0/200. Second: orphan placement assumed the orphan was the oldest generation, which holds only when the crash is followed by two rotations; in the ordinary sequence it is the NEWEST, so the branch deleted the newest and kept two older ones with no concurrency at all — a regression against v0.76.2, which had no orphan class and self-healed at that kill point. Placement is now by measured mtime, keeping the two newest of three. The ordering rationale that justified the original branch was also wrong: `logGenerations` returns a fixed path list and every consumer filters on each row's own `ts`, so nothing reads file order.

Also from that round: a live symlink whose target is UNREADABLE rather than absent was being deleted without a backup and with a warning that said the target does not exist; three comments and a paragraph of these notes claimed the §8 gate was untouched when its reverse-shell arm was not; `CLAUDEMD_LOG_MAX_MB` values large enough to overflow int64 wrapped to a zero cap, rotating on every append; a directory at an archive path swallowed every generation; and an unmatched glob under `failglob` returned non-zero from the append, dropping the telemetry row. Each is fixed with a test that fails against the code before it.

Verified: `npm run check` exit 0, node 1061 to 1069 pass / 0 fail, bash hook suites 1283 to 1302 pass / 0 fail plus 13 rule-hits rotation cases, the §8 corpus 707 to 717, shellcheck clean at warning+, and the version cascade reporting 0.77.0 across 6 sites. Every defect in this release carries a test confirmed to fail against the code before it.
