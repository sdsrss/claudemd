...
**Migration**: `/claudemd-refresh`, then restart Claude Code. That is the whole procedure, and it is worth being precise about what it does, because this release changes a file you own — `~/.claude/CLAUDE.md` §8. **The spec is overwritten automatically and without a prompt.** SessionStart sees the version mismatch and runs `install.js`, which treats a spec-shaped `~/.claude/CLAUDE.md` as claudemd's own file and copies the new one straight over it. Running `/claudemd-update` afterwards shows a zero diff — by then there is nothing left to accept. On an upgrade there is also **no backup**: `install.js` backs up `~/.claude/CLAUDE.md` only when it does *not* look like a spec, on the reasoning that there is no user-owned content to preserve.

> **Back up `~/.claude/CLAUDE.md` before upgrading if you keep your own text in it.** "Looks like a spec" is decided by `looksLikeSpec` — an `# AI-CODING-SPEC` line anywhere in the first 256 bytes — so a file that is the spec **with your notes appended below it**, which is the natural way to keep them there, still counts as spec-shaped. It is overwritten with no backup and no warning. Measured on this release's own `install.js`, four sandbox homes: spec-with-appended-notes and notes-prepended-above-the-spec both report `userContentDetected: false`, produce no `backup-` directory, and lose the text; only a file with no `# AI-CODING-SPEC` line at all is backed up and warned about, and a user in that state is not running the spec. This is not new in 0.83.0 — it is how every upgrade since the predicate was written has behaved — but it is the first time it is written down, and it was found by the pre-tag review of this entry, which had claimed the opposite.

**Way back**: for the spec, the only route is not to take the upgrade. Pin the marketplace entry to `v0.82.0` and run `CLAUDEMD_ALLOW_DOWNGRADE=1 node scripts/install.js` from a `v0.82.0` checkout — `install.js` refuses a downgrade without that variable, and it is the only thing that repoints both the plugin manifest and the installed spec. `docs/ROLLBACK.md` § "Local machine needs the previous version back" carries that procedure. Declining `/claudemd-update` is **not** a way back, and an earlier draft of this entry said it was. Since no hook changed, there is no new deny to escape from in this release.
