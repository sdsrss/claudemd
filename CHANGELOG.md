...
One spec change: **AI-CODING-SPEC v6.26.0 → v6.27.0**, the harness-convergence release the 2026-09-06 spec audit asked for. Core 23944 → 23478 bytes, extended 45899 → 44526 bytes, no new HARD rule, enforcement partition unchanged at 6 hook / 16 self / 2 both / 1 external. The full clause list is the v6.27.0 entry in `spec/CLAUDE-changelog.md`.
...
The first draft of the fix anchored candidates at a short list of separators, keywords and wrappers. The pre-tag review — a fresh subagent told to falsify, 60 probes against this hook and the 0.78.0 one — found three regressions in that draft: 22 executable positions the old hook denied and the draft allowed (`/usr/bin/python3`, `./venv/bin/python`, `PYTHONPATH=. python3`, `if python3 -c`, `! python3`, `find -execdir`, `>/dev/null python3`, `docker exec c python3`, `strace -f python3`, `su -c "…"`, `ssh h "…"` among them); a two-line heredoc trick (`echo "<<EOF"`, the one-liner, a bare `EOF`) that the shared heredoc stripper read as a real heredoc because it has no quote state; and the un-dotted call family (`from subprocess import call`, `import subprocess as sp; sp.call(`, `os.system ('…')` with a space) that the call-form `REVSH_EXEC` no longer matched. What ships: the anchor is "command position, then any chain of quote-free or simply-quoted tokens, then an optional path prefix" — a bare token before the interpreter is a wrapper, keyword, assignment or redirect and is not modelled further, a quoted token is consumed whole and never reaches the interpreter, which is exactly what keeps the five data shapes allowed. This arm reads heredocs through its own awk with quote state (an opener's `<<` must sit outside quotes; the tag may be quoted, as in `<<'EOF'`); `su -c` and `ssh host '…'` payloads are unwrapped the way `sh -c` already was; `REVSH_EXEC` accepts whitespace before the paren and the un-dotted forms when the same one-liner imports the module, with an `import subprocess as X` alias read out of the text and tested as a dotted call. Corpus: 15 R5 rows (5 red before the first draft, 727/732) plus 21 R6 rows (19 red against the first draft, 734/753); 753/753 after. Residual, stated in the hook: a quoted string that itself contains a separator before the one-liner (`git commit -m "x; python3 -c '…'"`) still anchors on the `;`; heredoc-fed interpreters (`python3 - <<EOF`), `$'…'` payloads and version-suffixed interpreter names were never covered and still are not — the review lists them as pre-existing gaps. Latency: two ten-run measurements of the same hook on this host disagree by more than the change (63 ms versus 146–162 ms for one ordinary command), so no latency figure is claimed beyond "one added awk pass".

**Tests.** The three verbatim spec pins in `tests/scripts/spec-structure.test.js` are re-pinned to the shortened §11 / §11-O / §12 lines in the same commit as the rule text, which is the only way a whole-line pin is allowed to move. The independent mutation battery in `tests/tools/spec-pin-mutations.py` still targets the v6.26.0 wording and is not run in CI; it is left as the record of that review rather than rewritten against lines it never reviewed. Suite: node 1074/1074, bash hook suites all passed (`pre-bash-safety` 753/753), one `memory-tags-parity` case failed once inside a full run and passed on three isolated re-runs and every later full run — the intermittent the Round-14 audit filed as REL-M2, not something this release touches.
