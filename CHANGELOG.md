...
One spec change: **AI-CODING-SPEC v6.27.0 → v6.28.0**, closing the three findings the 2026-09-06 audit left open in its §8 after v0.79.0 shipped the rest. Core 23478 → 24468 bytes, extended 44596 → 44431 bytes, no new HARD rule; the enforcement partition moves 6 hook / 16 self / 2 both / 1 external → 6 / 17 / 2 / 1 because a layer that was always self-enforced is now counted. The clause list is the v6.28.0 entry in `spec/CLAUDE-changelog.md`; the round's execution record is §11 of `docs/audit/20260906-231957.md`.
...
**What changes for you.** Three things Claude will do differently. It will treat an explicit instruction from you as permission to relax a spec clause — the §3 order ranks conflicts, not permissions — with the §5.1 Never-downgrade set as the floor, so asking for an English report or a different routing no longer loses to a spec default. It will stop reaching for a hook's `[allow-…]` escape token on its own: the four deny messages that advertise those tokens now say the authorization is yours, and the spec says the same. And §5's twelve hard-AUTH categories carry an explicit `self-enforced` tag, which is a statement of fact rather than a new restriction — no hook has ever verified that `[AUTH REQUIRED]` was emitted before a delete, a migration or a config write. **Migration**: run `/claudemd-update`; it shows the diff, waits for your choice, and backs the previous `~/.claude/CLAUDE*.md` trio up to `~/.claude/spec-backup-<stamp>/` before writing. **Way back**: pin the marketplace entry to `v0.79.0` and then run `/claudemd-update` — the pin alone leaves `~/.claude/` where it is, because `install.js` refuses a downgrade unless `CLAUDEMD_ALLOW_DOWNGRADE=1`. `docs/ROLLBACK.md` carries the per-failure-point routes.

**Why these three, and why now.** Each is a rule that only the agent can enforce, presented as if something mechanical stood behind it.

- **§5 AUTH had no row in `spec/hard-rules.json`.** Twelve hard-AUTH categories, and the §13.1 demote review and `/claudemd-rules` could not account the layer at all, while `scripts/sampling-audit.js` had been running a `§5-hard-auth` detector against it since v0.28.0. The layer is also fully agent-enforced, which this release verified rather than assumed: a literal `rm -rf src/` fed to `pre-bash-safety-check.sh` in a scratch state dir returns exit 0, because `§8-rm-rf-var` fires only on unvalidated variables. The manifest entry carries `rule_hits_section: null` — its observability channel is the transcript scanner, not the hook taxonomy, and claiming otherwise would have failed `hard-rules-2`.
- **The escape tokens were never declared.** `[allow-rm-rf-var]`, `[allow-npx-unpinned]`, `[allow-curl-sh]`, `[allow-banned-vocab]`, `[skip-memory-check]` and every `DISABLE_*_HOOK` are advertised in deny text to whoever reads it, which under `bypassPermissions` is the agent. The 30-day rule-hits window carries 3 rm-rf and 3 memory-read bypasses with nothing recording who authorized them. §8 now names all five literals and states that self-issuing one is a §5 breach and, on a §8 pattern, a §8 one.
- **§3 ranked the spec above the current-turn user with no rule for relaxation.** §5.1, §8.V3 and §13 each already carved out a local user override; read with "stricter reading wins", the spec pushed toward overruling the person typing. The audit's own sample: a user asked for an English report and core §1's default outranked them. The new clause reuses the Never-downgrade set as its floor rather than inventing a second list.

**One new measure, not a rule.** `scanAskRate` in `sampling-audit.js` reports answered asks per task and how many the user answered with assent rather than direction, rendered as a "Default-ASK cost (H4)" section beside C1. The audit refused to resolve the §0 default-ASK question without this number. Its disposition is pre-registered and fixed before the first run — with at least 30 answered asks, an assent rate at or above 50% flips §0 to default (b) — in the same shape as `OVER_CEREMONY_THRESHOLD`, and a test asserts both constants so a later retune is a visible diff. Status is `collecting` and precision is null: the ask predicate is `YIELD_ASK_RE`, written as the turn-yield precondition where over-matching is the safe direction, so a turn closing on a rhetorical question scores as an ask. First run on this repo, 30-day window: 11 transcripts, 398 turns, 86 task segments, 9 answered asks, 0 assent — below the 30-ask bar, so it decides nothing, which is what the bar is for. Its task denominator folds an answer back into the task that provoked the ask (§1.5 continuation) and so is smaller than C1's on the same transcript; a test pins that the two stay different rather than drifting into one.

**Budget.** Core is now 24468 of its 25000-byte cap — 97.9%, 532 bytes left — and `spec-coherence-audit` reports that as a LOW. v6.27.0's net-delete is what paid for these three clauses and the room is spent; per §0.1 the next version adding to core must remove more than it adds. Extended fell 165 bytes. The two remaining items from the audit's §7.2 are blocked rather than deferred by choice: deleting the §2.1 skill-wording clause needs at least 30 L0/L1 task segments and this repo's 30-day window yields 5, and pinning a tokenizer-measured size needs an `ANTHROPIC_API_KEY` this environment does not have — the spec already took the audit's stated fallback in v6.27.0 and carries no token estimate at all.

**Tests.** Each changed rule line is pinned whole in `tests/scripts/spec-structure.test.js`, and the pin loop now reads its version label from the spec instead of carrying a typed one, which is the drift a 2026-07-25 audit found when a test name sat a version behind its own assertion. The three new pins were run against a 12-case mutation battery in a `git archive` sandbox clone: paraphrase, inversion and same-anchor decoy fail the gate for all three rules, and the three neighbour-revocation cases pass — that is the limit the file's own comment documents as open, reproduced here rather than newly found. Suite: node 1081/1081, `spec-coherence-audit --strict` 4/5 clean with the one LOW above, `version-cascade-check` consistent across 3 spec sites and 6 semver sites, hook suites and `shellcheck --severity=warning` clean.
