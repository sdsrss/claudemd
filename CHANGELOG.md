...
What does exist is a measurement. Hooks are the one component Claude Code launches from the directory it loaded, and every hook already resolves its own location from `${BASH_SOURCE[0]}` — a `$` the shell expands inside a process Claude Code started, which is exactly the observer no later script can be. `session-start-check.sh` and `session-end-check.sh` now record that directory into `~/.claude/.claudemd-state/hook-root.json` through `hook_record_plugin_root`, a non-blocking writer that skips a root carrying a quote, a backslash or a newline rather than leaving a half-written file behind. `runningPluginRoot()` (`scripts/lib/paths.js`) reads it back, checks the recorded path still exists, and falls back to `activePluginRoot()` when there is no record — so a machine where hooks never fired, or one that has not restarted since installing this version, behaves exactly as it did on 0.84.1. Both drift rows compare against it. An in-place install therefore compares its own directory against itself and receives `compareHooks`'s `self-compare` skip: green by measurement rather than silent by policy.

**What changes in the exit code, and for whom.** Two skips make counting safe, not one, and the second was added in the pre-merge review after the first turned out to be insufficient on its own. The self-compare skip above covers the callers the row could not identify. The second covers a state the record itself can be in: `hook-root.json` is one global file and the last hook to write it wins, so a user who upgrades while an older session is still open in another terminal gets that session's closing hook writing the **old** cache directory into the record. doctor in the live session then compares two real installs, reports drift, names a cause that is false and advises `/claudemd-refresh`, which cannot clear it — only the next SessionStart rewrites the record. `plugin-root:stale-registration` does not catch that one, because it needs the running version to be *older* than the registered one and here doctor's own root is the registered one. So `hook-drift` now also skips when doctor's root and the recorded root are two **different** directories inside the plugin cache: that is a version transition, and the pending-restart question already has its own row.

With both skips in place, a red `hook-drift` moves `/claudemd-doctor` to exit 3 again. The row compares, and counts, when doctor's own tree and the recorded root differ and the two are **not both inside the plugin cache** — so it takes a checkout, a `--plugin-dir` tree or another root that runs where it sits on one side of the pair. A machine whose every root is a cache install cannot reach it by construction: the two roots are then either the same directory (self-compare) or two cache directories (the skip above), and the no-record fallback, `activePluginRoot()`, resolves nothing but cache paths. Who feels it is therefore who it always was — someone with a checkout **and** an install — but not only in the shape that reads as obvious.

The record is global, so the pair is not always "the tree you launched doctor from versus your own session". Run `/claudemd-doctor` from the ordinary cache install while a **checkout** session fired the last hook, and the row goes red about a root you did not launch: measured — exit 3, `hook-drift` the only counted red, the detail naming the checkout. Nothing is wrong on that install; two sessions disagree about which root is running and the last writer won. That is why the row names the recording session alongside the timestamp — `via hook-fired at <ts> (sid <id>)` — so a reader can tell "my tree has drifted" from "another session wrote this basis" instead of guessing, and why the condition clears on the next SessionStart from the session you care about rather than on a reinstall. `hook-drift:upstream` stays advisory on the 0.84.0 reasoning, which this release does not disturb — the marketplace clone tracks `main` rather than the released tag, so its red is a legitimate steady state between releases.

**No kill switch ships with the flip, and that is a decision rather than an oversight.** The skip above leaves no ordinary install for the change to reach, and a switch that nobody can be in a position to need is a switch that rots unexercised and is wrong by the time someone reaches for it. The way back is the documented one: `CLAUDEMD_ALLOW_DOWNGRADE=1 node scripts/install.js` from a checkout of `v0.84.1`, per `docs/ROLLBACK.md`.
