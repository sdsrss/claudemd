...
**Tests.** node 1091 → 1150 across 4 new suites (72 → 76 files), `pre-bash-safety` 763 → 783 corpus rows, hook suites green. Twenty-three whole-line golden pins now cover the changed spec rules and every line they depend on; each was verified to fail under a one-line mutation of its own subject. `version-cascade-check` ok across 3 spec sites and 6 semver sites; `spec-coherence-audit --strict` exit 0.
...
**Budget.** Closing the Highs put core at 24906 of 25000 — **99.6%, 94 bytes left**. Per §0.1 the next version that adds to core must remove more than it adds; there is no longer room to defer that.
...
Also from that round: §0's Signals bullet still said `[AUTH REQUIRED]` blocks, unqualified, so §3's stricter reading resolved against the subagent clause §5 had just gained; and §5's "the authorized part" was undefined for a subagent, which could read its spawn prompt as the authorization — self-authorization by another name, which the next clause forbids. Both corrected in nineteen bytes.
