...
**For contributors: the golden pins cover their neighbours.** A pin asserts one line verbatim; a bullet placed beside a rule, a preamble above it, a `### Superseded` header over it, or a clause appended to the NEXT line could revoke that rule with every gate green — four shapes demonstrated at exit 0 in an earlier review, one of them re-demonstrated against 1088 green tests. Each pinned line's markdown block is hashed now, 13 blocks over 26 pins, so a neighbour inside a pinned rule's own block cannot be added without the hash update landing in the same diff. The ordered inventory of every heading in both spec files is hashed too, which is what catches a `### Superseded` inserted at the END of a block — the case that leaves the block's own bytes untouched — and a duplicate of a registered heading. This does not judge what the surrounding prose means; the approach two review rounds falsified. **What it still does not cover**, because the pre-tag review measured it rather than letting it be discovered later: a section holding no pinned line is not watched, so the gate sees 59.1% of core's bytes and 15.3% of extended's. A sentence added under a pin-free heading can still weaken a rule two headings away. Unrelated edits inside a pinned block fail this gate; that is the intended cost.
