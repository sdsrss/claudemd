...
**What changes in the exit code, and for whom.** Because that skip now covers the callers the row could not identify, counting it can no longer hand an ordinary install a standing exit 3 — so it counts. A red `hook-drift` moves `/claudemd-doctor` to exit 3 again. Who feels that: a maintainer with a checkout **and** an install, the caller the axis was written for, and only after the tree they run doctor from genuinely differs from the plugin their session is firing hooks out of. `hook-drift:upstream` stays advisory on the 0.84.0 reasoning, which this release does not disturb — the marketplace clone tracks `main` rather than the released tag, so its red is a legitimate steady state between releases.
