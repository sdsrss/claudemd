...
**The long-task ledger survives compaction.** `tasks/<slug>-ledger.md` has five sections; `session-start-check.sh` re-injects two of them on `SessionStart` with `source` `compact` or `resume`. Not the work — that is in the repo — but the Decisions, which are filled once at task start and are what a later turn cannot re-derive, and Next. Bounded to the newest ledger, a 14-day window and 1600 bytes, because it lands in context at the moment context is scarce. Format and rationale: `docs/ARCHITECTURE.md`, "Long-task ledger (G7)".
