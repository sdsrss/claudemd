...
Seven commits executing sections 0a through G7 of `docs/spec-optimization-roadmap-2026-09-21.md`, which measured 178 transcripts and found that the rules governing whether a claim has evidence behind it are the rules nothing mechanical checks. Of the 26 HARD rules in `spec/hard-rules.json`, 16 are marked `self`, and they are exactly the ones about honesty: Iron Law #2, the §10 honesty rules, §11 turn-yield. Hook coverage of that set was zero. This release moves the mechanically decidable part of it hook-side and instruments the two behaviours the measurement found dominant.
...
- `evidence-gate.sh` (Stop, `EVIDENCE_GATE=1`) fires when the last assistant message makes a completion claim, the session edited code files, and the transcript holds no non-error `Bash` `tool_result` carrying test / typecheck / build output after the last code edit. It judges STRUCTURE, never prose. That distinction is the whole design: the 2026-07-24 labeling pass hand-labeled all 178 flagged instances across six detectors that read prose for evidence — hunting for `Checked:`, for digits, for the word "passed" — and closed all six at a pooled precision upper bound of 0.17. Prose has unbounded shapes. "Is there a command result after the last edit" does not.
- `rework-breaker.sh` (PostToolUse on `Edit|Write`, `REWORK_BREAKER=1`) counts edits per file per session and injects one line at each multiple of 8. The threshold was fixed before the data was collected: 58.7% of sessions with any edit reached 8 or more on their hottest file, the modal bucket was 8-14, and one session reached 58 on a single file. §1 "Root cause over patch" already forbids that loop and is pure prose, so nothing observed it while it was running. The line carries the only fact the agent does not otherwise have — an agent mid-loop knows it edited the file again, not that this was the sixteenth time.
...
Kill-switches: `DISABLE_EVIDENCE_GATE_HOOK`, `DISABLE_REWORK_BREAKER_HOOK`, and the sub-feature `DISABLE_LEDGER_INJECT` for the ledger half of the compaction banner.
