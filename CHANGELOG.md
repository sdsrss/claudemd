...
- **`hook-drift:upstream` is advisory.** Not because it lies — it is true when it fires — but because the marketplace clone tracks `main`, not the released tag, and of the last 60 commits 19 touch `hooks/*.sh` with 18 carrying no version bump. For anyone who has run `/plugin marketplace update` on its own since such a commit, the running hooks legitimately differ from the marketplace, and counting that would leave `/claudemd-doctor` exiting 3 for most of the window between releases. It still prints. `hook-drift` and `statusline` stay counted.
