...
**One hook fix: the §8 reverse-shell arm denied its own text as data.** Pattern 3d (an interpreter one-liner that opens a socket AND executes) extracted candidates from anywhere in the command with no command-position anchor and with heredoc bodies intact, and counted a bare module name as an execution primitive. So a `python3 -c` one-liner echoed into a notes file, quoted in a commit message, grepped for, or sitting inside a heredoc body denied, and so did `import socket, subprocess, os` with nothing called — five shapes reproduced in a sandbox HOME before the fix (Round-14 audit HK-H1). The Bash call that appended those five as corpus rows through a heredoc was itself denied by the defect it was recording, and so was the call that first tried to write this paragraph. Candidates now start at command position (line start, one of `|;&(){}` or a backtick, a `then`/`do`/`else`/`-exec` keyword, optionally behind a wrapper from the shared S8 taxonomy plus `xargs`) in a view with heredoc bodies blanked, and `REVSH_EXEC` requires a call form — `subprocess.call(`, `os.popen(`, `execSync(` — rather than a module name. Corpus: 15 R5 rows, 5 of them red before the change (727/732) and every row green after (732/732); the ten deny rows pin the shapes the anchor must keep reaching — `&&`, `$( )`, backticks, `timeout 30`, `env FOO=1`, `find -exec`, `xargs`, `bash -c`, `execSync(`, `os.popen(`. Latency on the deny path 94 → 101 ms and on ordinary commands 61 → 63 ms over ten runs each, the heredoc strip being the only added pass. Residual, stated in the hook: a one-liner inside a quoted string that itself contains a separator still anchors on the `;`.

**Tests.** The three verbatim spec pins in `tests/scripts/spec-structure.test.js` are re-pinned to the shortened §11 / §11-O / §12 lines in the same commit as the rule text, which is the only way a whole-line pin is allowed to move. The independent mutation battery in `tests/tools/spec-pin-mutations.py` still targets the v6.26.0 wording and is not run in CI; it is left as the record of that review rather than rewritten against lines it never reviewed. Suite: node 1074/1074, bash hook suites all passed, one `memory-tags-parity` case failed once inside the full run and passed on three isolated re-runs and the next full run — the intermittent the Round-14 audit filed as REL-M2, not something this release touches.
