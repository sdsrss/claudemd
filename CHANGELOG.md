...
**What changes for you.** Claude will now end a turn while it waits for a subagent it spawned, naming who it is waiting for, where it previously held the turn open. Nothing is required of you when it does: the subagent's completion re-invokes Claude on its own, with no input from you. The visible difference is that a long orchestrated task shows a short "waiting on `<name>`" turn where it used to show a long silence. **Migration**: run `/claudemd-update`; the installer backs up the previous `~/.claude/CLAUDE*.md` trio before overwriting it, and there is nothing else to do. **Way back**: pin the previous plugin version (`claudemd-cli@0.77.0` on npm, or the marketplace version pin), or restore the trio from the `backup-<stamp>/` directory the installer wrote. The change is a permission — one more legal reason to yield — so reverting removes that permission and leaves no state to unwind.

**The measurement.** Taken from the v0.77.0 ship's own session transcript rather than from recollection. That ship ran as **one turn of 3h02m across 126 tool calls with no yield**, because §11 permitted no stop before VALIDATE. Two review subagents finished inside it and **zero** of their completion notifications reached context across 1917 transcript records; the queue flushed **230 ms** after the closing text block. `ListAgents` reported `reviewer-rotation · idle` at 16:57:45 and its report was in hand at 17:09:40 — 12 minutes later, and only through a side-channel improvised mid-turn. Sixteen `sleep` calls totalling 6870 s, 64% of the window, each returned nothing: sleeping does not hasten a turn-end delivery, it postpones the turn end that IS the delivery. Six messages sent to already-idle teammates produced six further empty completions, which is six of the eight `finished` events that arrived at once when the turn finally closed.

**No new HARD rule.** §13.2's ratchet sends a new incident to `tasks/rule-candidates-<YYYY-MM>.md`, not into the spec, and its evidence-rebuttal shortcut says an existing HARD shown in session evidence to produce wrong behaviour is fixed rather than wrapped. So §11 Mid-SPINE turn-yield takes a fourth yield trigger — awaiting a spawned subagent — alongside `[AUTH REQUIRED]`, genuine ambiguity and context pressure; the yield must name what is awaited, which leaves untouched the anti-silence clause the rule exists for. Sleep-polling and re-messaging an idle teammate are written in as banned, because both were tried and neither delivered. §EXT §12's manual-ship atomicity takes a second exception covering the step-7 pre-tag review wait, and that half is what breaks the deadlock: author ≠ reviewer is HARD and step 7 sits between the push and the tag, inside the window that forbade turn-ending. §EXT §11-O records the delivery fact plus the fallback for a cycle that genuinely cannot yield — name an absolute output path in the spawn prompt and poll that file, since `TaskOutput` is deprecated for local agents and warns that a local agent's output file is a symlink to the full JSONL transcript, and the notification channel is not pollable.

Enforcement partition unchanged at 6 hook / 16 self / 2 both / 1 external: `§11-mid-spine-yield` keeps its `section_anchor` and `enforcement: self`, and no rule was added or removed. Two `spec-structure` tests pin the joint invariant rather than either clause alone, because removing either half restores the deadlock — 15/17 pass with the pre-change spec stashed, 17/17 after. Suite: node 1069 → **1071/0**, bash hook suites **1302/1302**, `version-cascade-check` ok at v6.26 across 3 files with Sizing drift inside ±20B. Core spec 23445 → 23755 bytes (1245 under the 25000 cap); extended 43819 → 45561 (4439 under 50000).
