...
- **An escaped quote in a double-quoted body no longer manufactures a command position.** `git commit -m "note: \" ; npx some-package\" is the shape we ban"` used to DENY: the single-token unquote paired the opening quote with the ESCAPED one, unwrapped `"note: \"`, and handed the rest over as bare text — the very boundary its own comment says cannot be manufactured from a quoted string. Bodies that carry a `$` or a backtick are preserved whole either way, so this shows up on the `$`-free ones.
