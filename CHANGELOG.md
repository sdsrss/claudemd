...
**The rotation mutex guarded the two moves, not the decision to move.** v0.76.0's `mkdir` mutex was adjudicated closed on a test that holds the lock across the second append — which can only ever reach the arm where `mkdir` fails. The arm that loses data is the one where it succeeds a moment too late: P2 reads the size while the log is still over the cap, P1 rotates and releases, and P2's `mkdir` then wins an uncontested lock and re-runs the two-step move on a log that is already rotated. `mv .1 .2` carries P1's fresh archive onto `.2` and `mv log .1` fails silently, leaving `{gone, gone, live-as-.2}` — the signature the mutex was added to prevent, under the mutex. Measured with four concurrent appends over an over-cap log, 200 trials per arm: 6/200 lost both archives with no lock involved at all, 33/200 with a stale lock also present. The size that authorizes a rotation is now re-read inside the mutex, so a second holder finds a log that is gone or under the cap and does nothing — 0/200 and 8/400 after. `ROT-6` pins it with a counting `stat` shim that answers the first size query over-cap and later ones truthfully, which is precisely the state the losing process is in.

**Left in place, on purpose**: the stale-lock reap is still check-then-`rmdir`, so a reaper that decided "stale" can remove a lock another process acquired in between (8/400, and only when a stale lock is already present). The `mv "$lock" "$lock.reap.$$"` recipe proposed for it does not close it, which was measured rather than assumed — 6/400 on the same fixture, because `rename(2)` is atomic about the move, not about the identity of what it moved. Claiming the live generation by rename would close it and introduces an orphan class needing its own reaper; the residual costs archived telemetry, never a gate decision or user data. The numbers and the design sketch are recorded at the reap branch.
